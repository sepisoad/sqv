...
/*
    ...
    On Windows, SOKOL_DLL will define SOKOL_ARGS_API_DECL as
   __declspec(dllexport) or __declspec(dllimport) as needed.

    OVERVIEW
    ========
    sokol_args.h provides a simple unified argument parsing API for WebAssembly
   and native apps.
    ...
    Such value-less keys are reported as the value being an empty string, but
   they can be tested with `sapp_exists("bla")` or `sapp_boolean("blub")`.
    ...
    The 'key' string must be a simple string without escape sequences or
   whitespace.
    ...
    ============
        ...
        int main(int argc, char* argv[]) {
            ...
            // check if a key's value is "true", "yes" or "on" or if this is a
   standalone key if (sargs_boolean("joystick_enabled")) {
                ...
            }
   ...
   sargs_value_at(index));
    ...
    void sargs_setup(const sargs_desc* desc)
        ...
        parameters:
            ...
            int buf_size    - size of the internal string buffer, default is
   16384
...
// get value by key name, return empty string if key doesn't exist or an
// existing key has no value
SOKOL_ARGS_API_DECL const char* sargs_value(const char* key);
// get value by key name, return provided default if key doesn't exist or has no
// value
SOKOL_ARGS_API_DECL const char* sargs_value_def(const char* key,
                                                const char* def);
...
// return true if key's value is "true", "yes", "on" or an existing key has no
// value
...
inline void sargs_setup(const sargs_desc& desc) {
  return sargs_setup(&desc);
}
...
#error \
    "SOKOL_MALLOC/CALLOC/FREE macros are no longer supported, please use sargs_desc.allocator to override memory allocation functions"
#endif
...
typedef struct {
  ...
  char* buf;  // character buffer, first char is reserved and zero for 'empty
              // string'
  bool valid;
...
_SOKOL_PRIVATE bool _sargs_any_expected(void) {
  return 0 != (_sargs.parse_state &
               (_SARGS_EXPECT_KEY | _SARGS_EXPECT_VAL | _SARGS_EXPECT_SEP));
}
...
SOKOL_API_IMPL void sargs_setup(const sargs_desc* desc) {
  ...
  _sargs.args = (_sargs_kvp_t*)_sargs_malloc_clear((size_t)_sargs.max_args *
                                                   sizeof(_sargs_kvp_t));
  _sargs.buf =
      (char*)_sargs_malloc_clear((size_t)_sargs.buf_size * sizeof(char));
  // the first character in buf is reserved and always zero, this is the 'empty
  // string'
  _sargs.buf_pos = 1;
...
SOKOL_API_IMPL bool sargs_boolean(const char* key) {
  if (sargs_exists(key)) {
    ...
    return (0 == strcmp("true", val)) || (0 == strcmp("yes", val)) ||
           (0 == strcmp("on", val)) || (0 == strcmp("", val));
  } else {
    return false;
  }
}

...
/*
    ...
    SOKOL_UNREACHABLE()         - a guard macro for unreachable code (default:
   assert(false)) SOKOL_GFX_API_DECL          - public function declaration
   prefix (default: extern) SOKOL_API_DECL              - same as
   SOKOL_GFX_API_DECL SOKOL_API_IMPL              - public function
   implementation prefix (default: -) SOKOL_TRACE_HOOKS           - enable trace
   hook callbacks (search below for TRACE HOOKS) SOKOL_EXTERNAL_GL_LOADER    -
   indicates that you're using your own GL loader, in this case sokol_gfx.h will
   not include any platform GL headers and disable the integrated Win32 GL
   loader
    ...
    On Windows, SOKOL_DLL will define SOKOL_GFX_API_DECL as
   __declspec(dllexport) or __declspec(dllimport) as needed.
    ...
    --- start a render- or compute-pass:
        ...
        Typically, render passes render into an externally provided swapchain
   which presents the rendering result on the display. Such a 'swapchain pass'
        is started like this:

            sg_begin_pass(&(sg_pass){ .action = { ... }, .swapchain =
   sglue_swapchain() })

        ...where .action is an sg_pass_action struct containing actions to be
   performed at the start and end of a render pass (such as clearing the render
   surfaces to a specific color), and .swapchain is an sg_swapchain struct with
   all the required information to render into the swapchain's surfaces.

        To start an 'offscreen render pass' into sokol-gfx image objects, an
   sg_attachment object handle is required instead of an sg_swapchain struct. An
   offscreen pass is started like this (assuming attachments is an
   sg_attachments handle):

            sg_begin_pass(&(sg_pass){ .action = { ... }, .attachments =
   attachments });
    ...
    --- fill an sg_bindings struct with the resource bindings for the next
        draw- or dispatch-call (0..N vertex buffers, 0 or 1 index buffer, 0..N
   images, samplers and storage-buffers), and call:

            sg_apply_bindings(const sg_bindings* bindings)

        to update the resource bindings. Note that in a compute pass, no vertex-
        or index-buffer bindings are allowed and will be rejected by the
   validation layer.
    ...
    --- optionally update shader uniform data with:
        ...
        Read the section 'UNIFORM DATA LAYOUT' to learn about the expected
   memory layout of the uniform data passed into sg_apply_uniforms().

    --- kick off a draw call with:

            sg_draw(int base_element, int num_elements, int num_instances)

        The sg_draw() function unifies all the different ways to render
   primitives in a single call (indexed vs non-indexed rendering, and instanced
   vs non-instanced rendering). In case of indexed rendering, base_element and
   num_element specify indices in the currently bound index buffer. In case of
   non-indexed rendering base_element and num_elements specify vertices in the
   currently bound vertex-buffer(s). To perform instanced rendering, the
   rendering pipeline must be setup for instancing (see sg_pipeline_desc below),
   a separate vertex buffer containing per-instance data must be bound, and the
   num_instances parameter must be > 1.
    ...
    --- to set a new viewport rectangle, call:
            ...
            sg_apply_viewport(int x, int y, int width, int height, bool
   origin_top_left)

        ...or if you want to specify the viewport rectangle with float values:

            sg_apply_viewportf(float x, float y, float width, float height, bool
   origin_top_left)

    --- to set a new scissor rect, call:

            sg_apply_scissor_rect(int x, int y, int width, int height, bool
   origin_top_left)

        ...or with float values:

            sg_apply_scissor_rectf(float x, float y, float width, float height,
   bool origin_top_left)

        Both sg_apply_viewport() and sg_apply_scissor_rect() must be called
        inside a rendering pass (e.g. not in a compute pass, or outside a pass)

        Note that sg_begin_default_pass() and sg_begin_pass() will reset both
   the viewport and scissor rectangles to cover the entire framebuffer.
    ...
    --- to append a chunk of data to a buffer resource, call:
        ...
        buffer piece by piece, optionally interleaved with draw calls
   referencing the previously written data.
        ...
        for (...) {
            const void* data = ...;
            const int num_bytes = ...;
            int offset = sg_append_buffer(buf, &(sg_range) { .ptr=data,
   .size=num_bytes }); bindings.vertex_buffer_offsets[0] = offset;
    ...
    --- to check at runtime for optional features, limits and pixelformat
   support, call:
    ...
    --- you can query the default resource creation parameters through the
   functions

            sg_buffer_desc sg_query_buffer_defaults(const sg_buffer_desc* desc)
            sg_image_desc sg_query_image_defaults(const sg_image_desc* desc)
            sg_sampler_desc sg_query_sampler_defaults(const sg_sampler_desc*
   desc) sg_shader_desc sg_query_shader_defaults(const sg_shader_desc* desc)
            sg_pipeline_desc sg_query_pipeline_defaults(const sg_pipeline_desc*
   desc) sg_attachments_desc sg_query_attachments_defaults(const
   sg_attachments_desc* desc)
    ...
    --- call the following helper functions to compute the number of
        ...
        These functions might be helpful when preparing image data for
   consumption by sg_make_image() or sg_update_image():

            int sg_query_row_pitch(sg_pixel_format fmt, int width, int int
   row_align_bytes); int sg_query_surface_pitch(sg_pixel_format fmt, int width,
   int height, int row_align_bytes);

        Width and height are generally in number pixels, but note that 'row' has
   different meaning for uncompressed vs compressed pixel formats: for
   uncompressed formats, a row is identical with a single line if pixels, while
   in compressed formats, one row is a line of *compression blocks*.

        This is why calling sg_query_surface_pitch() for a compressed pixel
   format and height N, N+1, N+2, ... may return the same result.

        The row_align_bytes parammeter is for added flexibility. For image data
   that goes into the sg_make_image() or sg_update_image() this should generally
   be 1, because these functions take tightly packed image data as input no
   matter what alignment restrictions exist in the backend 3D APIs.
    ...
    of information to sokol-gfx:
        ...
        (1) upper bounds and limits needed to allocate various internal
            data structures:
                ...
                - the max size of all dynamic resource updates
   (sg_update_buffer, sg_append_buffer and sg_update_image) per frame
        ...
        (2) 3D backend "environment information" in a nested sg_environment
   struct:
            - pointers to backend-specific context- or device-objects (for
   instance the D3D11, WebGPU or Metal device objects)
            - defaults for external swapchain pixel formats and sample counts,
              these will be used as default values in image and pipeline
   objects, and the sg_swapchain struct passed into sg_begin_pass() Usually you
   provide a complete sg_environment struct through a helper function, as an
   example look at the sglue_environment() function in the sokol_glue.h header.

    See the documentation block of the sg_desc struct below for more
   information.
    ...
    passes as textures (it is invalid to use the same image both as render
   target and as texture in the same pass).
    ...
    must be provided:
        ...
        - a number of backend specific objects:
            ...
            - WebGPU
                ...
                - if MSAA is used, a WGPUTextureView object as MSAA resolve
   target
    ...
    As an example for such a helper function check out the function
   sglue_swapchain() in the sokol_glue.h header.
    ...
    target and without multisampling, creating an attachment object looks like
   this:
    ...
    NOTE: when creating render target images, have in mind that some default
   values are aligned with the default environment attributes in the
   sg_environment struct that was passed into the sg_setup() call:
    ...
    The mipmap/slice selection happens at attachments creation time, for
   instance to render into mipmap 2 of slice 3 of an array texture:
    ...
    If an attachments object defines a resolve image in a specific resolve
   attachment slot, an 'msaa resolve operation' will happen in sg_end_pass().

    In this scenario, the content of the MSAA color attachment doesn't need to
   be preserved (since it's only needed inside sg_end_pass for the
   msaa-resolve), so the .store_action should be set to "don't care":
    ...
    ...after sg_end_pass() the only difference to the non-msaa scenario is that
   the rendering result which is going to be used as texture in a followup pass
   is in 'resolve_img', not in 'color_img' (in fact, trying to bind color_img as
   a texture would result in a validation error).
    ...
    ...this means compute shaders can't be used on the following
   platform/backend combos (the same restrictions apply to using storage buffers
   without compute shaders):
    ...
    To create a shader using a C header which has been code-generated by
   sokol-shdc:
        ...
        // create shader using a code-generated helper function from the C
   header: sg_shader shd =
   sg_make_shader(myshader_shader_desc(sg_query_backend()));
    ...
    - Shader code or shader binary blobs for the vertex- and fragment-, or the
      compute-shader-stage:
        - for the desktop GL backend, source code can be provided in '#version
   410' or
        ...
        - for the GLES3 backend, source code must be provided in '#version 300
   es' syntax
        - for the D3D11 backend, shaders can be provided as source or binary
          blobs, the source code should be in HLSL4.0 (for compatibility with
   old low-end GPUs) or preferrably in HLSL5.0 syntax, note that when shader
   source code is provided for the D3D11 backend, sokol-gfx will dynamically
   load 'd3dcompiler_47.dll'
        - for the Metal backends, shaders can be provided as source or binary
   blobs, the MSL version should be in 'metal-1.1' (other versions may work but
   are not tested)
        - for the WebGPU backend, shaders must be provided as WGSL source code
        - optionally the following shader-code related attributes can be
   provided:
            - an entry function name (only on D3D11 or Metal, but not OpenGL)
            - on D3D11 only, a compilation target (default is "vs_4_0" and
   "ps_4_0")
    ...
    - Information about the input vertex attributes used by the vertex shader,
      most of that backend-specific:
        ...
        - Metal: no location information needed since vertex attributes are
   always bound by their attribute location defined in the shader via
   '[[attribute(N)]]'
        - WebGPU: no location information needed since vertex attributes are
   always bound by their attribute location defined in the shader via
   `@location(N)`
        - GLSL: vertex attribute names can be optionally provided, in that case
   their location will be looked up by name, otherwise, the vertex attribute
   location can be defined with 'layout(location = N)'
        - D3D11: a 'semantic name' and 'semantic index' must be provided for
   each vertex attribute, e.g. if the vertex attribute is defined as 'TEXCOORD1'
   in the shader, the semantic name would be 'TEXCOORD', and the semantic index
   would be '1'

      NOTE that vertex attributes currently must not have gaps. This requirement
      may be relaxed in the future.

    - Specifically for Metal compute shaders, the 'number of threads per
   threadgroup' must be provided. Normally this is extracted by sokol-shdc from
   the GLSL shader source code. For instance the following statement in the
   input GLSL:
    ...
    - Information about each uniform block used in the shader:
        ...
        - a memory layout hint (currently 'native' or 'std140') where 'native'
   defines a backend-specific memory layout which shouldn't be used for
   cross-platform code. Only std140 guarantees a backend-agnostic memory layout.
        - a backend-specific bind slot:
            - D3D11/HLSL: the buffer register N (`register(bN)`) where N is 0..7
            - Metal/MSL: the buffer bind slot N (`[[buffer(N)]]`) where N is
   0..7
            - WebGPU: the binding N in `@group(0) @binding(N)` where N is 0..15
        - For GLSL only: a description of the internal uniform block layout,
   which maps member types and their offsets on the CPU side to uniform variable
   names in the GLSL shader
    ...
    - A description of each storage buffer used in the shader:
        ...
        - a backend-specific bind slot:
            - D3D11/HLSL:
                ...
                - for read/write storage buffer buffer bindings: the UAV
   register N
                  (`register(uN)`) where N is 0..7 (in HLSL, readwrite storage
                  buffers use their own bind space for 'unordered access views')
            - Metal/MSL: the buffer bind slot N (`[[buffer(N)]]`) where N
   is 8..15
            - WebGPU/WGSL: the binding N in `@group(0) @binding(N)` where N is
   0..127
            - GL/GLSL: the buffer binding N in `layout(binding=N)` where N is
   0..7
    ...
    - A description of each texture/image used in the shader:
        ...
        - a backend-specific bind slot:
            - D3D11/HLSL: the texture register N (`register(tN)`) where N is
   0..23 (in HLSL, readonly storage buffers and texture share the same bind
   space)
            - Metal/MSL: the texture bind slot N (`[[texture(N)]]`) where N is
   0..15
            - WebGPU/WGSL: the binding N in `@group(0) @binding(N)` where N is
   0..127
    ...
    - A description of each sampler used in the shader:
        ...
        - a backend-specific bind slot:
            - D3D11/HLSL: the sampler register N (`register(sN)`) where N is
   0..15
            - Metal/MSL: the sampler bind slot N (`[[sampler(N)]]`) where N is
   0..15
            - WebGPU/WGSL: the binding N in `@group(0) @binding(N)` where N is
   0..127
    ...
    Compatibility rules for image-sample-type vs sampler-type are as follows:
        ...
        - SG_IMAGESAMPLETYPE_FLOAT => (SG_SAMPLERTYPE_FILTERING or
   SG_SAMPLERTYPE_NONFILTERING)
    ...
    Backend-specific bindslot ranges (not relevant when using sokol-shdc):
        ...
        - WebGPU/WGSL:
            ...
            - textures, samplers and storage buffers: `@group(1)
   @binding(0..127)`
        - GL/GLSL:
    ...
    which can only be combined with 'nonfiltering' samplers (this is a
   restriction specific to WebGPU, but since the same sokol-gfx code should work
   across all backend, the sokol-gfx validation layer also enforces this
   restriction
    ...
    struct when creating the shader (see the above section "ON SHADER
   CREATION").
    ...
    is used, otherwise the default base_type will be
   SG_SHADERATTRBASETYPE_UNDEFINED which causes the sokol-gfx validation check
   to be skipped (of course you can also provide the per-attribute base type
   information manually when not using sokol-shdc).
    ...
    UNIFORM DATA LAYOUT:
    ====================
    NOTE: if you use the sokol-shdc shader compiler tool, you don't need to
   worry about the following details.
    ...
    For the D3D11 and Metal backends, sokol-gfx only cares about the size of
   uniform blocks, but not about the internal layout. The data will just be
   copied into a uniform/constant buffer in a single operation and it's up you
   to arrange the CPU-side layout so that it matches the GPU side layout. This
   also means that with the D3D11 and Metal backends you are not limited to a
   'cross-platform' subset of uniform variable types.

    If you ever only use one of the D3D11, Metal *or* WebGPU backend, you can
   stop reading here.
    ...
    ...the uniform block description in the sg_shader_desc must look like this:
        ...
        sg_shader_desc desc = {
            .vs.uniform_blocks[0] = {
                ...
                .layout = SG_UNIFORMLAYOUT_NATIVE,  // this is the default and
   can be omitted .uniforms = {
    ...
    With this information sokol-gfx can now compute the correct offsets of the
   data items within the uniform block struct.

    The SG_UNIFORMLAYOUT_NATIVE packing rule works fine if only the GL backends
   are used, but for proper D3D11/Metal/GL a subset of the std140 layout must be
   used which is described in the next section:


    CROSS-BACKEND COMMON UNIFORM DATA LAYOUT
    ========================================
    For cross-platform / cross-3D-backend code it is important that the same
   uniform block layout on the CPU side can be used for all sokol-gfx backends.
   To achieve this, a common subset of the std140 layout must be used:

    - The uniform block layout hint in sg_shader_desc must be explicitly set to
      SG_UNIFORMLAYOUT_STD140.
    - Only the following GLSL uniform types can be used (with their associated
   sokol-gfx enums):
    ...
    Storage buffers can be used to pass large amounts of random access
   structured data from the CPU side to the shaders. They are similar to data
   textures, but are more convenient to use both on the CPU and shader side
   since they can be accessed in shaders as as a 1-dimensional array of struct
   items.

    Storage buffers are *NOT* supported on the following platform/backend
   combos:

    - macOS+GL (because storage buffers require GL 4.3, while macOS only goes up
   to GL 4.1)
    - platforms which only support a GLES3.0 context (WebGL2 and iOS)

    To use storage buffers, the following steps are required:

        - write a shader which uses storage buffers (vertex- and
   fragment-shaders can only read from storage buffers, while compute-shaders
   can both read and write storage buffers)
   ...
   sg_shader_desc struct with binding info (when using sokol-shdc, this step
   will be taken care of automatically)
            - which storage buffer bind slots on the vertex-, fragment- or
   compute-stage are occupied
            - whether the storage buffer on that bind slot is readonly (readonly
              bindings are required for vertex- and fragment-shaders, and in
   compute shaders the readonly flag is used to control hazard tracking in some
              3D backends)

        - when calling sg_apply_bindings(), apply the matching bind slots with
   the previously created storage buffers
        - ...and that's it.
    ...
    - simple vertex pulling from a storage buffer:
        - C code:
   https://github.com/floooh/sokol-samples/blob/master/sapp/vertexpull-sapp.c
        - shader:
   https://github.com/floooh/sokol-samples/blob/master/sapp/vertexpull-sapp.glsl
    - instanced rendering via storage buffers (vertex- and instance-pulling):
        - C code:
   https://github.com/floooh/sokol-samples/blob/master/sapp/instancing-pull-sapp.c
        - shader:
   https://github.com/floooh/sokol-samples/blob/master/sapp/instancing-pull-sapp.glsl
    - storage buffers both on the vertex- and fragment-stage:
        - C code:
   https://github.com/floooh/sokol-samples/blob/master/sapp/sbuftex-sapp.c
        - shader:
   https://github.com/floooh/sokol-samples/blob/master/sapp/sbuftex-sapp.glsl
    - the Ozz animation sample rewritten to pull all rendering data from storage
   buffers:
        - C code:
   https://github.com/floooh/sokol-samples/blob/master/sapp/ozz-storagebuffer-sapp.cc
        - shader:
   https://github.com/floooh/sokol-samples/blob/master/sapp/ozz-storagebuffer-sapp.glsl
    - the instancing sample modified to use compute shaders:
        - C code:
   https://github.com/floooh/sokol-samples/blob/master/sapp/instancing-compute-sapp.c
        - shader:
   https://github.com/floooh/sokol-samples/blob/master/sapp/instancing-compute-sapp.glsl
    - the Compute Boids sample ported to sokol-gfx:
        - C code:
   https://github.com/floooh/sokol-samples/blob/master/sapp/computeboids-sapp.c
        - shader:
   https://github.com/floooh/sokol-samples/blob/master/sapp/computeboids-sapp.glsl

    ...also see the following backend-specific vertex pulling samples (those
   also don't use sokol-shdc):

    - D3D11:
   https://github.com/floooh/sokol-samples/blob/master/d3d11/vertexpulling-d3d11.c
    - desktop GL:
   https://github.com/floooh/sokol-samples/blob/master/glfw/vertexpulling-glfw.c
    - Metal:
   https://github.com/floooh/sokol-samples/blob/master/metal/vertexpulling-metal.c
    - WebGPU:
   https://github.com/floooh/sokol-samples/blob/master/wgpu/vertexpulling-wgpu.c

    ...and the backend specific compute shader samples:

    - D3D11:
   https://github.com/floooh/sokol-samples/blob/master/d3d11/instancing-compute-d3d11.c
    - desktop GL:
   https://github.com/floooh/sokol-samples/blob/master/glfw/instancing-compute-glfw.c
    - Metal:
   https://github.com/floooh/sokol-samples/blob/master/metal/instancing-compute-metal.c
    - WebGPU:
   https://github.com/floooh/sokol-samples/blob/master/wgpu/instancing-compute-wgpu.c

    Storage buffer shader authoring caveats when using sokol-shdc:

        - declare a read-only storage buffer interface block with
   `layout(binding=N) readonly buffer [name] { ... }` (where 'N' is the index in
   `sg_bindings.storage_buffers[N]`)
        - ...or a read/write storage buffer interface block with
   `layout(binding=N) buffer [name] { ... }`
        - declare a struct which describes a single array item in the storage
   buffer interface block
        - only put a single flexible array member into the storage buffer
   interface block
    ...
    Backend-specific storage-buffer caveats (not relevant when using
   sokol-shdc):
        ...
        D3D11:
            ...
            - in HLSL, use a ByteAddressBuffer for readonly access of the buffer
   content:
            ...
            - readonly-storage buffers and textures are both bound as
   'shader-resource-view' and share the same bind slots (declared as
   `register(tN)` in HLSL), where N must be in the range 0..23)
            - read/write storage buffers are bound as 'unordered-access-view'
   (declared as `register(uN)` in HLSL where N is in the range 0..7)

        Metal:
            - in Metal there is no internal difference between vertex-, uniform-
   and storage-buffers, all are bound to the same 'buffer bind slots' with the
              following reserved ranges:
                ...
                - fragment shader stage:
                    - uniform buffers: slots 0..7
                    - storage buffers: slots 8..15
            - this means in MSL, storage buffer bindings start at [[buffer(8)]]
   both in the vertex and fragment stage

        GL:
            - the GL backend doesn't use name-lookup to find storage buffer
   bindings, this means you must annotate buffers with `layout(std430,
   binding=N)` in GLSL
            - ...where N is 0..7 in the vertex shader, and 8..15 in the fragment
   shader
   ...
   3=info uint32_t log_item_id,           // SG_LOGITEM_* const char*
   message_or_null,    // a message string, may be nullptr in release mode
                    uint32_t line_nr,               // line number in
   sokol_gfx.h const char* filename_or_null,   // source filename, may be
   ...
   any warnings or errors.
    ...
    COMMIT LISTENERS
    ================
    It's possible to hook callback functions into sokol-gfx which are called
   from inside sg_commit() in unspecified order. This is mainly useful for
   libraries that build on top of sokol_gfx.h to be notified about the end/start
   of a frame.
    ...
    The function returns false if the internal array of commit listeners is
   full, or the same commit listener had already been added.
    ...
    When calling the 'make functions', the created resource goes through a
   number of states:

        - INITIAL: the resource slot associated with the new resource is
   currently free (technically, there is no resource yet, just an empty pool
   slot)
        - ALLOC: a handle for the new resource has been allocated, this just
   means a pool slot has been reserved.
        - VALID or FAILED: in VALID state any 3D API backend resource objects
   have been successfully created, otherwise if anything went wrong, the
   resource will be in FAILED state.
    ...
    Such an 'incomplete' handle can be used in most sokol-gfx rendering
   functions without doing any harm, sokol-gfx will simply skip any rendering
   operation that involve resources which are not in VALID state.
    ...
    'desc struct':
        ...
        void sg_init_attachments(sg_attachments atts, const sg_attachments_desc*
   desc)
    ...
    Calling an 'uninit function' and 'dealloc function' in sequence is
   equivalent with calling the associated 'destroy function':
    ...
    The 'destroy functions' can be called on resources in any state and
   generally do the right thing (for instance if the resource is in ALLOC state,
   the destroy function will be equivalent to the 'dealloc function' and skip
   the 'uninit part').

    And finally to close the circle, the 'fail functions' can be called to
   manually put a resource in ALLOC state into the FAILED state:
    ...
    This is recommended if anything went wrong outside of sokol-gfx during
   asynchronous resource setup (for instance a file loading operation failed).
   In this case, the 'fail function' should be called instead of the 'init
   function'.

    Calling a 'fail function' on a resource that's not in ALLOC state is a
   no-op, but will generate a warning log message.

    NOTE: that two-step resource creation usually only makes sense for buffers
    and images, but not for samplers, shaders, pipelines or attachments. Most
   notably, trying to create a pipeline object with a shader that's not in VALID
   state will trigger a validation layer error, or if the validation layer is
   disabled, result in a pipeline object in FAILED state. Same when trying to
   create an attachments object with invalid image objects.
    ...
    paradoxical situation that some WebGPU code may be slower than similar
   WebGL2 code.
    ...
    - when writing WGSL shader code by hand, a specific bind-slot convention
      ...
      If you use sokol-shdc to generate WGSL shader code, you don't need to
   worry about the above binding conventions since sokol-shdc.
    ...
    - The sokol-gfx WebGPU backend uses the sg_desc.uniform_buffer_size item
      ...
      call will cost at least 256 bytes of uniform buffer size). The default
   size is 4 MB, which is enough for 16384 sg_apply_uniform() calls per frame
   (assuming the uniform data 'payload' is less than 256 bytes per call). These
   rules are the same as for the Metal backend, so if you are already using the
   Metal backend you'll be fine.
    ...
    - sg_apply_bindings(): the sokol-gfx WebGPU backend implements a bindgroup
      ...
      sg_setup. The cache size must be a power-of-2 number, with the default
   being 1024. The bindgroups cache behaviour can be observed by calling the new
    ...
    - sg_apply_viewport(): WebGPU currently has a unique restriction that
   viewport rectangles must be contained entirely within the framebuffer. As a
   shitty workaround sokol_gfx.h will clip incoming viewport rectangles against
      the framebuffer, but this will distort the clipspace-to-screenspace
   mapping. There's no proper way to handle this inside sokol_gfx.h, this must
   be fixed in a future WebGPU update (see:
   https://github.com/gpuweb/gpuweb/issues/373 and
   https://github.com/gpuweb/gpuweb/pull/5025)

    - The sokol shader compiler generally adds `diagnostic(off,
   derivative_uniformity);` into the WGSL output. Currently only the Chrome
   WebGPU implementation seems to recognize this.

    - Likewise, the following sokol-gfx pixel formats are not supported in
   WebGPU: R16, R16SN, RG16, RG16SN, RGBA16, RGBA16SN. Unlike unsupported vertex
   formats, unsupported pixel formats can be queried in cross-backend code via
   sg_query_pixel_format() though.

    - The Emscripten WebGPU shim currently doesn't support the Closure
   minification post-link-step (e.g. currently the emcc argument '--closure 1'
   or '--closure 2' will generate broken Javascript code.

    - sokol-gfx requires the WebGPU device feature `depth32float-stencil8` to be
   enabled (this should be widely supported)

    - sokol-gfx expects that the WebGPU device feature `float32-filterable` to
   *not* be enabled (since this would exclude all iOS devices)
...
/*
   ...
   information sg_pipeline:    associated shader and vertex-layouts, and render
   states sg_attachments: a baked collection of render pass attachment images
...
/*
    ...
    Not all pixel formats can be used for everything, call
   sg_query_pixelformat() to inspect the capabilities of a given pixelformat.
   The function returns an sg_pixelformat_info struct with the following
   members:
        ...
        - bytes_per_pixel: the numbers of bytes in a pixel (0 for compressed
   formats)
...
typedef enum sg_pixel_format {
  ...
  // NOTE: when adding/removing pixel formats before DEPTH, also update
  // sokol_app.h/_SAPP_PIXELFORMAT_*
...
/*
    Runtime information about a pixel format, returned by
   sg_query_pixelformat().
*/
typedef struct sg_pixelformat_info {
  bool sample;  // pixel format can be sampled in shaders at least with nearest
                // filtering
  ...
  bool blend;   // pixel format supports alpha-blending when used as render-pass
                // attachment
  ...
  int bytes_per_pixel;  // NOTE: this is 0 for compressed formats, use
                        // sg_query_row_pitch() / sg_query_surface_pitch() as
                        // alternative
} sg_pixelformat_info;
...
typedef struct sg_features {
  bool
      origin_top_left;  // framebuffer- and texture-origin is in top left corner
  bool image_clamp_to_border;  // border color and clamp-to-border uv-wrap mode
                               // is supported
  bool mrt_independent_blend_state;  // multiple-render-target rendering can use
                                     // per-render-target blend state
  bool mrt_independent_write_mask;   // multiple-render-target rendering can use
                                     // per-render-target color write masks
  bool compute;  // storage buffers and compute shaders are supported
  bool msaa_image_bindings;  // if true, multisampled images can be bound as
                             // texture resources
} sg_features;
...
typedef struct sg_limits {
  ...
  int max_image_array_layers;  // max number of layers in SG_IMAGETYPE_ARRAY
                               // images
  int max_vertex_attrs;        // max number of vertex attributes, clamped to
...
/*
    ...
    (other 3D APIs treat texture/sampler type mismatches as undefined
   behaviour).
...
/*
    ...
    Platforms which don't support SG_WRAP_CLAMP_TO_BORDER will silently fall
   back to SG_WRAP_CLAMP_TO_EDGE without a validation error.
*/
...
typedef enum sg_uniform_layout {
  ...
  SG_UNIFORMLAYOUT_NATIVE,    // default: layout depends on currently active
                              // backend
...
/*
   ...
   pass:
    ...
    SG_STOREACTION_STORE:       store the rendered content to the color
   attachment image SG_STOREACTION_DONTCARE:    allows the GPU to discard the
   rendered content
*/
...
/*
   ...
   cleared, loaded with their previous content, or start in an undefined state
...
/*
   ...
   objects).
    ...
    than on D3D11 or WebGPU in case of MSAA vs non-MSAA rendering):
        ...
        - A current CAMetalDrawable (NOT an MTLDrawable!) which will be
   presented. This will either be rendered to directly (if no MSAA is used), or
   serve as MSAA-resolve target.
   ...
   currentDrawable];
    ...
    sg_pass.swapchain. Look at the function sglue_swapchain() in the
   sokol_glue.h as an example.
*/
...
/*
    ...
    .num_mipmaps:       1
    .usage:             SG_USAGE_IMMUTABLE
    .pixel_format:      SG_PIXELFORMAT_RGBA8 for textures, or
   sg_desc.environment.defaults.color_format for render targets .sample_count:
   1 for textures, or sg_desc.environment.defaults.sample_count for render
   targets .data               an sg_image_data struct to define the initial
   content .label              0 (optional string label for trace hooks)
    ...
    though it is a bit strange/confusing that offscreen render targets by
   default get the same sample count as 'default swapchains', but it's better
   that an offscreen render target created with default parameters matches a
   pipeline object created with default parameters.
...
/*
   ...
   int), this information is only used in the validation layer to check that the
          pipeline object vertex formats are compatible with the input vertex
   attribute type used in the vertex shader. NOTE that the default base type
          'undefined' skips the validation layer check.
        - for the GL backend: optional vertex attribute names used for name
   lookup
...
typedef struct sg_shader_vertex_attr {
  sg_shader_attr_base_type
      base_type;              // default: UNDEFINED (disables validation)
...
typedef struct sg_shader_storage_buffer {
  ...
  bool readonly;
  uint8_t
      hlsl_register_t_n;  // HLSL register(tn) bind slot (for readonly access)
  uint8_t
      hlsl_register_u_n;  // HLSL register(un) bind slot (for read/write access)
...
typedef struct sg_shader_image_sampler_pair {
  ...
  const char*
      glsl_name;  // glsl name binding required because of GL 4.1 and WebGL2
...
/*
   ...
   no gaps) .format         SG_VERTEXFORMAT_INVALID (must be initialized!)
    .depth:
...
/*
    ...
    Each attachment definition consists of an image object, and two additional
   indices describing which subimage the pass will render into: one mipmap
   index, and if the image is a cubemap, array-texture or 3D-texture, the
   face-index, array-layer or depth-slice.
    ...
    associated color attachment image into the resolve attachment image will
   take place in the sg_end_pass() function. In this case, the color attachment
...
typedef struct sg_trace_hooks {
  ...
  void (*make_buffer)(const sg_buffer_desc* desc,
                      sg_buffer result,
                      void* user_data);
  void (*make_image)(const sg_image_desc* desc,
                     sg_image result,
                     void* user_data);
  void (*make_sampler)(const sg_sampler_desc* desc,
                       sg_sampler result,
                       void* user_data);
  void (*make_shader)(const sg_shader_desc* desc,
                      sg_shader result,
                      void* user_data);
  void (*make_pipeline)(const sg_pipeline_desc* desc,
                        sg_pipeline result,
                        void* user_data);
  void (*make_attachments)(const sg_attachments_desc* desc,
  ...
  void (*update_image)(sg_image img,
                       const sg_image_data* data,
                       void* user_data);
  void (*append_buffer)(sg_buffer buf,
                        const sg_range* data,
                        int result,
                        void* user_data);
  void (*begin_pass)(const sg_pass* pass, void* user_data);
  void (*apply_viewport)(int x,
                         int y,
                         int width,
                         int height,
                         bool origin_top_left,
                         void* user_data);
  void (*apply_scissor_rect)(int x,
                             int y,
                             int width,
                             int height,
                             bool origin_top_left,
  ...
  void (*draw)(int base_element,
               int num_elements,
               int num_instances,
               void* user_data);
  void (*dispatch)(int num_groups_x,
                   int num_groups_y,
                   int num_groups_z,
                   void* user_data);
  ...
  void (*init_buffer)(sg_buffer buf_id,
                      const sg_buffer_desc* desc,
                      void* user_data);
  void (*init_image)(sg_image img_id,
                     const sg_image_desc* desc,
                     void* user_data);
  void (*init_sampler)(sg_sampler smp_id,
                       const sg_sampler_desc* desc,
                       void* user_data);
  void (*init_shader)(sg_shader shd_id,
                      const sg_shader_desc* desc,
                      void* user_data);
  void (*init_pipeline)(sg_pipeline pip_id,
                        const sg_pipeline_desc* desc,
                        void* user_data);
  void (*init_attachments)(sg_attachments atts_id,
...
typedef struct sg_buffer_info {
  ...
  bool
      append_overflow;  // is buffer in overflow state (due to sg_append_buffer)
  int num_slots;    // number of renaming-slots for dynamically updated buffers
  int active_slot;  // currently active write-slot for dynamically updated
                    // buffers
} sg_buffer_info;
...
#define _SG_LOG_ITEMS                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "GLSL storage buffer bindslot is out of range (must be 0..7) (gl)")      \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "vertex attribute not found in shader; NOTE: may be caused by GL "       \
      "driver's GLSL compiler removing unused globals")                        \
  _SG_LOGITEM_XMACRO(                                                          \
      GL_UNIFORMBLOCK_NAME_NOT_FOUND_IN_SHADER,                                \
      "uniform block name not found in shader; NOTE: may be caused by GL "     \
      "driver's GLSL compiler removing unused globals")                        \
  _SG_LOGITEM_XMACRO(                                                          \
      GL_IMAGE_SAMPLER_NAME_NOT_FOUND_IN_SHADER,                               \
      "image-sampler name not found in shader; NOTE: may be caused by GL "     \
      "driver's GLSL compiler removing unused globals")                        \
  _SG_LOGITEM_XMACRO(GL_FRAMEBUFFER_STATUS_UNDEFINED,                          \
                     "framebuffer completeness check failed with "             \
                     "GL_FRAMEBUFFER_UNDEFINED (gl)")                          \
  _SG_LOGITEM_XMACRO(GL_FRAMEBUFFER_STATUS_INCOMPLETE_ATTACHMENT,              \
                     "framebuffer completeness check failed with "             \
                     "GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT (gl)")              \
  _SG_LOGITEM_XMACRO(GL_FRAMEBUFFER_STATUS_INCOMPLETE_MISSING_ATTACHMENT,      \
                     "framebuffer completeness check failed with "             \
                     "GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT (gl)")      \
  _SG_LOGITEM_XMACRO(GL_FRAMEBUFFER_STATUS_UNSUPPORTED,                        \
                     "framebuffer completeness check failed with "             \
                     "GL_FRAMEBUFFER_UNSUPPORTED (gl)")                        \
  _SG_LOGITEM_XMACRO(GL_FRAMEBUFFER_STATUS_INCOMPLETE_MULTISAMPLE,             \
                     "framebuffer completeness check failed with "             \
                     "GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE (gl)")             \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(D3D11_CREATE_2D_SRV_FAILED,                               \
                     "CreateShaderResourceView() failed for 2d-, cube- or "    \
                     "array-texture (d3d11)")                                  \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "uniform block 'hlsl_register_b_n' is out of range (must be 0..7)")      \
  _SG_LOGITEM_XMACRO(                                                          \
      D3D11_STORAGEBUFFER_HLSL_REGISTER_T_OUT_OF_RANGE,                        \
      "storage buffer 'hlsl_register_t_n' is out of range (must be 0..23)")    \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "image 'hlsl_register_t_n' is out of range (must be 0..23)")             \
  _SG_LOGITEM_XMACRO(                                                          \
      D3D11_SAMPLER_HLSL_REGISTER_S_OUT_OF_RANGE,                              \
      "sampler 'hlsl_register_s_n' is out of rang (must be 0..15)")            \
  ...
  _SG_LOGITEM_XMACRO(METAL_CREATE_BUFFER_FAILED,                               \
                     "failed to create buffer object (metal)")                 \
  _SG_LOGITEM_XMACRO(METAL_TEXTURE_FORMAT_NOT_SUPPORTED,                       \
                     "pixel format not supported for texture (metal)")         \
  _SG_LOGITEM_XMACRO(METAL_CREATE_TEXTURE_FAILED,                              \
                     "failed to create texture object (metal)")                \
  _SG_LOGITEM_XMACRO(METAL_CREATE_SAMPLER_FAILED,                              \
                     "failed to create sampler object (metal)")                \
  _SG_LOGITEM_XMACRO(METAL_SHADER_COMPILATION_FAILED,                          \
                     "shader compilation failed (metal)")                      \
  _SG_LOGITEM_XMACRO(METAL_SHADER_CREATION_FAILED,                             \
                     "shader creation failed (metal)")                         \
  _SG_LOGITEM_XMACRO(METAL_SHADER_COMPILATION_OUTPUT, "")                      \
  _SG_LOGITEM_XMACRO(METAL_SHADER_ENTRY_NOT_FOUND,                             \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "storage buffer 'msl_buffer_n' is out of range (must be 8..15)")         \
  _SG_LOGITEM_XMACRO(METAL_IMAGE_MSL_TEXTURE_SLOT_OUT_OF_RANGE,                \
                     "image 'msl_texture_n' is out of range (must be 0..15)")  \
  _SG_LOGITEM_XMACRO(                                                          \
      METAL_SAMPLER_MSL_SAMPLER_SLOT_OUT_OF_RANGE,                             \
      "sampler 'msl_sampler_n' is out of range (must be 0..15)")               \
  _SG_LOGITEM_XMACRO(METAL_CREATE_CPS_FAILED,                                  \
                     "failed to create compute pipeline state (metal)")        \
  _SG_LOGITEM_XMACRO(METAL_CREATE_CPS_OUTPUT, "")                              \
  _SG_LOGITEM_XMACRO(METAL_CREATE_RPS_FAILED,                                  \
                     "failed to create render pipeline state (metal)")         \
  _SG_LOGITEM_XMACRO(METAL_CREATE_RPS_OUTPUT, "")                              \
  _SG_LOGITEM_XMACRO(METAL_CREATE_DSS_FAILED,                                  \
                     "failed to create depth stencil state (metal)")           \
  _SG_LOGITEM_XMACRO(WGPU_BINDGROUPS_POOL_EXHAUSTED,                           \
                     "bindgroups pool exhausted (increase "                    \
                     "sg_desc.bindgroups_cache_size) (wgpu)")                  \
  _SG_LOGITEM_XMACRO(WGPU_BINDGROUPSCACHE_SIZE_GREATER_ONE,                    \
                     "sg_desc.wgpu_bindgroups_cache_size must be > 1 (wgpu)")  \
  _SG_LOGITEM_XMACRO(                                                          \
      WGPU_BINDGROUPSCACHE_SIZE_POW2,                                          \
      "sg_desc.wgpu_bindgroups_cache_size must be a power of 2 (wgpu)")        \
  _SG_LOGITEM_XMACRO(WGPU_CREATEBINDGROUP_FAILED,                              \
                     "wgpuDeviceCreateBindGroup failed")                       \
  _SG_LOGITEM_XMACRO(WGPU_CREATE_BUFFER_FAILED,                                \
                     "wgpuDeviceCreateBuffer() failed")                        \
  _SG_LOGITEM_XMACRO(WGPU_CREATE_TEXTURE_FAILED,                               \
                     "wgpuDeviceCreateTexture() failed")                       \
  _SG_LOGITEM_XMACRO(WGPU_CREATE_TEXTURE_VIEW_FAILED,                          \
                     "wgpuTextureCreateView() failed")                         \
  _SG_LOGITEM_XMACRO(WGPU_CREATE_SAMPLER_FAILED,                               \
                     "wgpuDeviceCreateSampler() failed")                       \
  _SG_LOGITEM_XMACRO(WGPU_CREATE_SHADER_MODULE_FAILED,                         \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "uniform block 'wgsl_group0_binding_n' is out of range (must be 0..15)") \
  _SG_LOGITEM_XMACRO(WGPU_STORAGEBUFFER_WGSL_GROUP1_BINDING_OUT_OF_RANGE,      \
                     "storage buffer 'wgsl_group1_binding_n' is out of range " \
                     "(must be 0..127)")                                       \
  _SG_LOGITEM_XMACRO(                                                          \
      WGPU_IMAGE_WGSL_GROUP1_BINDING_OUT_OF_RANGE,                             \
      "image 'wgsl_group1_binding_n' is out of range (must be 0..127)")        \
  _SG_LOGITEM_XMACRO(                                                          \
      WGPU_SAMPLER_WGSL_GROUP1_BINDING_OUT_OF_RANGE,                           \
      "sampler 'wgsl_group1_binding_n' is out of range (must be 0..127)")      \
  _SG_LOGITEM_XMACRO(WGPU_CREATE_PIPELINE_LAYOUT_FAILED,                       \
                     "wgpuDeviceCreatePipelineLayout() failed")                \
  _SG_LOGITEM_XMACRO(WGPU_CREATE_RENDER_PIPELINE_FAILED,                       \
                     "wgpuDeviceCreateRenderPipeline() failed")                \
  _SG_LOGITEM_XMACRO(WGPU_CREATE_COMPUTE_PIPELINE_FAILED,                      \
                     "wgpuDeviceCreateComputePipeline() failed")               \
  _SG_LOGITEM_XMACRO(WGPU_ATTACHMENTS_CREATE_TEXTURE_VIEW_FAILED,              \
                     "wgpuTextureCreateView() failed in create attachments")   \
  _SG_LOGITEM_XMACRO(IDENTICAL_COMMIT_LISTENER,                                \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "sg_install_trace_hooks() called, but SOKOL_TRACE_HOOKS is not defined") \
  _SG_LOGITEM_XMACRO(DEALLOC_BUFFER_INVALID_STATE,                             \
                     "sg_dealloc_buffer(): buffer must be in ALLOC state")     \
  _SG_LOGITEM_XMACRO(DEALLOC_IMAGE_INVALID_STATE,                              \
                     "sg_dealloc_image(): image must be in alloc state")       \
  _SG_LOGITEM_XMACRO(DEALLOC_SAMPLER_INVALID_STATE,                            \
                     "sg_dealloc_sampler(): sampler must be in alloc state")   \
  _SG_LOGITEM_XMACRO(DEALLOC_SHADER_INVALID_STATE,                             \
                     "sg_dealloc_shader(): shader must be in ALLOC state")     \
  _SG_LOGITEM_XMACRO(DEALLOC_PIPELINE_INVALID_STATE,                           \
                     "sg_dealloc_pipeline(): pipeline must be in ALLOC state") \
  _SG_LOGITEM_XMACRO(                                                          \
      DEALLOC_ATTACHMENTS_INVALID_STATE,                                       \
      "sg_dealloc_attachments(): attachments must be in ALLOC state")          \
  _SG_LOGITEM_XMACRO(INIT_BUFFER_INVALID_STATE,                                \
                     "sg_init_buffer(): buffer must be in ALLOC state")        \
  _SG_LOGITEM_XMACRO(INIT_IMAGE_INVALID_STATE,                                 \
                     "sg_init_image(): image must be in ALLOC state")          \
  _SG_LOGITEM_XMACRO(INIT_SAMPLER_INVALID_STATE,                               \
                     "sg_init_sampler(): sampler must be in ALLOC state")      \
  _SG_LOGITEM_XMACRO(INIT_SHADER_INVALID_STATE,                                \
                     "sg_init_shader(): shader must be in ALLOC state")        \
  _SG_LOGITEM_XMACRO(INIT_PIPELINE_INVALID_STATE,                              \
                     "sg_init_pipeline(): pipeline must be in ALLOC state")    \
  _SG_LOGITEM_XMACRO(INIT_ATTACHMENTS_INVALID_STATE,                           \
                     "sg_init_attachments(): pass must be in ALLOC state")     \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "sg_uninit_sampler(): sampler must be in VALID or FAILED state")         \
  _SG_LOGITEM_XMACRO(                                                          \
      UNINIT_SHADER_INVALID_STATE,                                             \
      "sg_uninit_shader(): shader must be in VALID or FAILED state")           \
  _SG_LOGITEM_XMACRO(                                                          \
      UNINIT_PIPELINE_INVALID_STATE,                                           \
      "sg_uninit_pipeline(): pipeline must be in VALID or FAILED state")       \
  _SG_LOGITEM_XMACRO(                                                          \
      UNINIT_ATTACHMENTS_INVALID_STATE,                                        \
      "sg_uninit_attachments(): attachments must be in VALID or FAILED state") \
  _SG_LOGITEM_XMACRO(FAIL_BUFFER_INVALID_STATE,                                \
                     "sg_fail_buffer(): buffer must be in ALLOC state")        \
  _SG_LOGITEM_XMACRO(FAIL_IMAGE_INVALID_STATE,                                 \
                     "sg_fail_image(): image must be in ALLOC state")          \
  _SG_LOGITEM_XMACRO(FAIL_SAMPLER_INVALID_STATE,                               \
                     "sg_fail_sampler(): sampler must be in ALLOC state")      \
  _SG_LOGITEM_XMACRO(FAIL_SHADER_INVALID_STATE,                                \
                     "sg_fail_shader(): shader must be in ALLOC state")        \
  _SG_LOGITEM_XMACRO(FAIL_PIPELINE_INVALID_STATE,                              \
                     "sg_fail_pipeline(): pipeline must be in ALLOC state")    \
  _SG_LOGITEM_XMACRO(                                                          \
      FAIL_ATTACHMENTS_INVALID_STATE,                                          \
      "sg_fail_attachments(): attachments must be in ALLOC state")             \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "sg_begin_pass: an attachment was provided that no longer exists")       \
  _SG_LOGITEM_XMACRO(APPLY_BINDINGS_STORAGE_BUFFER_TRACKER_EXHAUSTED,          \
                     "sg_apply_bindings: too many read/write storage buffers " \
                     "in pass (bump sg_desc.max_dispatch_calls_per_pass")      \
  _SG_LOGITEM_XMACRO(DRAW_WITHOUT_BINDINGS,                                    \
                     "attempting to draw without resource bindings")           \
  _SG_LOGITEM_XMACRO(VALIDATE_BUFFERDESC_CANARY,                               \
                     "sg_buffer_desc not initialized")                         \
  _SG_LOGITEM_XMACRO(VALIDATE_BUFFERDESC_EXPECT_NONZERO_SIZE,                  \
                     "sg_buffer_desc.size must be greater zero")               \
  _SG_LOGITEM_XMACRO(VALIDATE_BUFFERDESC_EXPECT_MATCHING_DATA_SIZE,            \
                     "sg_buffer_desc.size and .data.size must be equal")       \
  _SG_LOGITEM_XMACRO(VALIDATE_BUFFERDESC_EXPECT_ZERO_DATA_SIZE,                \
                     "sg_buffer_desc.data.size expected to be zero")           \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BUFFERDESC_EXPECT_NO_DATA,                                      \
      "sg_buffer_desc.data.ptr must be null for dynamic/stream buffers")       \
  _SG_LOGITEM_XMACRO(VALIDATE_BUFFERDESC_STORAGEBUFFER_SUPPORTED,              \
                     "storage buffers not supported by the backend 3D API "    \
                     "(requires OpenGL >= 4.3)")                               \
  _SG_LOGITEM_XMACRO(VALIDATE_BUFFERDESC_STORAGEBUFFER_SIZE_MULTIPLE_4,        \
                     "size of storage buffers must be a multiple of 4")        \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDATA_NODATA,                                \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_CANARY,                                \
                     "sg_image_desc not initialized")                          \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_WIDTH,                                 \
                     "sg_image_desc.width must be > 0")                        \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_HEIGHT,                                \
                     "sg_image_desc.height must be > 0")                       \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_RT_PIXELFORMAT,                        \
                     "invalid pixel format for render-target image")           \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_NONRT_PIXELFORMAT,                     \
                     "invalid pixel format for non-render-target image")       \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_MSAA_BUT_NO_RT,                        \
                     "non-render-target images cannot be multisampled")        \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_NO_MSAA_RT_SUPPORT,                    \
                     "MSAA not supported for this pixel format")               \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_MSAA_NUM_MIPMAPS,                      \
                     "MSAA images must have num_mipmaps == 1")                 \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_MSAA_3D_IMAGE,                         \
                     "3D images cannot have a sample_count > 1")               \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_MSAA_CUBE_IMAGE,                       \
                     "cube images cannot have sample_count > 1")               \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_DEPTH_3D_IMAGE,                        \
                     "3D images cannot have a depth/stencil image format")     \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_RT_IMMUTABLE,                          \
                     "render target images must be SG_USAGE_IMMUTABLE")        \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_RT_NO_DATA,                            \
                     "render target images cannot be initialized with data")   \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_IMAGEDESC_INJECTED_NO_DATA,                                     \
      "images with injected textures cannot be initialized with data")         \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_DYNAMIC_NO_DATA,                       \
                     "dynamic/stream images cannot be initialized with data")  \
  _SG_LOGITEM_XMACRO(VALIDATE_IMAGEDESC_COMPRESSED_IMMUTABLE,                  \
                     "compressed images must be immutable")                    \
  _SG_LOGITEM_XMACRO(VALIDATE_SAMPLERDESC_CANARY,                              \
                     "sg_sampler_desc not initialized")                        \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SAMPLERDESC_ANISTROPIC_REQUIRES_LINEAR_FILTERING,               \
      "sg_sampler_desc.max_anisotropy > 1 requires min/mag/mipmap_filter to "  \
      "be SG_FILTER_LINEAR")                                                   \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_CANARY,                               \
                     "sg_shader_desc not initialized")                         \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_VERTEX_SOURCE,                        \
                     "vertex shader source code expected")                     \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_FRAGMENT_SOURCE,                      \
                     "fragment shader source code expected")                   \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_COMPUTE_SOURCE,                       \
                     "compute shader source code expected")                    \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_VERTEX_SOURCE_OR_BYTECODE,            \
                     "vertex shader source or byte code expected")             \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_FRAGMENT_SOURCE_OR_BYTECODE,          \
                     "fragment shader source or byte code expected")           \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_COMPUTE_SOURCE_OR_BYTECODE,           \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_BYTECODE_SIZE,                     \
                     "shader byte code length (in bytes) required")            \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_METAL_THREADS_PER_THREADGROUP,        \
                     "sg_shader_desc.mtl_threads_per_threadgroup must be "     \
                     "initialized for compute shaders (metal)")                \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_UNIFORMBLOCK_NO_CONT_MEMBERS,         \
                     "uniform block members must occupy continuous slots")     \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_UNIFORMBLOCK_SIZE_IS_ZERO,            \
                     "bound uniform block size cannot be zero")                \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "uniform block 'msl_buffer_n' must be unique across uniform blocks and " \
      "storage buffers in same shader stage")                                  \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_UNIFORMBLOCK_HLSL_REGISTER_B_OUT_OF_RANGE,           \
      "uniform block 'hlsl_register_b_n' is out of range (must be 0..7)")      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_UNIFORMBLOCK_HLSL_REGISTER_B_COLLISION,              \
      "uniform block 'hlsl_register_b_n' must be unique across uniform "       \
      "blocks in same shader stage")                                           \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "uniform block 'wgsl_group0_binding_n' must be unique across all "       \
      "uniform blocks")                                                        \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_UNIFORMBLOCK_NO_MEMBERS,              \
                     "GL backend requires uniform block member declarations")  \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_UNIFORMBLOCK_UNIFORM_GLSL_NAME,       \
                     "uniform block member 'glsl_name' missing")               \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_UNIFORMBLOCK_SIZE_MISMATCH,                          \
      "size of uniform block members doesn't match uniform block size")        \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_UNIFORMBLOCK_ARRAY_COUNT,             \
                     "uniform array count must be >= 1")                       \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_UNIFORMBLOCK_STD140_ARRAY_TYPE,                      \
      "uniform arrays only allowed for FLOAT4, INT4, MAT4 in std140 layout")   \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "storage buffer 'msl_buffer_n' must be unique across uniform blocks "    \
      "and storage buffer in same shader stage")                               \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "storage_buffer 'hlsl_register_t_n' must be unique across read-only "    \
      "storage buffers and images in same shader stage")                       \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "storage_buffer 'hlsl_register_u_n' must be unique across read/write "   \
      "storage buffers in same shader stage")                                  \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_STORAGEBUFFER_GLSL_BINDING_OUT_OF_RANGE,             \
      "storage buffer 'glsl_binding_n' is out of range (must be 0..7)")        \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_STORAGEBUFFER_GLSL_BINDING_COLLISION,                \
      "storage buffer 'glsl_binding_n' must be unique across shader stages")   \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_STORAGEBUFFER_WGSL_GROUP1_BINDING_OUT_OF_RANGE,      \
      "storage buffer 'wgsl_group1_binding_n' is out of range (must be "       \
      "0..127)")                                                               \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_STORAGEBUFFER_WGSL_GROUP1_BINDING_COLLISION,         \
      "storage buffer 'wgsl_group1_binding_n' must be unique across all "      \
      "images, samplers and storage buffers")                                  \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_IMAGE_METAL_TEXTURE_SLOT_OUT_OF_RANGE,               \
      "image 'msl_texture_n' is out of range (must be 0..15)")                 \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_IMAGE_METAL_TEXTURE_SLOT_COLLISION,                  \
      "image 'msl_texture_n' must be unique in same shader stage")             \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_IMAGE_HLSL_REGISTER_T_OUT_OF_RANGE,                  \
      "image 'hlsl_register_t_n' is out of range (must be 0..23)")             \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_HLSL_REGISTER_T_COLLISION,      \
                     "image 'hlsl_register_t_n' must be unique across images " \
                     "and storage buffers in same shader stage")               \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_IMAGE_WGSL_GROUP1_BINDING_OUT_OF_RANGE,              \
      "image 'wgsl_group1_binding_n' is out of range (must be 0..127)")        \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_WGSL_GROUP1_BINDING_COLLISION,  \
                     "image 'wgsl_group1_binding_n' must be unique across "    \
                     "all images, samplers and storage buffers")               \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_SAMPLER_METAL_SAMPLER_SLOT_OUT_OF_RANGE,             \
      "sampler 'msl_sampler_n' is out of range (must be 0..15)")               \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_SAMPLER_METAL_SAMPLER_SLOT_COLLISION,                \
      "sampler 'msl_sampler_n' must be unique in same shader stage")           \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_SAMPLER_HLSL_REGISTER_S_OUT_OF_RANGE,                \
      "sampler 'hlsl_register_s_n' is out of rang (must be 0..15)")            \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_SAMPLER_HLSL_REGISTER_S_COLLISION,                   \
      "sampler 'hlsl_register_s_n' must be unique in same shader stage")       \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_SAMPLER_WGSL_GROUP1_BINDING_OUT_OF_RANGE,            \
      "sampler 'wgsl_group1_binding_n' is out of range (must be 0..127)")      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_SAMPLER_WGSL_GROUP1_BINDING_COLLISION,               \
      "sampler 'wgsl_group1_binding_n' must be unique across all images, "     \
      "samplers and storage buffers")                                          \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_IMAGE_SLOT_OUT_OF_RANGE,          \
      "image-sampler-pair image slot index is out of range "                   \
      "(sg_shader_desc.image_sampler_pairs[].image_slot)")                     \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_SAMPLER_SLOT_OUT_OF_RANGE,        \
      "image-sampler-pair sampler slot index is out of range "                 \
      "(sg_shader_desc.image_sampler_pairs[].sampler_slot)")                   \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_IMAGE_STAGE_MISMATCH,             \
      "image-sampler-pair stage doesn't match referenced image stage")         \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_SAMPLER_STAGE_MISMATCH,           \
      "image-sampler-pair stage doesn't match referenced sampler stage")       \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_GLSL_NAME,         \
                     "image-sampler-pair 'glsl_name' missing")                 \
  _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NONFILTERING_SAMPLER_REQUIRED,        \
                     "image sample type UNFILTERABLE_FLOAT, UINT, SINT can "   \
                     "only be used with NONFILTERING sampler")                 \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "one or more images are not referenced by by image-sampler-pairs "       \
      "(sg_shader_desc.image_sampler_pairs[].image_slot)")                     \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_SAMPLER_NOT_REFERENCED_BY_IMAGE_SAMPLER_PAIRS,       \
      "one or more samplers are not referenced by image-sampler-pairs "        \
      "(sg_shader_desc.image_sampler_pairs[].sampler_slot)")                   \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_SHADERDESC_ATTR_STRING_TOO_LONG,                                \
      "vertex attribute name/semantic string too long (max len 16)")           \
  _SG_LOGITEM_XMACRO(VALIDATE_PIPELINEDESC_CANARY,                             \
                     "sg_pipeline_desc not initialized")                       \
  _SG_LOGITEM_XMACRO(VALIDATE_PIPELINEDESC_SHADER,                             \
                     "sg_pipeline_desc.shader missing or invalid")             \
  _SG_LOGITEM_XMACRO(VALIDATE_PIPELINEDESC_COMPUTE_SHADER_EXPECTED,            \
                     "sg_pipeline_desc.shader must be a compute shader")       \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_PIPELINEDESC_NO_COMPUTE_SHADER_EXPECTED,                        \
      "sg_pipeline_desc.compute is false, but shader is a compute shader")     \
  _SG_LOGITEM_XMACRO(VALIDATE_PIPELINEDESC_NO_CONT_ATTRS,                      \
                     "sg_pipeline_desc.layout.attrs is not continuous")        \
  _SG_LOGITEM_XMACRO(VALIDATE_PIPELINEDESC_ATTR_BASETYPE_MISMATCH,             \
                     "sg_pipeline_desc.layout.attrs[].format is incompatble "  \
                     "with sg_shader_desc.attrs[].base_type")                  \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_PIPELINEDESC_LAYOUT_STRIDE4,                                    \
      "sg_pipeline_desc.layout.buffers[].stride must be multiple of 4")        \
  _SG_LOGITEM_XMACRO(VALIDATE_PIPELINEDESC_ATTR_SEMANTICS,                     \
                     "D3D11 missing vertex attribute semantics in shader")     \
  _SG_LOGITEM_XMACRO(VALIDATE_PIPELINEDESC_SHADER_READONLY_STORAGEBUFFERS,     \
                     "sg_pipeline_desc.shader: only readonly storage buffer "  \
                     "bindings allowed in render pipelines")                   \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_PIPELINEDESC_BLENDOP_MINMAX_REQUIRES_BLENDFACTOR_ONE,           \
      "SG_BLENDOP_MIN/MAX requires all blend factors to be "                   \
      "SG_BLENDFACTOR_ONE")                                                    \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_CANARY,                          \
                     "sg_attachments_desc not initialized")                    \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_NO_ATTACHMENTS,                                 \
      "sg_attachments_desc no color or depth-stencil attachments")             \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_NO_CONT_COLOR_ATTS,              \
                     "color attachments must occupy continuous slots")         \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_IMAGE,                           \
                     "pass attachment image is not valid")                     \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_MIPLEVEL,                                       \
      "pass attachment mip level is bigger than image has mipmaps")            \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_FACE,                                           \
      "pass attachment image is cubemap, but face index is too big")           \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_LAYER,                                          \
      "pass attachment image is array texture, but layer index is too big")    \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_SLICE,                                          \
      "pass attachment image is 3d texture, but slice value is too big")       \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_IMAGE_NO_RT,                     \
                     "pass attachment image must be have render_target=true")  \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_COLOR_INV_PIXELFORMAT,                          \
      "pass color-attachment images must be renderable color pixel format")    \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_DEPTH_INV_PIXELFORMAT,           \
                     "pass depth-attachment image must be depth or "           \
                     "depth-stencil pixel format")                             \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_IMAGE_SIZES,                     \
                     "all pass attachments must have the same size")           \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_IMAGE_SAMPLE_COUNTS,             \
                     "all pass attachments must have the same sample count")   \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_RESOLVE_COLOR_IMAGE_MSAA,        \
                     "pass resolve attachments must have a color attachment "  \
                     "image with sample count > 1")                            \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE,                   \
                     "pass resolve attachment image not valid")                \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_RESOLVE_SAMPLE_COUNT,            \
                     "pass resolve attachment image sample count must be 1")   \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_RESOLVE_MIPLEVEL,                               \
      "pass resolve attachment mip level is bigger than image has mipmaps")    \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_RESOLVE_FACE,                                   \
      "pass resolve attachment is cubemap, but face index is too big")         \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_RESOLVE_LAYER,                                  \
      "pass resolve attachment is array texture, but layer index is too big")  \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_RESOLVE_SLICE,                                  \
      "pass resolve attachment is 3d texture, but slice value is too big")     \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_NO_RT,                            \
      "pass resolve attachment image must have render_target=true")            \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_SIZES,                            \
      "pass resolve attachment size must match color attachment image size")   \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_FORMAT,            \
                     "pass resolve attachment pixel format must match color "  \
                     "attachment pixel format")                                \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_DEPTH_IMAGE,                     \
                     "pass depth attachment image is not valid")               \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_DEPTH_MIPLEVEL,                                 \
      "pass depth attachment mip level is bigger than image has mipmaps")      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_DEPTH_FACE,                                     \
      "pass depth attachment image is cubemap, but face index is too big")     \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_DEPTH_LAYER,                     \
                     "pass depth attachment image is array texture, but "      \
                     "layer index is too big")                                 \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_DEPTH_SLICE,                                    \
      "pass depth attachment image is 3d texture, but slice value is too big") \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ATTACHMENTSDESC_DEPTH_IMAGE_NO_RT,                              \
      "pass depth attachment image must be have render_target=true")           \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_DEPTH_IMAGE_SIZES,               \
                     "pass depth attachment image size must match color "      \
                     "attachment image size")                                  \
  _SG_LOGITEM_XMACRO(VALIDATE_ATTACHMENTSDESC_DEPTH_IMAGE_SAMPLE_COUNT,        \
                     "pass depth attachment sample count must match color "    \
                     "attachment sample count")                                \
  _SG_LOGITEM_XMACRO(VALIDATE_BEGINPASS_CANARY,                                \
                     "sg_begin_pass: pass struct not initialized")             \
  _SG_LOGITEM_XMACRO(VALIDATE_BEGINPASS_EXPECT_NO_ATTACHMENTS,                 \
                     "sg_begin_pass: compute passes cannot have attachments")  \
  _SG_LOGITEM_XMACRO(VALIDATE_BEGINPASS_ATTACHMENTS_EXISTS,                    \
                     "sg_begin_pass: attachments object no longer alive")      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BEGINPASS_ATTACHMENTS_VALID,                                    \
      "sg_begin_pass: attachments object not in resource state VALID")         \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BEGINPASS_COLOR_ATTACHMENT_IMAGE,                               \
      "sg_begin_pass: one or more color attachment images are not valid")      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BEGINPASS_RESOLVE_ATTACHMENT_IMAGE,                             \
      "sg_begin_pass: one or more resolve attachment images are not valid")    \
  _SG_LOGITEM_XMACRO(VALIDATE_BEGINPASS_DEPTHSTENCIL_ATTACHMENT_IMAGE,         \
                     "sg_begin_pass: one or more depth-stencil attachment "    \
                     "images are not valid")                                   \
  _SG_LOGITEM_XMACRO(VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_WIDTH,                \
                     "sg_begin_pass: expected pass.swapchain.width > 0")       \
  _SG_LOGITEM_XMACRO(VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_WIDTH_NOTSET,         \
                     "sg_begin_pass: expected pass.swapchain.width == 0")      \
  _SG_LOGITEM_XMACRO(VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_HEIGHT,               \
                     "sg_begin_pass: expected pass.swapchain.height > 0")      \
  _SG_LOGITEM_XMACRO(VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_HEIGHT_NOTSET,        \
                     "sg_begin_pass: expected pass.swapchain.height == 0")     \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_SAMPLECOUNT,                         \
      "sg_begin_pass: expected pass.swapchain.sample_count > 0")               \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_SAMPLECOUNT_NOTSET,                  \
      "sg_begin_pass: expected pass.swapchain.sample_count == 0")              \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_COLORFORMAT,                         \
      "sg_begin_pass: expected pass.swapchain.color_format to be valid")       \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_COLORFORMAT_NOTSET,                  \
      "sg_begin_pass: expected pass.swapchain.color_format to be unset")       \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_DEPTHFORMAT_NOTSET,                  \
      "sg_begin_pass: expected pass.swapchain.depth_format to be unset")       \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_BEGINPASS_SWAPCHAIN_METAL_EXPECT_CURRENTDRAWABLE,               \
      "sg_begin_pass: expected pass.swapchain.metal.current_drawable != 0")    \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "sg_begin_pass: expected pass.swapchain.metal.depth_stencil_texture != " \
      "0")                                                                     \
  _SG_LOGITEM_XMACRO(                                                          \
  ...
  _SG_LOGITEM_XMACRO(                                                          \
      ...
      "sg_begin_pass: expected pass.swapchain.gl.framebuffer == 0")            \
  _SG_LOGITEM_XMACRO(VALIDATE_AVP_RENDERPASS_EXPECTED,                         \
                     "sg_apply_viewport: must be called in a render pass")     \
  _SG_LOGITEM_XMACRO(VALIDATE_ASR_RENDERPASS_EXPECTED,                         \
                     "sg_apply_scissor_rect: must be called in a render pass") \
  _SG_LOGITEM_XMACRO(VALIDATE_APIP_PIPELINE_VALID_ID,                          \
                     "sg_apply_pipeline: invalid pipeline id provided")        \
  _SG_LOGITEM_XMACRO(VALIDATE_APIP_PIPELINE_EXISTS,                            \
                     "sg_apply_pipeline: pipeline object no longer alive")     \
  _SG_LOGITEM_XMACRO(VALIDATE_APIP_PIPELINE_VALID,                             \
                     "sg_apply_pipeline: pipeline object not in valid state")  \
  _SG_LOGITEM_XMACRO(VALIDATE_APIP_PASS_EXPECTED,                              \
                     "sg_apply_pipeline: must be called in a pass")            \
  _SG_LOGITEM_XMACRO(VALIDATE_APIP_SHADER_EXISTS,                              \
                     "sg_apply_pipeline: shader object no longer alive")       \
  _SG_LOGITEM_XMACRO(VALIDATE_APIP_SHADER_VALID,                               \
                     "sg_apply_pipeline: shader object not in valid state")    \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_APIP_COMPUTEPASS_EXPECTED,                                      \
      "sg_apply_pipeline: trying to apply compute pipeline in render pass")    \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_APIP_RENDERPASS_EXPECTED,                                       \
      "sg_apply_pipeline: trying to apply render pipeline in compute pass")    \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_APIP_CURPASS_ATTACHMENTS_EXISTS,                                \
      "sg_apply_pipeline: current pass attachments no longer alive")           \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_APIP_CURPASS_ATTACHMENTS_VALID,                                 \
      "sg_apply_pipeline: current pass attachments not in valid state")        \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_APIP_ATT_COUNT,                                                 \
      "sg_apply_pipeline: number of pipeline color attachments doesn't match " \
      "number of pass color attachments")                                      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_APIP_COLOR_FORMAT,                                              \
      "sg_apply_pipeline: pipeline color attachment pixel format doesn't "     \
      "match pass color attachment pixel format")                              \
  _SG_LOGITEM_XMACRO(VALIDATE_APIP_DEPTH_FORMAT,                               \
                     "sg_apply_pipeline: pipeline depth pixel_format doesn't " \
                     "match pass depth attachment pixel format")               \
  _SG_LOGITEM_XMACRO(VALIDATE_APIP_SAMPLE_COUNT,                               \
                     "sg_apply_pipeline: pipeline MSAA sample count doesn't "  \
                     "match render pass attachment sample count")              \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_PASS_EXPECTED,                              \
                     "sg_apply_bindings: must be called in a pass")            \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_EMPTY_BINDINGS,                                            \
      "sg_apply_bindings: the provided sg_bindings struct is empty")           \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_PIPELINE,                                                  \
      "sg_apply_bindings: must be called after sg_apply_pipeline")             \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_PIPELINE_EXISTS,                                           \
      "sg_apply_bindings: currently applied pipeline object no longer alive")  \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_PIPELINE_VALID,                             \
                     "sg_apply_bindings: currently applied pipeline object "   \
                     "not in valid state")                                     \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_COMPUTE_EXPECTED_NO_VBS,                    \
                     "sg_apply_bindings: vertex buffer bindings not allowed "  \
                     "in a compute pass")                                      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_COMPUTE_EXPECTED_NO_IB,                                    \
      "sg_apply_bindings: index buffer binding not allowed in compute pass")   \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_EXPECTED_VB,                                \
                     "sg_apply_bindings: vertex buffer binding is missing or " \
                     "buffer handle is invalid")                               \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VB_EXISTS,                                  \
                     "sg_apply_bindings: vertex buffer no longer alive")       \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_VB_TYPE,                                    \
                     "sg_apply_bindings: buffer in vertex buffer slot is not " \
                     "a SG_BUFFERTYPE_VERTEXBUFFER")                           \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_VB_OVERFLOW,                                               \
      "sg_apply_bindings: buffer in vertex buffer slot is overflown")          \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_NO_IB,                                      \
                     "sg_apply_bindings: pipeline object defines indexed "     \
                     "rendering, but no index buffer provided")                \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_IB,                                         \
                     "sg_apply_bindings: pipeline object defines non-indexed " \
                     "rendering, but index buffer provided")                   \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_IB_EXISTS,                                  \
                     "sg_apply_bindings: index buffer no longer alive")        \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_IB_TYPE,                                    \
                     "sg_apply_bindings: buffer in index buffer slot is not "  \
                     "a SG_BUFFERTYPE_INDEXBUFFER")                            \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_IB_OVERFLOW,                                               \
      "sg_apply_bindings: buffer in index buffer slot is overflown")           \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_EXPECTED_IMAGE_BINDING,                     \
                     "sg_apply_bindings: image binding is missing or the "     \
                     "image handle is invalid")                                \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_IMG_EXISTS,                                 \
                     "sg_apply_bindings: bound image no longer alive")         \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_IMAGE_TYPE_MISMATCH,                                       \
      "sg_apply_bindings: type of bound image doesn't match shader desc")      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_EXPECTED_MULTISAMPLED_IMAGE,                               \
      "sg_apply_bindings: expected image with sample_count > 1")               \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_IMAGE_MSAA,                                                \
      "sg_apply_bindings: cannot bind image with sample_count>1")              \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_EXPECTED_FILTERABLE_IMAGE,                  \
                     "sg_apply_bindings: filterable image expected")           \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_EXPECTED_DEPTH_IMAGE,                       \
                     "sg_apply_bindings: depth image expected")                \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_EXPECTED_SAMPLER_BINDING,                   \
                     "sg_apply_bindings: sampler binding is missing or the "   \
                     "sampler handle is invalid")                              \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_UNEXPECTED_SAMPLER_COMPARE_NEVER,                          \
      "sg_apply_bindings: shader expects SG_SAMPLERTYPE_COMPARISON but "       \
      "sampler has SG_COMPAREFUNC_NEVER")                                      \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_EXPECTED_SAMPLER_COMPARE_NEVER,                            \
      "sg_apply_bindings: shader expects SG_SAMPLERTYPE_FILTERING or "         \
      "SG_SAMPLERTYPE_NONFILTERING but sampler doesn't have "                  \
      "SG_COMPAREFUNC_NEVER")                                                  \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_EXPECTED_NONFILTERING_SAMPLER,                             \
      "sg_apply_bindings: shader expected SG_SAMPLERTYPE_NONFILTERING, but "   \
      "sampler has SG_FILTER_LINEAR filters")                                  \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_SMP_EXISTS,                                 \
                     "sg_apply_bindings: bound sampler no longer alive")       \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_EXPECTED_STORAGEBUFFER_BINDING,             \
                     "sg_apply_bindings: storage buffer binding is missing "   \
                     "or the buffer handle is invalid")                        \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_ABND_STORAGEBUFFER_EXISTS,                                      \
      "sg_apply_bindings: bound storage buffer no longer alive")               \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_STORAGEBUFFER_BINDING_BUFFERTYPE,           \
                     "sg_apply_bindings: buffer bound to storage buffer slot " \
                     "is not of type storage buffer")                          \
  _SG_LOGITEM_XMACRO(VALIDATE_ABND_STORAGEBUFFER_READWRITE_IMMUTABLE,          \
                     "sg_apply_bindings: storage buffers bound as read/write " \
                     "must have usage immutable")                              \
  _SG_LOGITEM_XMACRO(VALIDATE_AU_PASS_EXPECTED,                                \
                     "sg_apply_uniforms: must be called in a pass")            \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_AU_NO_PIPELINE,                                                 \
      "sg_apply_uniforms: must be called after sg_apply_pipeline()")           \
  _SG_LOGITEM_XMACRO(VALIDATE_AU_NO_UNIFORMBLOCK_AT_SLOT,                      \
                     "sg_apply_uniforms: no uniform block declaration at "     \
                     "this shader stage UB slot")                              \
  _SG_LOGITEM_XMACRO(VALIDATE_AU_SIZE, "sg_apply_uniforms: data size doesn't " \
                                       "match declared uniform block size")    \
  _SG_LOGITEM_XMACRO(VALIDATE_DRAW_RENDERPASS_EXPECTED,                        \
                     "sg_draw: must be called in a render pass")               \
  _SG_LOGITEM_XMACRO(VALIDATE_DRAW_BASEELEMENT,                                \
                     "sg_draw: base_element cannot be < 0")                    \
  _SG_LOGITEM_XMACRO(VALIDATE_DRAW_NUMELEMENTS,                                \
                     "sg_draw: num_elements cannot be < 0")                    \
  _SG_LOGITEM_XMACRO(VALIDATE_DRAW_NUMINSTANCES,                               \
                     "sg_draw: num_instances cannot be < 0")                   \
  _SG_LOGITEM_XMACRO(VALIDATE_DRAW_REQUIRED_BINDINGS_OR_UNIFORMS_MISSING,      \
                     "sg_draw: call to sg_apply_bindings() and/or "            \
                     "sg_apply_uniforms() missing after sg_apply_pipeline()")  \
  _SG_LOGITEM_XMACRO(VALIDATE_DISPATCH_COMPUTEPASS_EXPECTED,                   \
                     "sg_dispatch: must be called in a compute pass")          \
  _SG_LOGITEM_XMACRO(VALIDATE_DISPATCH_NUMGROUPSX,                             \
                     "sg_dispatch: num_groups_x must be >=0 and <65536")       \
  _SG_LOGITEM_XMACRO(VALIDATE_DISPATCH_NUMGROUPSY,                             \
                     "sg_dispatch: num_groups_y must be >=0 and <65536")       \
  _SG_LOGITEM_XMACRO(VALIDATE_DISPATCH_NUMGROUPSZ,                             \
                     "sg_dispatch: num_groups_z must be >=0 and <65536")       \
  _SG_LOGITEM_XMACRO(VALIDATE_DISPATCH_REQUIRED_BINDINGS_OR_UNIFORMS_MISSING,  \
                     "sg_dispatch: call to sg_apply_bindings() and/or "        \
                     "sg_apply_uniforms() missing after sg_apply_pipeline()")  \
  _SG_LOGITEM_XMACRO(VALIDATE_UPDATEBUF_USAGE,                                 \
                     "sg_update_buffer: cannot update immutable buffer")       \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_UPDATEBUF_SIZE,                                                 \
      "sg_update_buffer: update size is bigger than buffer size")              \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_UPDATEBUF_ONCE,                                                 \
      "sg_update_buffer: only one update allowed per buffer and frame")        \
  _SG_LOGITEM_XMACRO(VALIDATE_UPDATEBUF_APPEND,                                \
                     "sg_update_buffer: cannot call sg_update_buffer and "     \
                     "sg_append_buffer in same frame")                         \
  _SG_LOGITEM_XMACRO(VALIDATE_APPENDBUF_USAGE,                                 \
  ...
  _SG_LOGITEM_XMACRO(VALIDATE_APPENDBUF_UPDATE,                                \
                     "sg_append_buffer: cannot call sg_append_buffer and "     \
                     "sg_update_buffer in same frame")                         \
  _SG_LOGITEM_XMACRO(VALIDATE_UPDIMG_USAGE,                                    \
                     "sg_update_image: cannot update immutable image")         \
  _SG_LOGITEM_XMACRO(                                                          \
      VALIDATE_UPDIMG_ONCE,                                                    \
      "sg_update_image: only one update allowed per image and frame")          \
  _SG_LOGITEM_XMACRO(VALIDATION_FAILED, "validation layer checks failed")

#define _SG_LOGITEM_XMACRO(item, msg) SG_LOGITEM_##item,
typedef enum sg_log_item { _SG_LOG_ITEMS } sg_log_item;
...
/*
    ...
    .environment.defaults.color_format: default value depends on selected
   backend: all GL backends:    SG_PIXELFORMAT_RGBA8 Metal and D3D11:
   SG_PIXELFORMAT_BGRA8 WebGPU:             *no default* (must be queried from
   WebGPU swapchain object) .environment.defaults.depth_format:
   SG_PIXELFORMAT_DEPTH_STENCIL .environment.defaults.sample_count: 1
   ...
   managed storage mode, otherwise sokol-gfx will decide whether to create
   buffers and textures in managed or shared storage mode (this is mainly a
   debugging option) .mtl_use_command_buffer_with_retained_references when true,
   the sokol-gfx Metal backend will use Metal command buffers which bump the
   reference count of resource objects as long as they are inflight, this is
   slower than the default command-buffer-with-unretained-references method,
   this may be a workaround when confronted with lifetime validation errors from
   the Metal validation layer until a proper fix has been implemented
    ...
    D3D11 specific:
        ...
        .d3d11_shader_debugging
            set this to true to compile shaders which are provided as HLSL
   source code with debug information and without optimization, this allows
    ...
    initialize the sg_desc.environment nested struct. sglue_environment()
   returns a completely initialized sg_environment struct with information
...
typedef struct sg_logger {
  void (*func)(const char* tag,       // always "sg"
...
typedef struct sg_desc {
  ...
  int max_dispatch_calls_per_pass;  // max expected number of dispatch calls per
                                    // pass (default: 1024)
  int max_commit_listeners;
  bool disable_validation;      // disable validation layer even in debug mode,
                                // useful for tests
  bool d3d11_shader_debugging;  // if true, HLSL shaders are compiled with
                                // D3DCOMPILE_DEBUG |
                                // D3DCOMPILE_SKIP_OPTIMIZATION
  bool mtl_force_managed_storage_mode;  // for debugging: use Metal managed
                                        // storage mode for resources even with
                                        // UMA
  bool mtl_use_command_buffer_with_retained_references;  // Metal: use a managed
                                                         // MTLCommandBuffer
                                                         // which ref-counts
                                                         // used resources
  bool wgpu_disable_bindgroups_cache;  // set to true to disable the WebGPU
                                       // backend BindGroup cache
  int wgpu_bindgroups_cache_size;  // number of slots in the WebGPU bindgroup
                                   // cache (must be 2^N)
...
SOKOL_GFX_API_DECL sg_trace_hooks
...
SOKOL_GFX_API_DECL sg_attachments
sg_make_attachments(const sg_attachments_desc* desc);
...
SOKOL_GFX_API_DECL void sg_update_image(sg_image img,
                                        const sg_image_data* data);
...
SOKOL_GFX_API_DECL bool sg_query_buffer_will_overflow(sg_buffer buf,
                                                      size_t size);

// render and compute functions
SOKOL_GFX_API_DECL void sg_begin_pass(const sg_pass* pass);
SOKOL_GFX_API_DECL void sg_apply_viewport(int x,
                                          int y,
                                          int width,
                                          int height,
                                          bool origin_top_left);
SOKOL_GFX_API_DECL void sg_apply_viewportf(float x,
                                           float y,
                                           float width,
                                           float height,
                                           bool origin_top_left);
SOKOL_GFX_API_DECL void sg_apply_scissor_rect(int x,
                                              int y,
                                              int width,
                                              int height,
                                              bool origin_top_left);
SOKOL_GFX_API_DECL void sg_apply_scissor_rectf(float x,
                                               float y,
                                               float width,
                                               float height,
                                               bool origin_top_left);
...
SOKOL_GFX_API_DECL void sg_draw(int base_element,
                                int num_elements,
                                int num_instances);
SOKOL_GFX_API_DECL void sg_dispatch(int num_groups_x,
                                    int num_groups_y,
                                    int num_groups_z);
...
SOKOL_GFX_API_DECL sg_pixelformat_info
sg_query_pixelformat(sg_pixel_format fmt);
SOKOL_GFX_API_DECL int sg_query_row_pitch(sg_pixel_format fmt,
                                          int width,
                                          int row_align_bytes);
SOKOL_GFX_API_DECL int sg_query_surface_pitch(sg_pixel_format fmt,
                                              int width,
                                              int height,
                                              int row_align_bytes);
...
sg_query_attachments_state(sg_attachments atts);
...
SOKOL_GFX_API_DECL sg_attachments_info
sg_query_attachments_info(sg_attachments atts);
...
SOKOL_GFX_API_DECL sg_attachments_desc
sg_query_attachments_desc(sg_attachments atts);
// get resource creation desc struct with their default values replaced
SOKOL_GFX_API_DECL sg_buffer_desc
sg_query_buffer_defaults(const sg_buffer_desc* desc);
SOKOL_GFX_API_DECL sg_image_desc
sg_query_image_defaults(const sg_image_desc* desc);
SOKOL_GFX_API_DECL sg_sampler_desc
sg_query_sampler_defaults(const sg_sampler_desc* desc);
SOKOL_GFX_API_DECL sg_shader_desc
sg_query_shader_defaults(const sg_shader_desc* desc);
SOKOL_GFX_API_DECL sg_pipeline_desc
sg_query_pipeline_defaults(const sg_pipeline_desc* desc);
SOKOL_GFX_API_DECL sg_attachments_desc
sg_query_attachments_defaults(const sg_attachments_desc* desc);
...
SOKOL_GFX_API_DECL void sg_init_buffer(sg_buffer buf,
                                       const sg_buffer_desc* desc);
SOKOL_GFX_API_DECL void sg_init_image(sg_image img, const sg_image_desc* desc);
SOKOL_GFX_API_DECL void sg_init_sampler(sg_sampler smg,
                                        const sg_sampler_desc* desc);
SOKOL_GFX_API_DECL void sg_init_shader(sg_shader shd,
                                       const sg_shader_desc* desc);
SOKOL_GFX_API_DECL void sg_init_pipeline(sg_pipeline pip,
                                         const sg_pipeline_desc* desc);
SOKOL_GFX_API_DECL void sg_init_attachments(sg_attachments attachments,
                                            const sg_attachments_desc* desc);
...
typedef struct sg_d3d11_shader_info {
  const void* cbufs[SG_MAX_UNIFORMBLOCK_BINDSLOTS];  // ID3D11Buffer* (constant
                                                     // buffers by bind slot)
...
SOKOL_GFX_API_DECL sg_d3d11_buffer_info
sg_d3d11_query_buffer_info(sg_buffer buf);
...
SOKOL_GFX_API_DECL sg_d3d11_sampler_info
sg_d3d11_query_sampler_info(sg_sampler smp);
// D3D11: get internal shader resource objects
SOKOL_GFX_API_DECL sg_d3d11_shader_info
sg_d3d11_query_shader_info(sg_shader shd);
// D3D11: get internal pipeline resource objects
SOKOL_GFX_API_DECL sg_d3d11_pipeline_info
sg_d3d11_query_pipeline_info(sg_pipeline pip);
// D3D11: get internal pass resource objects
SOKOL_GFX_API_DECL sg_d3d11_attachments_info
sg_d3d11_query_attachments_info(sg_attachments atts);
...
// Metal: return __bridge-casted MTLComputeCommandEncoder when inside compute
// pass (otherwise zero)
...
SOKOL_GFX_API_DECL sg_mtl_sampler_info
sg_mtl_query_sampler_info(sg_sampler smp);
...
SOKOL_GFX_API_DECL sg_mtl_pipeline_info
sg_mtl_query_pipeline_info(sg_pipeline pip);
...
SOKOL_GFX_API_DECL sg_wgpu_sampler_info
sg_wgpu_query_sampler_info(sg_sampler smp);
...
SOKOL_GFX_API_DECL sg_wgpu_pipeline_info
sg_wgpu_query_pipeline_info(sg_pipeline pip);
// WebGPU: get internal pass resource objects
SOKOL_GFX_API_DECL sg_wgpu_attachments_info
sg_wgpu_query_attachments_info(sg_attachments atts);
...
SOKOL_GFX_API_DECL sg_gl_attachments_info
sg_gl_query_attachments_info(sg_attachments atts);

#ifdef __cplusplus
}  // extern "C"

// reference-based equivalents for c++
inline void sg_setup(const sg_desc& desc) {
  return sg_setup(&desc);
}

inline sg_buffer sg_make_buffer(const sg_buffer_desc& desc) {
  return sg_make_buffer(&desc);
}
inline sg_image sg_make_image(const sg_image_desc& desc) {
  return sg_make_image(&desc);
}
inline sg_sampler sg_make_sampler(const sg_sampler_desc& desc) {
  return sg_make_sampler(&desc);
}
inline sg_shader sg_make_shader(const sg_shader_desc& desc) {
  return sg_make_shader(&desc);
}
inline sg_pipeline sg_make_pipeline(const sg_pipeline_desc& desc) {
  return sg_make_pipeline(&desc);
}
inline sg_attachments sg_make_attachments(const sg_attachments_desc& desc) {
  return sg_make_attachments(&desc);
}
inline void sg_update_image(sg_image img, const sg_image_data& data) {
  return sg_update_image(img, &data);
}

inline void sg_begin_pass(const sg_pass& pass) {
  return sg_begin_pass(&pass);
}
inline void sg_apply_bindings(const sg_bindings& bindings) {
  return sg_apply_bindings(&bindings);
}
inline void sg_apply_uniforms(int ub_slot, const sg_range& data) {
  return sg_apply_uniforms(ub_slot, &data);
}

inline sg_buffer_desc sg_query_buffer_defaults(const sg_buffer_desc& desc) {
  return sg_query_buffer_defaults(&desc);
}
inline sg_image_desc sg_query_image_defaults(const sg_image_desc& desc) {
  return sg_query_image_defaults(&desc);
}
inline sg_sampler_desc sg_query_sampler_defaults(const sg_sampler_desc& desc) {
  return sg_query_sampler_defaults(&desc);
}
inline sg_shader_desc sg_query_shader_defaults(const sg_shader_desc& desc) {
  return sg_query_shader_defaults(&desc);
}
inline sg_pipeline_desc sg_query_pipeline_defaults(
    const sg_pipeline_desc& desc) {
  return sg_query_pipeline_defaults(&desc);
}
inline sg_attachments_desc sg_query_attachments_defaults(
    const sg_attachments_desc& desc) {
  return sg_query_attachments_defaults(&desc);
}

inline void sg_init_buffer(sg_buffer buf, const sg_buffer_desc& desc) {
  return sg_init_buffer(buf, &desc);
}
inline void sg_init_image(sg_image img, const sg_image_desc& desc) {
  return sg_init_image(img, &desc);
}
inline void sg_init_sampler(sg_sampler smp, const sg_sampler_desc& desc) {
  return sg_init_sampler(smp, &desc);
}
inline void sg_init_shader(sg_shader shd, const sg_shader_desc& desc) {
  return sg_init_shader(shd, &desc);
}
inline void sg_init_pipeline(sg_pipeline pip, const sg_pipeline_desc& desc) {
  return sg_init_pipeline(pip, &desc);
}
inline void sg_init_attachments(sg_attachments atts,
                                const sg_attachments_desc& desc) {
  return sg_init_attachments(atts, &desc);
}

inline void sg_update_buffer(sg_buffer buf_id, const sg_range& data) {
  return sg_update_buffer(buf_id, &data);
}
inline int sg_append_buffer(sg_buffer buf_id, const sg_range& data) {
  return sg_append_buffer(buf_id, &data);
}
#endif
...
#error \
    "Please select a backend with SOKOL_GLCORE, SOKOL_GLES3, SOKOL_D3D11, SOKOL_METAL, SOKOL_WGPU or SOKOL_DUMMY_BACKEND"
#endif
#if defined(SOKOL_MALLOC) || defined(SOKOL_CALLOC) || defined(SOKOL_FREE)
#error \
    "SOKOL_MALLOC/CALLOC/FREE macros are no longer supported, please use sg_desc.allocator to override memory allocation functions"
#endif
...
#pragma warning(disable : 4201)  // nonstandard extension used: nameless
                                 // struct/union (needed by d3d11.h)
...
_SOKOL_PRIVATE uint32_t _sg_slot_alloc(_sg_pool_t* pool,
                                       _sg_slot_t* slot,
                                       int slot_index);
...
struct _sg_pools_s;
struct _sg_buffer_s;
_SOKOL_PRIVATE struct _sg_buffer_s* _sg_lookup_buffer(
    const struct _sg_pools_s* p,
    uint32_t buf_id);
...
_SOKOL_PRIVATE void _sg_buffer_common_init(_sg_buffer_common_t* cmn,
                                           const sg_buffer_desc* desc) {
  ...
  cmn->num_slots =
      (desc->usage == SG_USAGE_IMMUTABLE) ? 1 : SG_NUM_INFLIGHT_FRAMES;
  cmn->active_slot = 0;
  cmn->type = desc->type;
...
_SOKOL_PRIVATE void _sg_image_common_init(_sg_image_common_t* cmn,
                                          const sg_image_desc* desc) {
...
_SOKOL_PRIVATE void _sg_sampler_common_init(_sg_sampler_common_t* cmn,
                                            const sg_sampler_desc* desc) {
...
_SOKOL_PRIVATE void _sg_shader_common_init(_sg_shader_common_t* cmn,
                                           const sg_shader_desc* desc) {
  cmn->is_compute =
      desc->compute_func.source || desc->compute_func.bytecode.ptr;
  ...
  for (size_t i = 0; i < SG_MAX_IMAGE_SAMPLER_PAIRS; i++) {
    ...
    if (src->stage != SG_SHADERSTAGE_NONE) {
      dst->stage = src->stage;
      SOKOL_ASSERT((src->image_slot >= 0) &&
                   (src->image_slot < SG_MAX_IMAGE_BINDSLOTS));
      SOKOL_ASSERT(desc->images[src->image_slot].stage == src->stage);
      dst->image_slot = src->image_slot;
      SOKOL_ASSERT((src->sampler_slot >= 0) &&
                   (src->sampler_slot < SG_MAX_SAMPLER_BINDSLOTS));
...
_SOKOL_PRIVATE void _sg_pipeline_common_init(_sg_pipeline_common_t* cmn,
                                             const sg_pipeline_desc* desc) {
  SOKOL_ASSERT((desc->color_count >= 0) &&
...
_SOKOL_PRIVATE void _sg_attachment_common_init(_sg_attachment_common_t* cmn,
                                               const sg_attachment_desc* desc) {
  cmn->image_id = desc->image;
  cmn->mip_level = desc->mip_level;
  cmn->slice = desc->slice;
}

_SOKOL_PRIVATE void _sg_attachments_common_init(_sg_attachments_common_t* cmn,
                                                const sg_attachments_desc* desc,
                                                int width,
                                                int height) {
...
typedef struct _sg_buffer_s {
  ...
  struct {
    ...
    bool injected;   // if true, external buffers were injected with
                     // sg_buffer_desc.gl_buffers
    bool gpu_dirty;  // true if modified by GPU shader but memory barrier hasn't
                     // been issued yet
  } gl;
} _sg_gl_buffer_t;
...
typedef struct _sg_image_s {
  ...
  struct {
    ...
    bool injected;  // if true, external textures were injected with
...
typedef struct _sg_sampler_s {
  ...
  struct {
    GLuint smp;
    bool injected;  // true if external sampler was injects in
                    // sg_sampler_desc.gl_sampler
  } gl;
} _sg_gl_sampler_t;
...
typedef struct {
  ...
  GLuint prog;
  _sg_gl_cache_texture_sampler_bind_slot
      texture_samplers[_SG_GL_MAX_IMG_SMP_BINDINGS];
...
#define _SG_D3D11_MAX_STAGE_SRV_BINDINGS \
  (SG_MAX_IMAGE_BINDSLOTS + SG_MAX_STORAGEBUFFER_BINDSLOTS)
...
typedef struct {
  ...
  D3D11_SUBRESOURCE_DATA
      subres_data[SG_MAX_MIPMAPS * SG_MAX_TEXTUREARRAY_LAYERS];
} _sg_d3d11_backend_t;
...
#define _SG_MTL_MAX_STAGE_UB_SBUF_BINDINGS \
  (_SG_MTL_MAX_STAGE_UB_BINDINGS + SG_MAX_STORAGEBUFFER_BINDSLOTS)
#define _SG_MTL_MAX_STAGE_BUFFER_BINDINGS \
  (_SG_MTL_MAX_STAGE_UB_SBUF_BINDINGS + SG_MAX_VERTEXBUFFER_BINDSLOTS)
...
#define _SG_WGPU_MAX_UNIFORM_UPDATE_SIZE \
  (1 << 16)  // also see WGPULimits.maxUniformBufferBindingSize
#define _SG_WGPU_NUM_BINDGROUPS \
  (2)  // 0: uniforms, 1: images, samplers, storage buffers
...
#define _SG_WGPU_MAX_IMG_SMP_SBUF_BINDGROUP_ENTRIES    \
  (SG_MAX_IMAGE_BINDSLOTS + SG_MAX_SAMPLER_BINDSLOTS + \
...
typedef struct _sg_shader_s {
  ...
  struct {
    ...
    // a mapping of sokol-gfx bind slots to setBindGroup dynamic-offset-array
    // indices
...
typedef struct {
  ...
  uint32_t
      bind_offsets[SG_MAX_UNIFORMBLOCK_BINDSLOTS];  // NOTE: index is sokol-gfx
                                                    // ub slot index!
...
#define _SG_WGPU_BINDGROUPSCACHEKEY_NUM_ITEMS \
  (1 + _SG_WGPU_MAX_IMG_SMP_SBUF_BINDGROUP_ENTRIES)
typedef struct {
  uint64_t hash;
...
typedef struct {
  ...
  uint32_t required_bindings_and_uniforms;  // used to check that bindings and
                                            // uniforms are applied after
                                            // applying pipeline
  uint32_t applied_bindings_and_uniforms;   // bits 0..7: uniform blocks, bit 8:
                                            // bindings
#if defined(SOKOL_DEBUG)
  sg_log_item validate_error;
#endif
  struct {
    _sg_tracker_t readwrite_sbufs;  // tracks read/write storage buffers used in
                                    // compute pass
  } compute;
  _sg_pools_t pools;
  sg_backend backend;
...
static const char* _sg_log_messages[] = {_SG_LOG_ITEMS};
#undef _SG_LOGITEM_XMACRO
#endif  // SOKOL_DEBUG
...
#endif
    _sg.desc.logger.func("sg", log_level, (uint32_t)log_item, msg, line_nr,
                         filename, _sg.desc.logger.user_data);
  } else {
...
#if defined(__cplusplus)
#define _SG_CLEAR_ARC_STRUCT(type, item) \
  {                                      \
    item = type();                       \
  }
#else
...
_SOKOL_PRIVATE _sg_recti_t
_sg_clipi(int x, int y, int w, int h, int clip_width, int clip_height) {
...
_SOKOL_PRIVATE const char* _sg_shaderattrbasetype_to_string(
    sg_shader_attr_base_type b) {
  switch (b) {
...
_SOKOL_PRIVATE sg_shader_attr_base_type
_sg_vertexformat_basetype(sg_vertex_format fmt) {
  switch (fmt) {
    case SG_VERTEXFORMAT_FLOAT:
    case SG_VERTEXFORMAT_FLOAT2:
    case SG_VERTEXFORMAT_FLOAT3:
...
_SOKOL_PRIVATE uint32_t _sg_uniform_alignment(sg_uniform_type type,
                                              int array_count,
                                              sg_uniform_layout ub_layout) {
...
_SOKOL_PRIVATE uint32_t _sg_uniform_size(sg_uniform_type type,
                                         int array_count,
                                         sg_uniform_layout ub_layout) {
  SOKOL_ASSERT(array_count > 0);
  if (array_count == 1) {
...
_SOKOL_PRIVATE bool _sg_is_valid_rendertarget_color_format(
    sg_pixel_format fmt) {
...
_SOKOL_PRIVATE bool _sg_is_valid_rendertarget_depth_format(
    sg_pixel_format fmt) {
  const int fmt_index = (int)fmt;
  SOKOL_ASSERT((fmt_index >= 0) && (fmt_index < _SG_PIXELFORMAT_NUM));
  return _sg.formats[fmt_index].render && _sg.formats[fmt_index].depth;
...
*/
_SOKOL_PRIVATE int _sg_row_pitch(sg_pixel_format fmt,
                                 int width,
                                 int row_align) {
  int pitch;
  switch (fmt) {
...
/* return pitch of a 2D subimage / texture slice
   ...
   https://github.com/microsoft/DirectXTex/blob/master/DirectXTex/DirectXTexUtil.cpp
*/
_SOKOL_PRIVATE int _sg_surface_pitch(sg_pixel_format fmt,
                                     int width,
                                     int height,
                                     int row_align) {
...
_SOKOL_PRIVATE sg_pass_action
_sg_pass_action_defaults(const sg_pass_action* action) {
  SOKOL_ASSERT(action);
...
_sg_dummy_create_buffer(_sg_buffer_t* buf, const sg_buffer_desc* desc) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_dummy_create_image(_sg_image_t* img, const sg_image_desc* desc) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_dummy_create_sampler(_sg_sampler_t* smp, const sg_sampler_desc* desc) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_dummy_create_shader(_sg_shader_t* shd, const sg_shader_desc* desc) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_dummy_create_pipeline(_sg_pipeline_t* pip,
                          _sg_shader_t* shd,
                          const sg_pipeline_desc* desc) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_dummy_create_attachments(_sg_attachments_t* atts,
                             _sg_image_t** color_images,
                             _sg_image_t** resolve_images,
                             _sg_image_t* ds_img,
                             const sg_attachments_desc* desc) {
  ...
  for (int i = 0; i < atts->cmn.num_colors; i++) {
    ...
    SOKOL_ASSERT(color_images[i] &&
                 (color_images[i]->slot.id == color_desc->image.id));
    SOKOL_ASSERT(_sg_is_valid_rendertarget_color_format(
        color_images[i]->cmn.pixel_format));
    ...
    if (resolve_desc->image.id != SG_INVALID_ID) {
      ...
      SOKOL_ASSERT(resolve_images[i] &&
                   (resolve_images[i]->slot.id == resolve_desc->image.id));
      SOKOL_ASSERT(color_images[i] && (color_images[i]->cmn.pixel_format ==
                                       resolve_images[i]->cmn.pixel_format));
  ...
  if (ds_desc->image.id != SG_INVALID_ID) {
    ...
    SOKOL_ASSERT(
        _sg_is_valid_rendertarget_depth_format(ds_img->cmn.pixel_format));
...
_SOKOL_PRIVATE _sg_image_t* _sg_dummy_attachments_color_image(
    const _sg_attachments_t* atts,
    int index) {
  SOKOL_ASSERT(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
  return atts->dmy.colors[index].image;
}

_SOKOL_PRIVATE _sg_image_t* _sg_dummy_attachments_resolve_image(
    const _sg_attachments_t* atts,
    int index) {
  SOKOL_ASSERT(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t* _sg_dummy_attachments_ds_image(
    const _sg_attachments_t* atts) {
  SOKOL_ASSERT(atts);
...
_SOKOL_PRIVATE void _sg_dummy_apply_viewport(int x,
                                             int y,
                                             int w,
                                             int h,
                                             bool origin_top_left) {
...
_SOKOL_PRIVATE void _sg_dummy_apply_scissor_rect(int x,
                                                 int y,
                                                 int w,
                                                 int h,
                                                 bool origin_top_left) {
...
_SOKOL_PRIVATE void _sg_dummy_apply_uniforms(int ub_slot,
                                             const sg_range* data) {
  _SOKOL_UNUSED(ub_slot);
  _SOKOL_UNUSED(data);
}

_SOKOL_PRIVATE void _sg_dummy_draw(int base_element,
                                   int num_elements,
                                   int num_instances) {
...
_SOKOL_PRIVATE void _sg_dummy_dispatch(int num_groups_x,
                                       int num_groups_y,
                                       int num_groups_z) {
...
_SOKOL_PRIVATE void _sg_dummy_update_buffer(_sg_buffer_t* buf,
                                            const sg_range* data) {
...
_SOKOL_PRIVATE bool _sg_dummy_append_buffer(_sg_buffer_t* buf,
                                            const sg_range* data,
                                            bool new_frame) {
  SOKOL_ASSERT(buf && data && data->ptr && (data->size > 0));
  _SOKOL_UNUSED(data);
...
_SOKOL_PRIVATE void _sg_dummy_update_image(_sg_image_t* img,
                                           const sg_image_data* data) {
...
#define _SG_GL_FUNCS                                                           \
  ...
  _SG_XMACRO(glFramebufferTextureLayer, void,                                  \
  ...
  _SG_XMACRO(glClearBufferfi, void,                                            \
             (GLenum buffer, GLint drawbuffer, GLfloat depth, GLint stencil))  \
  _SG_XMACRO(glClearBufferfv, void,                                            \
             (GLenum buffer, GLint drawbuffer, const GLfloat* value))          \
  _SG_XMACRO(glClearBufferuiv, void,                                           \
             (GLenum buffer, GLint drawbuffer, const GLuint* value))           \
  _SG_XMACRO(glClearBufferiv, void,                                            \
             (GLenum buffer, GLint drawbuffer, const GLint* value))            \
  _SG_XMACRO(glDeleteRenderbuffers, void,                                      \
             (GLsizei n, const GLuint* renderbuffers))                         \
  _SG_XMACRO(glUniform1fv, void,                                               \
             (GLint location, GLsizei count, const GLfloat* value))            \
  _SG_XMACRO(glUniform2fv, void,                                               \
             (GLint location, GLsizei count, const GLfloat* value))            \
  _SG_XMACRO(glUniform3fv, void,                                               \
             (GLint location, GLsizei count, const GLfloat* value))            \
  _SG_XMACRO(glUniform4fv, void,                                               \
             (GLint location, GLsizei count, const GLfloat* value))            \
  _SG_XMACRO(glUniform1iv, void,                                               \
             (GLint location, GLsizei count, const GLint* value))              \
  _SG_XMACRO(glUniform2iv, void,                                               \
             (GLint location, GLsizei count, const GLint* value))              \
  _SG_XMACRO(glUniform3iv, void,                                               \
             (GLint location, GLsizei count, const GLint* value))              \
  _SG_XMACRO(glUniform4iv, void,                                               \
             (GLint location, GLsizei count, const GLint* value))              \
  _SG_XMACRO(glUniformMatrix4fv, void,                                         \
             (GLint location, GLsizei count, GLboolean transpose,              \
              const GLfloat* value))                                           \
  _SG_XMACRO(glUseProgram, void, (GLuint program))                             \
  _SG_XMACRO(glShaderSource, void,                                             \
             (GLuint shader, GLsizei count, const GLchar* const* string,       \
              const GLint* length))                                            \
  _SG_XMACRO(glLinkProgram, void, (GLuint program))                            \
  _SG_XMACRO(glGetUniformLocation, GLint,                                      \
             (GLuint program, const GLchar* name))                             \
  _SG_XMACRO(glGetShaderiv, void,                                              \
  ...
  _SG_XMACRO(glStencilFuncSeparate, void,                                      \
             (GLenum face, GLenum func, GLint ref, GLuint mask))               \
  _SG_XMACRO(glStencilOpSeparate, void,                                        \
             (GLenum face, GLenum sfail, GLenum dpfail, GLenum dppass))        \
  _SG_XMACRO(glRenderbufferStorageMultisample, void,                           \
  ...
  _SG_XMACRO(glFramebufferRenderbuffer, void,                                  \
  ...
  _SG_XMACRO(glDrawElementsInstanced, void,                                    \
             (GLenum mode, GLsizei count, GLenum type, const void* indices,    \
              GLsizei instancecount))                                          \
  _SG_XMACRO(glVertexAttribPointer, void,                                      \
             (GLuint index, GLint size, GLenum type, GLboolean normalized,     \
              GLsizei stride, const void* pointer))                            \
  _SG_XMACRO(glVertexAttribIPointer, void,                                     \
  ...
  _SG_XMACRO(glBlendColor, void,                                               \
             (GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha))        \
  _SG_XMACRO(glTexParameterf, void,                                            \
             (GLenum target, GLenum pname, GLfloat param))                     \
  _SG_XMACRO(glTexParameterfv, void,                                           \
...
_SOKOL_PRIVATE void* _sg_gl_getprocaddr(
    const char* name,
    _sg_wglGetProcAddress wgl_getprocaddress) {
...
_SOKOL_PRIVATE void _sg_gl_load_opengl(void) {
  ...
  _sg_wglGetProcAddress wgl_getprocaddress =
      (_sg_wglGetProcAddress)GetProcAddress(_sg.gl.opengl32_dll,
                                            "wglGetProcAddress");
  SOKOL_ASSERT(wgl_getprocaddress);
#define _SG_XMACRO(name, ret, args) \
  name = (PFN_##name)_sg_gl_getprocaddr(#name, wgl_getprocaddress);
  _SG_GL_FUNCS
#undef _SG_XMACRO
}
...
#else
  SOKOL_ASSERT(sample_count == 1);
  _SOKOL_UNUSED(sample_count);
  switch (t) {
    case SG_IMAGETYPE_2D:
      return GL_TEXTURE_2D;
    case SG_IMAGETYPE_CUBE:
...
_SOKOL_PRIVATE void _sg_gl_init_pixelformats_float(
    bool has_colorbuffer_float,
...
_SOKOL_PRIVATE void _sg_gl_cache_bind_buffer(GLenum target, GLuint buffer) {
  SOKOL_ASSERT((GL_ARRAY_BUFFER == target) ||
...
_SOKOL_PRIVATE void _sg_gl_cache_bind_storage_buffer(uint8_t glsl_binding_n,
                                                     GLuint buffer) {
...
_SOKOL_PRIVATE void _sg_gl_cache_clear_texture_sampler_bindings(bool force) {
  ...
  for (int i = 0; (i < _SG_GL_MAX_IMG_SMP_BINDINGS) &&
                  (i < _sg.limits.gl_max_combined_texture_image_units);
       i++) {
...
_SOKOL_PRIVATE void _sg_gl_cache_bind_texture_sampler(int8_t gl_tex_slot,
                                                      GLenum target,
                                                      GLuint texture,
                                                      GLuint sampler) {
  /* it's valid to call this function with target=0 and/or texture=0
     ...
     the new binding
  */
  SOKOL_ASSERT((gl_tex_slot >= 0) &&
               (gl_tex_slot < _SG_GL_MAX_IMG_SMP_BINDINGS));
  if (gl_tex_slot >= _sg.limits.gl_max_combined_texture_image_units) {
    return;
  }
  _SG_GL_CHECK_ERROR();
  _sg_gl_cache_texture_sampler_bind_slot* slot =
      &_sg.gl.cache.texture_samplers[gl_tex_slot];
...
_SOKOL_PRIVATE void _sg_gl_cache_store_texture_sampler_binding(
    int8_t gl_tex_slot) {
  SOKOL_ASSERT((gl_tex_slot >= 0) &&
               (gl_tex_slot < _SG_GL_MAX_IMG_SMP_BINDINGS));
  _sg.gl.cache.stored_texture_sampler =
      _sg.gl.cache.texture_samplers[gl_tex_slot];
}

_SOKOL_PRIVATE void _sg_gl_cache_restore_texture_sampler_binding(
    int8_t gl_tex_slot) {
  SOKOL_ASSERT((gl_tex_slot >= 0) &&
               (gl_tex_slot < _SG_GL_MAX_IMG_SMP_BINDINGS));
  _sg_gl_cache_texture_sampler_bind_slot* slot =
  ...
  if (slot->texture != 0) {
    ...
    _sg_gl_cache_bind_texture_sampler(gl_tex_slot, slot->target, slot->texture,
                                      slot->sampler);
    slot->target = 0;
    slot->texture = 0;
    slot->sampler = 0;
...
_SOKOL_PRIVATE void _sg_gl_cache_invalidate_texture_sampler(GLuint tex,
                                                            GLuint smp) {
  _SG_GL_CHECK_ERROR();
  for (size_t i = 0; i < _SG_GL_MAX_IMG_SMP_BINDINGS; i++) {
    _sg_gl_cache_texture_sampler_bind_slot* slot =
        &_sg.gl.cache.texture_samplers[i];
    if ((0 != slot->target) &&
        ((tex == slot->texture) || (smp == slot->sampler))) {
  ...
  if ((tex == _sg.gl.cache.stored_texture_sampler.texture) ||
      (smp == _sg.gl.cache.stored_texture_sampler.sampler)) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_gl_create_buffer(_sg_buffer_t* buf, const sg_buffer_desc* desc) {
  SOKOL_ASSERT(buf && desc);
  _SG_GL_CHECK_ERROR();
...
_SOKOL_PRIVATE bool _sg_gl_supported_texture_format(sg_pixel_format fmt) {
  const int fmt_index = (int)fmt;
  SOKOL_ASSERT((fmt_index > SG_PIXELFORMAT_NONE) &&
               (fmt_index < _SG_PIXELFORMAT_NUM));
  return _sg.formats[fmt_index].sample;
}

_SOKOL_PRIVATE sg_resource_state
_sg_gl_create_image(_sg_image_t* img, const sg_image_desc* desc) {
  SOKOL_ASSERT(img && desc);
  _SG_GL_CHECK_ERROR();
  ...
  const GLenum gl_internal_format =
      _sg_gl_teximage_internal_format(img->cmn.pixel_format);

  // GLES3/WebGL2/macOS doesn't have support for multisampled textures, so
  // create a render buffer object instead
  if (!_sg.features.msaa_image_bindings && img->cmn.render_target && msaa) {
    ...
    img->gl.target =
        _sg_gl_texture_target(img->cmn.type, img->cmn.sample_count);
    ...
    const bool is_compressed =
        _sg_is_compressed_pixel_format(img->cmn.pixel_format);
    for (int slot = 0; slot < img->cmn.num_slots; slot++) {
      glGenTextures(1, &img->gl.tex[slot]);
      SOKOL_ASSERT(img->gl.tex[slot]);
      _sg_gl_cache_store_texture_sampler_binding(0);
      _sg_gl_cache_bind_texture_sampler(0, img->gl.target, img->gl.tex[slot],
                                        0);
      glTexParameteri(img->gl.target, GL_TEXTURE_MAX_LEVEL,
      ...
      if (desc->data.subimage[0][0].ptr == 0) {
        ...
        if ((SG_IMAGETYPE_2D == img->cmn.type) ||
            (SG_IMAGETYPE_CUBE == img->cmn.type)) {
          glTexStorage2D(img->gl.target, img->cmn.num_mipmaps,
                         gl_internal_format, img->cmn.width, img->cmn.height);
        } else if ((SG_IMAGETYPE_3D == img->cmn.type) ||
                   (SG_IMAGETYPE_ARRAY == img->cmn.type)) {
          glTexStorage3D(img->gl.target, img->cmn.num_mipmaps,
                         gl_internal_format, img->cmn.width, img->cmn.height,
                         img->cmn.num_slices);
        }
      }
            ...
            if ((SG_IMAGETYPE_2D == img->cmn.type) ||
              ...
              if (is_compressed) {
                ...
                const GLenum gl_type =
                    _sg_gl_teximage_type(img->cmn.pixel_format);
#if defined(SOKOL_GLCORE) && !defined(__APPLE__)
                if (msaa) {
                  glTexImage2DMultisample(gl_img_target, img->cmn.sample_count,
                                          gl_internal_format, mip_width,
                                          mip_height, GL_TRUE);
                } else {
              ...
              if (is_compressed) {
                ...
                if (msaa) {
                  ...
                  glTexImage3DMultisample(gl_img_target, img->cmn.sample_count,
                                          gl_internal_format, mip_width,
                                          mip_height, mip_depth, GL_TRUE);
                } else {
...
_SOKOL_PRIVATE sg_resource_state
_sg_gl_create_sampler(_sg_sampler_t* smp, const sg_sampler_desc* desc) {
  SOKOL_ASSERT(smp && desc);
  _SG_GL_CHECK_ERROR();
  ...
  if (smp->gl.injected) {
    ...
    const GLenum gl_min_filter =
        _sg_gl_min_filter(smp->cmn.min_filter, smp->cmn.mipmap_filter);
    const GLenum gl_mag_filter = _sg_gl_mag_filter(smp->cmn.mag_filter);
    glSamplerParameteri(smp->gl.smp, GL_TEXTURE_MIN_FILTER,
                        (GLint)gl_min_filter);
    glSamplerParameteri(smp->gl.smp, GL_TEXTURE_MAG_FILTER,
                        (GLint)gl_mag_filter);
    ...
    glSamplerParameteri(smp->gl.smp, GL_TEXTURE_WRAP_S,
                        (GLint)_sg_gl_wrap(smp->cmn.wrap_u));
    glSamplerParameteri(smp->gl.smp, GL_TEXTURE_WRAP_T,
                        (GLint)_sg_gl_wrap(smp->cmn.wrap_v));
    glSamplerParameteri(smp->gl.smp, GL_TEXTURE_WRAP_R,
                        (GLint)_sg_gl_wrap(smp->cmn.wrap_w));
#if defined(SOKOL_GLCORE)
    float border[4];
    ...
    if (smp->cmn.compare != SG_COMPAREFUNC_NEVER) {
      glSamplerParameteri(smp->gl.smp, GL_TEXTURE_COMPARE_MODE,
                          GL_COMPARE_REF_TO_TEXTURE);
      glSamplerParameteri(smp->gl.smp, GL_TEXTURE_COMPARE_FUNC,
                          (GLint)_sg_gl_compare_func(smp->cmn.compare));
    } else {
    ...
    if (_sg.gl.ext_anisotropic && (smp->cmn.max_anisotropy > 1)) {
      ...
      glSamplerParameteri(smp->gl.smp, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                          max_aniso);
    }
  }
  _SG_GL_CHECK_ERROR();
...
_SOKOL_PRIVATE GLuint _sg_gl_compile_shader(sg_shader_stage stage,
                                            const char* src) {
  SOKOL_ASSERT(src);
  _SG_GL_CHECK_ERROR();
...
// NOTE: this is an out-of-range check for GLSL bindslots that's also active in
// release mode
_SOKOL_PRIVATE bool _sg_gl_ensure_glsl_bindslot_ranges(
    const sg_shader_desc* desc) {
  SOKOL_ASSERT(desc);
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_gl_create_shader(_sg_shader_t* shd, const sg_shader_desc* desc) {
  SOKOL_ASSERT(shd && desc);
  ...
  if (has_vs && has_fs) {
    GLuint gl_vs =
        _sg_gl_compile_shader(SG_SHADERSTAGE_VERTEX, desc->vertex_func.source);
    GLuint gl_fs = _sg_gl_compile_shader(SG_SHADERSTAGE_FRAGMENT,
    ...
    GLuint gl_cs = _sg_gl_compile_shader(SG_SHADERSTAGE_COMPUTE,
                                         desc->compute_func.source);
    if (!gl_cs) {
      glDeleteProgram(gl_prog);
      return SG_RESOURCESTATE_FAILED;
    }
  ...
  // resolve uniforms
  _SG_GL_CHECK_ERROR();
  for (size_t ub_index = 0; ub_index < SG_MAX_UNIFORMBLOCK_BINDSLOTS;
       ub_index++) {
    ...
    for (int u_index = 0; u_index < SG_MAX_UNIFORMBLOCK_MEMBERS; u_index++) {
      ...
      const uint32_t u_align = _sg_uniform_alignment(
          u_desc->type, u_desc->array_count, ub_desc->layout);
      const uint32_t u_size =
          _sg_uniform_size(u_desc->type, u_desc->array_count, ub_desc->layout);
  ...
  for (size_t sbuf_index = 0; sbuf_index < SG_MAX_STORAGEBUFFER_BINDSLOTS;
       sbuf_index++) {
    const sg_shader_storage_buffer* sbuf_desc =
        &desc->storage_buffers[sbuf_index];
  ...
  GLint gl_tex_slot = 0;
  for (size_t img_smp_index = 0; img_smp_index < SG_MAX_IMAGE_SAMPLER_PAIRS;
       img_smp_index++) {
    const sg_shader_image_sampler_pair* img_smp_desc =
        &desc->image_sampler_pairs[img_smp_index];
    ...
    if (gl_loc != -1) {
      ...
      _SG_LOGMSG(GL_IMAGE_SAMPLER_NAME_NOT_FOUND_IN_SHADER,
                 img_smp_desc->glsl_name);
    }
  }
...
_SOKOL_PRIVATE sg_resource_state
_sg_gl_create_pipeline(_sg_pipeline_t* pip,
                       _sg_shader_t* shd,
                       const sg_pipeline_desc* desc) {
  ...
  for (int attr_index = 0; attr_index < SG_MAX_VERTEX_ATTRIBUTES;
       attr_index++) {
    pip->gl.attrs[attr_index].vb_index = -1;
  }
  for (int attr_index = 0; attr_index < _sg.limits.max_vertex_attrs;
       attr_index++) {
    ...
    SOKOL_ASSERT(a_state->buffer_index < SG_MAX_VERTEXBUFFER_BINDSLOTS);
    const sg_vertex_buffer_layout_state* l_state =
        &desc->layout.buffers[a_state->buffer_index];
    ...
    if (!_sg_strempty(&shd->gl.attrs[attr_index].name)) {
      attr_loc = glGetAttribLocation(
          pip->shader->gl.prog, _sg_strptr(&shd->gl.attrs[attr_index].name));
    }
    if (attr_loc != -1) {
      ...
      _SG_LOGMSG(GL_VERTEX_ATTRIBUTE_NOT_FOUND_IN_SHADER,
                 _sg_strptr(&shd->gl.attrs[attr_index].name));
    }
  }
  return SG_RESOURCESTATE_VALID;
}
...
_SOKOL_PRIVATE void _sg_gl_fb_attach_texture(
    const _sg_gl_attachment_t* gl_att,
    const _sg_attachment_common_t* cmn_att,
    GLenum gl_att_type) {
  ...
  switch (img->cmn.type) {
    case SG_IMAGETYPE_2D:
      glFramebufferTexture2D(GL_FRAMEBUFFER, gl_att_type, gl_target, gl_tex,
                             mip_level);
      break;
    case SG_IMAGETYPE_CUBE:
      glFramebufferTexture2D(GL_FRAMEBUFFER, gl_att_type,
                             _sg_gl_cubeface_target(slice), gl_tex, mip_level);
      break;
    default:
      glFramebufferTextureLayer(GL_FRAMEBUFFER, gl_att_type, gl_tex, mip_level,
                                slice);
      break;
  }
}

_SOKOL_PRIVATE GLenum
_sg_gl_depth_stencil_attachment_type(const _sg_gl_attachment_t* ds_att) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_gl_create_attachments(_sg_attachments_t* atts,
                          _sg_image_t** color_images,
                          _sg_image_t** resolve_images,
  ...
  if (ds_desc->image.id != SG_INVALID_ID) {
    ...
    SOKOL_ASSERT(
        _sg_is_valid_rendertarget_depth_format(ds_image->cmn.pixel_format));
  ...
  for (int i = 0; i < atts->cmn.num_colors; i++) {
    ...
    if (gl_msaa_render_buffer) {
      glFramebufferRenderbuffer(GL_FRAMEBUFFER,
                                ...
                                GL_RENDERBUFFER, gl_msaa_render_buffer);
    } else {
      const GLenum gl_att_type = (GLenum)(GL_COLOR_ATTACHMENT0 + i);
      _sg_gl_fb_attach_texture(&atts->gl.colors[i], &atts->cmn.colors[i],
                               gl_att_type);
    }
  }
  ...
  if (atts->gl.depth_stencil.image) {
    const GLenum gl_att =
        _sg_gl_depth_stencil_attachment_type(&atts->gl.depth_stencil);
    const _sg_image_t* ds_img = atts->gl.depth_stencil.image;
    const GLuint gl_msaa_render_buffer = ds_img->gl.msaa_render_buffer;
    if (gl_msaa_render_buffer) {
      glFramebufferRenderbuffer(GL_FRAMEBUFFER, gl_att, GL_RENDERBUFFER,
                                gl_msaa_render_buffer);
    } else {
      const GLenum gl_att_type =
          _sg_gl_depth_stencil_attachment_type(&atts->gl.depth_stencil);
      _sg_gl_fb_attach_texture(&atts->gl.depth_stencil,
                               &atts->cmn.depth_stencil, gl_att_type);
    }
  }
  ...
  for (int i = 0; i < atts->cmn.num_colors; i++) {
    ...
    if (gl_resolve_att->image) {
      ...
      _sg_gl_fb_attach_texture(gl_resolve_att, cmn_resolve_att,
...
_SOKOL_PRIVATE _sg_image_t* _sg_gl_attachments_color_image(
    const _sg_attachments_t* atts,
    int index) {
  SOKOL_ASSERT(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t* _sg_gl_attachments_resolve_image(
    const _sg_attachments_t* atts,
    int index) {
  SOKOL_ASSERT(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t* _sg_gl_attachments_ds_image(
    const _sg_attachments_t* atts) {
  SOKOL_ASSERT(atts);
...
// default pass
  ...
  if ((atts == 0) || (atts->gl.depth_stencil.image)) {
    if (clear_depth && clear_stencil) {
      glClearBufferfi(GL_DEPTH_STENCIL, 0, action->depth.clear_value,
                      action->stencil.clear_value);
    } else if (clear_depth) {
...
_SOKOL_PRIVATE void _sg_gl_end_pass(void) {
  ...
  if (_sg.cur_pass.atts) {
    ...
    for (int i = 0; i < num_color_atts; i++) {
      ...
      if (atts->gl.msaa_resolve_framebuffer[i] != 0) {
        ...
        glBindFramebuffer(GL_DRAW_FRAMEBUFFER,
                          atts->gl.msaa_resolve_framebuffer[i]);
...
#if defined(SOKOL_GLES3)
    // need to restore framebuffer binding before invalidate if the MSAA resolve
    // had changed the binding
    if (fb_draw_bound) {
      glBindFramebuffer(GL_FRAMEBUFFER, atts->gl.fb);
    }
    ...
    if ((_sg.gl.depth_store_action == SG_STOREACTION_DONTCARE) &&
        (_sg.cur_pass.atts->cmn.depth_stencil.image_id.id != SG_INVALID_ID)) {
...
_SOKOL_PRIVATE void _sg_gl_apply_viewport(int x,
                                          int y,
                                          int w,
                                          int h,
                                          bool origin_top_left) {
...
_SOKOL_PRIVATE void _sg_gl_apply_scissor_rect(int x,
                                              int y,
                                              int w,
                                              int h,
                                              bool origin_top_left) {
...
_SOKOL_PRIVATE void _sg_gl_apply_pipeline(_sg_pipeline_t* pip) {
  ...
  if ((_sg.gl.cache.cur_pipeline != pip) ||
    ...
    _sg.gl.cache.cur_primitive_type =
        _sg_gl_primitive_type(pip->gl.primitive_type);
    ...
    // update depth state
    {
      ...
      if (!_sg_fequal(state_ds->bias, cache_ds->bias, 0.000001f) ||
          !_sg_fequal(state_ds->bias_slope_scale, cache_ds->bias_slope_scale,
                      0.000001f)) {
    ...
    // update stencil state
    {
      ...
      for (int i = 0; i < 2; i++) {
        const sg_stencil_face_state* state_sfs =
            (i == 0) ? &state_ss->front : &state_ss->back;
        sg_stencil_face_state* cache_sfs =
        ...
        if ((state_sfs->fail_op != cache_sfs->fail_op) ||
          ...
          glStencilOpSeparate(gl_face, _sg_gl_stencil_op(state_sfs->fail_op),
    ...
    if (pip->cmn.color_count > 0) {
      ...
      if ((state_bs->op_rgb != cache_bs->op_rgb) ||
        ...
        glBlendEquationSeparate(_sg_gl_blend_op(state_bs->op_rgb),
                                _sg_gl_blend_op(state_bs->op_alpha));
        _sg_stats_add(gl.num_render_state, 1);
      }
...
#ifdef SOKOL_GLCORE
          glColorMaski(i, (cm & SG_COLORMASK_R) != 0,
                       (cm & SG_COLORMASK_G) != 0, (cm & SG_COLORMASK_B) != 0,
                       (cm & SG_COLORMASK_A) != 0);
#else
          if (0 == i) {
            glColorMask((cm & SG_COLORMASK_R) != 0, (cm & SG_COLORMASK_G) != 0,
                        (cm & SG_COLORMASK_B) != 0, (cm & SG_COLORMASK_A) != 0);
          }
#endif
          _sg_stats_add(gl.num_render_state, 1);
        }
      ...
      if (!_sg_fequal(pip->cmn.blend_color.r, _sg.gl.cache.blend_color.r,
                      0.0001f) ||
          !_sg_fequal(pip->cmn.blend_color.g, _sg.gl.cache.blend_color.g,
                      0.0001f) ||
          !_sg_fequal(pip->cmn.blend_color.b, _sg.gl.cache.blend_color.b,
                      0.0001f) ||
          !_sg_fequal(pip->cmn.blend_color.a, _sg.gl.cache.blend_color.a,
                      0.0001f)) {
    ...
    if (pip->gl.cull_mode != _sg.gl.cache.cull_mode) {
      ...
      if (SG_CULLMODE_NONE == pip->gl.cull_mode) {
        ...
        GLenum gl_mode =
            (SG_CULLMODE_FRONT == pip->gl.cull_mode) ? GL_FRONT : GL_BACK;
        glCullFace(gl_mode);
        _sg_stats_add(gl.num_render_state, 2);
      }
    }
    if (pip->gl.face_winding != _sg.gl.cache.face_winding) {
      ...
      GLenum gl_winding =
          (SG_FACEWINDING_CW == pip->gl.face_winding) ? GL_CW : GL_CCW;
      glFrontFace(gl_winding);
      _sg_stats_add(gl.num_render_state, 1);
    }
    if (pip->gl.alpha_to_coverage_enabled !=
        _sg.gl.cache.alpha_to_coverage_enabled) {
      _sg.gl.cache.alpha_to_coverage_enabled =
...
_SOKOL_PRIVATE void _sg_gl_handle_memory_barriers(const _sg_shader_t* shd,
                                                  const _sg_bindings_t* bnd) {
  if (!_sg.features.compute) {
    return;
  }
  ...
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
    ...
    // if this binding is going to be written by the GPU set the buffer to
    // 'gpu_dirty'
...
// take care of storage buffer memory barriers
  ...
  for (size_t img_smp_index = 0; img_smp_index < SG_MAX_IMAGE_SAMPLER_PAIRS;
       img_smp_index++) {
    const _sg_shader_image_sampler_t* img_smp =
        &shd->cmn.image_samplers[img_smp_index];
  ...
  // vertex attributes
  for (GLuint attr_index = 0; attr_index < (GLuint)_sg.limits.max_vertex_attrs;
       attr_index++) {
    ...
    if (attr->vb_index >= 0) {
      ...
      if ((gl_vb != cache_attr->gl_vbuf) ||
        ...
        if (attr->base_type == SG_SHADERATTRBASETYPE_FLOAT) {
          ...
          glVertexAttribIPointer(attr_index, attr->size, attr->type,
                                 attr->stride,
                                 (const GLvoid*)(GLintptr)vb_offset);
        }
...
_SOKOL_PRIVATE void _sg_gl_draw(int base_element,
                                int num_elements,
                                int num_instances) {
  SOKOL_ASSERT(_sg.gl.cache.cur_pipeline);
  const GLenum i_type = _sg.gl.cache.cur_index_type;
  const GLenum p_type = _sg.gl.cache.cur_primitive_type;
  const bool use_instanced_draw =
      (num_instances > 1) ||
      (_sg.gl.cache.cur_pipeline->cmn.use_instanced_draw);
  if (0 != i_type) {
    // indexed rendering
...
_SOKOL_PRIVATE void _sg_gl_dispatch(int num_groups_x,
                                    int num_groups_y,
                                    int num_groups_z) {
#if defined(_SOKOL_GL_HAS_COMPUTE)
  if (!_sg.features.compute) {
...
_SOKOL_PRIVATE void _sg_gl_update_buffer(_sg_buffer_t* buf,
                                         const sg_range* data) {
  SOKOL_ASSERT(buf && data && data->ptr && (data->size > 0));
...
_SOKOL_PRIVATE void _sg_gl_append_buffer(_sg_buffer_t* buf,
                                         const sg_range* data,
                                         bool new_frame) {
  SOKOL_ASSERT(buf && data && data->ptr && (data->size > 0));
  if (new_frame) {
...
_SOKOL_PRIVATE void _sg_gl_update_image(_sg_image_t* img,
                                        const sg_image_data* data) {
  SOKOL_ASSERT(img && data);
  ...
  _sg_gl_cache_store_texture_sampler_binding(0);
  _sg_gl_cache_bind_texture_sampler(0, img->gl.target,
...
// NOTE: This needs to be a macro since we can't use the polymorphism in C. It's
// called on many kinds of resources. NOTE: Based on microsoft docs, it's fine
// to call this with pData=NULL if DataSize is also zero.
#if defined(__cplusplus)
#define _sg_d3d11_SetPrivateData(self, guid, DataSize, pData) \
  (self)->SetPrivateData(guid, DataSize, pData)
#else
#define _sg_d3d11_SetPrivateData(self, guid, DataSize, pData) \
  (self)->lpVtbl->SetPrivateData(self, guid, DataSize, pData)
#endif
...
static const GUID _sg_d3d11_WKPDID_D3DDebugObjectName = {
    0x429b8c22,
    0x9188,
    0x4b0c,
...
#define _sg_d3d11_setlabel(self, label)                             \
  _sg_d3d11_SetPrivateData(                                         \
      self, _sg_win32_refguid(_sg_d3d11_WKPDID_D3DDebugObjectName), \
      label ? (UINT)strlen(label) : 0, label)
#else
#define _sg_d3d11_setlabel(self, label)
#endif

//-- D3D11 C/C++ wrappers ------------------------------------------------------
static inline HRESULT _sg_d3d11_CheckFormatSupport(ID3D11Device* self,
                                                   DXGI_FORMAT Format,
                                                   UINT* pFormatSupport) {
#if defined(__cplusplus)
...
static inline void _sg_d3d11_OMSetRenderTargets(
...
static inline void _sg_d3d11_RSSetState(
    ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_OMSetDepthStencilState(
    ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_OMSetBlendState(ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_IASetVertexBuffers(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumBuffers,
...
static inline void _sg_d3d11_IASetIndexBuffer(ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_IASetInputLayout(ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_VSSetShader(
    ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_PSSetShader(
    ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_CSSetShader(
    ID3D11DeviceContext* self,
...
#else
  self->lpVtbl->CSSetShader(self, pComputeShader, ppClassInstances,
                            NumClassInstances);
#endif
}

static inline void _sg_d3d11_VSSetConstantBuffers(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumBuffers,
...
static inline void _sg_d3d11_PSSetConstantBuffers(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumBuffers,
    ID3D11Buffer* const* ppConstantBuffers) {
...
static inline void _sg_d3d11_CSSetConstantBuffers(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumBuffers,
    ID3D11Buffer* const* ppConstantBuffers) {
...
#else
  self->lpVtbl->CSSetConstantBuffers(self, StartSlot, NumBuffers,
                                     ppConstantBuffers);
#endif
}

static inline void _sg_d3d11_VSSetShaderResources(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumViews,
...
static inline void _sg_d3d11_PSSetShaderResources(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumViews,
    ID3D11ShaderResourceView* const* ppShaderResourceViews) {
...
static inline void _sg_d3d11_CSSetShaderResources(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumViews,
    ID3D11ShaderResourceView* const* ppShaderResourceViews) {
...
#else
  self->lpVtbl->CSSetShaderResources(self, StartSlot, NumViews,
                                     ppShaderResourceViews);
#endif
}

static inline void _sg_d3d11_VSSetSamplers(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumSamplers,
...
static inline void _sg_d3d11_PSSetSamplers(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumSamplers,
    ID3D11SamplerState* const* ppSamplers) {
...
static inline void _sg_d3d11_CSSetSamplers(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumSamplers,
    ID3D11SamplerState* const* ppSamplers) {
...
static inline void _sg_d3d11_CSSetUnorderedAccessViews(
    ID3D11DeviceContext* self,
    UINT StartSlot,
    UINT NumUAVs,
...
#if defined(__cplusplus)
  self->CSSetUnorderedAccessViews(StartSlot, NumUAVs, ppUnorderedAccessViews,
                                  pUAVInitialCounts);
#else
  self->lpVtbl->CSSetUnorderedAccessViews(
      self, StartSlot, NumUAVs, ppUnorderedAccessViews, pUAVInitialCounts);
#endif
}

static inline HRESULT _sg_d3d11_CreateBuffer(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreateTexture2D(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreateShaderResourceView(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreateUnorderedAccessView(
    ID3D11Device* self,
    ID3D11Resource* pResource,
    const D3D11_UNORDERED_ACCESS_VIEW_DESC* pDesc,
    ID3D11UnorderedAccessView** ppUAVView) {
#if defined(__cplusplus)
  return self->CreateUnorderedAccessView(pResource, pDesc, ppUAVView);
#else
  return self->lpVtbl->CreateUnorderedAccessView(self, pResource, pDesc,
                                                 ppUAVView);
#endif
}

static inline void _sg_d3d11_GetResource(ID3D11View* self,
...
static inline HRESULT _sg_d3d11_CreateTexture3D(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreateSamplerState(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreateVertexShader(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreatePixelShader(
    ID3D11Device* self,
    const void* pShaderBytecode,
    SIZE_T BytecodeLength,
    ID3D11ClassLinkage* pClassLinkage,
...
static inline HRESULT _sg_d3d11_CreateComputeShader(
    ID3D11Device* self,
    const void* pShaderBytecode,
    SIZE_T BytecodeLength,
    ID3D11ClassLinkage* pClassLinkage,
...
#if defined(__cplusplus)
  return self->CreateComputeShader(pShaderBytecode, BytecodeLength,
                                   pClassLinkage, ppComputeShader);
#else
  return self->lpVtbl->CreateComputeShader(
      self, pShaderBytecode, BytecodeLength, pClassLinkage, ppComputeShader);
#endif
}

static inline HRESULT _sg_d3d11_CreateInputLayout(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreateRasterizerState(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreateDepthStencilState(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreateBlendState(
    ID3D11Device* self,
...
static inline HRESULT _sg_d3d11_CreateRenderTargetView(
    ID3D11Device* self,
    ID3D11Resource* pResource,
...
static inline HRESULT _sg_d3d11_CreateDepthStencilView(
    ID3D11Device* self,
    ID3D11Resource* pResource,
...
static inline void _sg_d3d11_RSSetViewports(ID3D11DeviceContext* self,
                                            UINT NumViewports,
...
static inline void _sg_d3d11_RSSetScissorRects(ID3D11DeviceContext* self,
                                               UINT NumRects,
...
static inline void _sg_d3d11_ClearRenderTargetView(
    ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_ClearDepthStencilView(
    ID3D11DeviceContext* self,
    ID3D11DepthStencilView* pDepthStencilView,
    UINT ClearFlags,
    FLOAT Depth,
...
static inline void _sg_d3d11_ResolveSubresource(ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_IASetPrimitiveTopology(
    ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_UpdateSubresource(ID3D11DeviceContext* self,
                                               ID3D11Resource* pDstResource,
                                               UINT DstSubresource,
...
static inline void _sg_d3d11_DrawIndexed(ID3D11DeviceContext* self,
                                         UINT IndexCount,
...
static inline void _sg_d3d11_DrawIndexedInstanced(ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_Draw(ID3D11DeviceContext* self,
                                  UINT VertexCount,
...
static inline void _sg_d3d11_DrawInstanced(ID3D11DeviceContext* self,
...
static inline void _sg_d3d11_Dispatch(ID3D11DeviceContext* self,
...
#else
  self->lpVtbl->Dispatch(self, ThreadGroupCountX, ThreadGroupCountY,
                         ThreadGroupCountZ);
#endif
}

static inline HRESULT _sg_d3d11_Map(ID3D11DeviceContext* self,
                                    ID3D11Resource* pResource,
                                    UINT Subresource,
                                    D3D11_MAP MapType,
                                    UINT MapFlags,
...
static inline void _sg_d3d11_Unmap(ID3D11DeviceContext* self,
                                   ID3D11Resource* pResource,
                                   UINT Subresource) {
#if defined(__cplusplus)
...
_SOKOL_PRIVATE D3D11_USAGE _sg_d3d11_buffer_usage(sg_usage usg,
                                                  sg_buffer_type type) {
  switch (usg) {
    case SG_USAGE_IMMUTABLE:
...
_SOKOL_PRIVATE UINT _sg_d3d11_buffer_bind_flags(sg_usage usg,
                                                sg_buffer_type t) {
  switch (t) {
    case SG_BUFFERTYPE_VERTEXBUFFER:
...
_SOKOL_PRIVATE D3D11_PRIMITIVE_TOPOLOGY
_sg_d3d11_primitive_topology(sg_primitive_type prim_type) {
  switch (prim_type) {
    case SG_PRIMITIVETYPE_POINTS:
...
_SOKOL_PRIVATE D3D11_FILTER _sg_d3d11_filter(sg_filter min_f,
                                             sg_filter mag_f,
                                             sg_filter mipmap_f,
                                             bool comparison,
                                             uint32_t max_anisotropy) {
...
_SOKOL_PRIVATE D3D11_INPUT_CLASSIFICATION
_sg_d3d11_input_classification(sg_vertex_step step) {
  switch (step) {
...
_SOKOL_PRIVATE UINT _sg_d3d11_dxgi_fmt_caps(DXGI_FORMAT dxgi_fmt) {
  ...
  if (dxgi_fmt != DXGI_FORMAT_UNKNOWN) {
    HRESULT hr =
        _sg_d3d11_CheckFormatSupport(_sg.d3d11.dev, dxgi_fmt, &dxgi_fmt_caps);
...
_SOKOL_PRIVATE void _sg_d3d11_init_caps(void) {
  ...
  // see:
  // https://docs.microsoft.com/en-us/windows/win32/api/d3d11/ne-d3d11-d3d11_format_support
  for (int fmt = (SG_PIXELFORMAT_NONE + 1); fmt < _SG_PIXELFORMAT_NUM; fmt++) {
    const UINT srv_dxgi_fmt_caps = _sg_d3d11_dxgi_fmt_caps(
        _sg_d3d11_srv_pixel_format((sg_pixel_format)fmt));
    const UINT rtv_dxgi_fmt_caps = _sg_d3d11_dxgi_fmt_caps(
        _sg_d3d11_rtv_pixel_format((sg_pixel_format)fmt));
    const UINT dsv_dxgi_fmt_caps = _sg_d3d11_dxgi_fmt_caps(
        _sg_d3d11_dsv_pixel_format((sg_pixel_format)fmt));
    _sg_pixelformat_info_t* info = &_sg.formats[fmt];
    const bool render =
        0 != (rtv_dxgi_fmt_caps & D3D11_FORMAT_SUPPORT_RENDER_TARGET);
    const bool depth =
        0 != (dsv_dxgi_fmt_caps & D3D11_FORMAT_SUPPORT_DEPTH_STENCIL);
    info->sample = 0 != (srv_dxgi_fmt_caps & D3D11_FORMAT_SUPPORT_TEXTURE2D);
    info->filter =
        0 != (srv_dxgi_fmt_caps & D3D11_FORMAT_SUPPORT_SHADER_SAMPLE);
    info->render = render || depth;
    if (depth) {
      info->blend = 0 != (dsv_dxgi_fmt_caps & D3D11_FORMAT_SUPPORT_BLENDABLE);
      info->msaa = 0 != (dsv_dxgi_fmt_caps &
                         D3D11_FORMAT_SUPPORT_MULTISAMPLE_RENDERTARGET);
    } else {
      info->blend = 0 != (rtv_dxgi_fmt_caps & D3D11_FORMAT_SUPPORT_BLENDABLE);
      info->msaa = 0 != (rtv_dxgi_fmt_caps &
                         D3D11_FORMAT_SUPPORT_MULTISAMPLE_RENDERTARGET);
    }
    info->depth = depth;
  }
...
_SOKOL_PRIVATE void _sg_d3d11_clear_state(void) {
  // clear all the device context state, so that resource refs don't keep stuck
  // in the d3d device context
...
_SOKOL_PRIVATE sg_resource_state
_sg_d3d11_create_buffer(_sg_buffer_t* buf, const sg_buffer_desc* desc) {
  SOKOL_ASSERT(buf && desc);
  ...
  if (injected) {
    ...
    d3d11_buf_desc.Usage =
        _sg_d3d11_buffer_usage(buf->cmn.usage, buf->cmn.type);
    d3d11_buf_desc.BindFlags =
        _sg_d3d11_buffer_bind_flags(buf->cmn.usage, buf->cmn.type);
    ...
    if (buf->cmn.usage == SG_USAGE_IMMUTABLE) {
      ...
      if (desc->data.ptr) {
        ...
        init_data.pSysMem =
            (const void*)_sg_malloc_clear((size_t)buf->cmn.size);
      }
      init_data_ptr = &init_data;
    }
    HRESULT hr = _sg_d3d11_CreateBuffer(_sg.d3d11.dev, &d3d11_buf_desc,
    ...
    if (buf->cmn.type == SG_BUFFERTYPE_STORAGEBUFFER) {
      ...
      hr = _sg_d3d11_CreateShaderResourceView(_sg.d3d11.dev,
                                              (ID3D11Resource*)buf->d3d11.buf,
                                              &d3d11_srv_desc, &buf->d3d11.srv);
      ...
      if (buf->cmn.usage == SG_USAGE_IMMUTABLE) {
        ...
        hr = _sg_d3d11_CreateUnorderedAccessView(
            _sg.d3d11.dev, (ID3D11Resource*)buf->d3d11.buf, &d3d11_uav_desc,
            &buf->d3d11.uav);
...
_SOKOL_PRIVATE void _sg_d3d11_fill_subres_data(const _sg_image_t* img,
  ...
  const int num_slices =
      (img->cmn.type == SG_IMAGETYPE_ARRAY) ? img->cmn.num_slices : 1;
  int subres_index = 0;
  for (int face_index = 0; face_index < num_faces; face_index++) {
    for (int slice_index = 0; slice_index < num_slices; slice_index++) {
      for (int mip_index = 0; mip_index < img->cmn.num_mipmaps;
           mip_index++, subres_index++) {
        SOKOL_ASSERT(subres_index <
                     (SG_MAX_MIPMAPS * SG_MAX_TEXTUREARRAY_LAYERS));
        ...
        subres_data->SysMemPitch =
            (UINT)_sg_row_pitch(img->cmn.pixel_format, mip_width, 1);
        if (img->cmn.type == SG_IMAGETYPE_3D) {
          // FIXME? const int mip_depth = _sg_miplevel_dim(img->depth,
          // mip_index);
          subres_data->SysMemSlicePitch = (UINT)_sg_surface_pitch(
...
_SOKOL_PRIVATE sg_resource_state
_sg_d3d11_create_image(_sg_image_t* img, const sg_image_desc* desc) {
  SOKOL_ASSERT(img && desc);
  SOKOL_ASSERT((0 == img->d3d11.tex2d) && (0 == img->d3d11.tex3d) &&
               (0 == img->d3d11.res) && (0 == img->d3d11.srv));
  HRESULT hr;
  ...
  if (!injected && (img->cmn.usage == SG_USAGE_IMMUTABLE) &&
  ...
  if (img->cmn.type != SG_IMAGETYPE_3D) {
    ...
    if (injected) {
      ...
      if (img->cmn.render_target) {
        ...
        d3d11_tex_desc.CPUAccessFlags =
            _sg_d3d11_cpu_access_flags(img->cmn.usage);
      }
      ...
      d3d11_tex_desc.MiscFlags = (img->cmn.type == SG_IMAGETYPE_CUBE)
                                     ? D3D11_RESOURCE_MISC_TEXTURECUBE
                                     : 0;

      hr = _sg_d3d11_CreateTexture2D(_sg.d3d11.dev, &d3d11_tex_desc, init_data,
                                     &img->d3d11.tex2d);
      ...
      switch (img->cmn.type) {
        case SG_IMAGETYPE_2D:
          d3d11_srv_desc.ViewDimension = msaa ? D3D11_SRV_DIMENSION_TEXTURE2DMS
        ...
        case SG_IMAGETYPE_CUBE:
          ...
          break;
        case SG_IMAGETYPE_ARRAY:
          d3d11_srv_desc.ViewDimension =
              msaa ? D3D11_SRV_DIMENSION_TEXTURE2DMSARRAY
    ...
    // 3D texture - same procedure, first check if injected, than create
    // non-injected
    if (injected) {
      ...
      hr = _sg_d3d11_CreateTexture3D(_sg.d3d11.dev, &d3d11_tex_desc, init_data,
                                     &img->d3d11.tex3d);
      ...
      if (!msaa) {
        D3D11_SHADER_RESOURCE_VIEW_DESC d3d11_srv_desc;
        _sg_clear(&d3d11_srv_desc, sizeof(d3d11_srv_desc));
        d3d11_srv_desc.Format =
            _sg_d3d11_srv_pixel_format(img->cmn.pixel_format);
        d3d11_srv_desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE3D;
        d3d11_srv_desc.Texture3D.MipLevels = (UINT)img->cmn.num_mipmaps;
        hr = _sg_d3d11_CreateShaderResourceView(
            _sg.d3d11.dev, (ID3D11Resource*)img->d3d11.tex3d, &d3d11_srv_desc,
            &img->d3d11.srv);
        if (!(SUCCEEDED(hr) && img->d3d11.srv)) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_d3d11_create_sampler(_sg_sampler_t* smp, const sg_sampler_desc* desc) {
  SOKOL_ASSERT(smp && desc);
  ...
  if (injected) {
    ...
    d3d11_smp_desc.Filter = _sg_d3d11_filter(
        desc->min_filter, desc->mag_filter, desc->mipmap_filter,
    ...
    HRESULT hr = _sg_d3d11_CreateSamplerState(_sg.d3d11.dev, &d3d11_smp_desc,
                                              &smp->d3d11.smp);
...
_SOKOL_PRIVATE bool _sg_d3d11_load_d3dcompiler_dll(void) {
  if ((0 == _sg.d3d11.d3dcompiler_dll) &&
      !_sg.d3d11.d3dcompiler_dll_load_failed) {
    ...
    _sg.d3d11.D3DCompile_func = (pD3DCompile)(void*)GetProcAddress(
        _sg.d3d11.d3dcompiler_dll, "D3DCompile");
    SOKOL_ASSERT(_sg.d3d11.D3DCompile_func);
  }
  return 0 != _sg.d3d11.d3dcompiler_dll;
}

_SOKOL_PRIVATE ID3DBlob* _sg_d3d11_compile_shader(
    const sg_shader_function* shd_func) {
  ...
  if (errors_or_warnings) {
    ...
    _SG_LOGMSG(D3D11_SHADER_COMPILATION_OUTPUT,
               (LPCSTR)_sg_d3d11_GetBufferPointer(errors_or_warnings));
    _sg_d3d11_Release(errors_or_warnings);
...
// NOTE: this is an out-of-range check for HLSL bindslots that's also active in
// release mode
_SOKOL_PRIVATE bool _sg_d3d11_ensure_hlsl_bindslot_ranges(
    const sg_shader_desc* desc) {
  SOKOL_ASSERT(desc);
  for (size_t i = 0; i < SG_MAX_UNIFORMBLOCK_BINDSLOTS; i++) {
    if (desc->uniform_blocks[i].hlsl_register_b_n >=
        _SG_D3D11_MAX_STAGE_UB_BINDINGS) {
      _SG_ERROR(D3D11_UNIFORMBLOCK_HLSL_REGISTER_B_OUT_OF_RANGE);
      return false;
    }
  }
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
    if (desc->storage_buffers[i].hlsl_register_t_n >=
        _SG_D3D11_MAX_STAGE_SRV_BINDINGS) {
      _SG_ERROR(D3D11_STORAGEBUFFER_HLSL_REGISTER_T_OUT_OF_RANGE);
      return false;
    }
    if (desc->storage_buffers[i].hlsl_register_u_n >=
  ...
  for (size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
    if (desc->samplers[i].hlsl_register_s_n >=
        _SG_D3D11_MAX_STAGE_SMP_BINDINGS) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_d3d11_create_shader(_sg_shader_t* shd, const sg_shader_desc* desc) {
  SOKOL_ASSERT(shd && desc);
  SOKOL_ASSERT(!shd->d3d11.vs && !shd->d3d11.fs && !shd->d3d11.cs &&
               !shd->d3d11.vs_blob);
  HRESULT hr;
  ...
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
    shd->d3d11.sbuf_register_t_n[i] =
        desc->storage_buffers[i].hlsl_register_t_n;
    shd->d3d11.sbuf_register_u_n[i] =
        desc->storage_buffers[i].hlsl_register_u_n;
  }
  for (size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
  ...
  const bool has_vs =
      desc->vertex_func.bytecode.ptr || desc->vertex_func.source;
  const bool has_fs =
      desc->fragment_func.bytecode.ptr || desc->fragment_func.source;
  const bool has_cs =
      desc->compute_func.bytecode.ptr || desc->compute_func.source;
  bool vs_valid = false;
  bool fs_valid = false;
  bool cs_valid = false;
  if (has_vs) {
    ...
    if (vs_ptr && (vs_length > 0)) {
      hr = _sg_d3d11_CreateVertexShader(_sg.d3d11.dev, vs_ptr, vs_length, NULL,
                                        &shd->d3d11.vs);
      vs_valid = SUCCEEDED(hr) && shd->d3d11.vs;
    }
    // set label, and need to store a copy of the vertex shader blob for the
    // pipeline creation
    if (vs_valid) {
  ...
  if (has_fs) {
    ...
    if (fs_ptr && (fs_length > 0)) {
      hr = _sg_d3d11_CreatePixelShader(_sg.d3d11.dev, fs_ptr, fs_length, NULL,
                                       &shd->d3d11.fs);
  ...
  if (has_cs) {
    ...
    if (cs_ptr && (cs_length > 0)) {
      hr = _sg_d3d11_CreateComputeShader(_sg.d3d11.dev, cs_ptr, cs_length, NULL,
                                         &shd->d3d11.cs);
...
_SOKOL_PRIVATE sg_resource_state
_sg_d3d11_create_pipeline(_sg_pipeline_t* pip,
                          _sg_shader_t* shd,
                          const sg_pipeline_desc* desc) {
  SOKOL_ASSERT(pip && shd && desc);
  SOKOL_ASSERT(desc->shader.id == shd->slot.id);
  ...
  SOKOL_ASSERT(!pip->d3d11.il && !pip->d3d11.rs && !pip->d3d11.dss &&
               !pip->d3d11.bs);
  ...
  for (; attr_index < SG_MAX_VERTEX_ATTRIBUTES; attr_index++) {
    ...
    d3d11_comp->SemanticName =
        _sg_strptr(&shd->d3d11.attrs[attr_index].sem_name);
  ...
  for (size_t layout_index = 0; layout_index < SG_MAX_VERTEXBUFFER_BINDSLOTS;
       layout_index++) {
  ...
  if (attr_index > 0) {
    hr = _sg_d3d11_CreateInputLayout(
        _sg.d3d11.dev,
  ...
  dss_desc.FrontFace.StencilDepthFailOp =
      _sg_d3d11_stencil_op(sf->depth_fail_op);
  ...
  dss_desc.BackFace.StencilDepthFailOp =
      _sg_d3d11_stencil_op(sb->depth_fail_op);
  ...
  hr = _sg_d3d11_CreateDepthStencilState(_sg.d3d11.dev, &dss_desc,
                                         &pip->d3d11.dss);
  ...
  bs_desc.IndependentBlendEnable = TRUE;
  {
    ...
    for (i = 0; i < (size_t)desc->color_count; i++) {
      ...
      dst->RenderTargetWriteMask =
          _sg_d3d11_color_write_mask(desc->colors[i].write_mask);
    }
    for (; i < 8; i++) {
      D3D11_RENDER_TARGET_BLEND_DESC* dst = &bs_desc.RenderTarget[i];
...
_SOKOL_PRIVATE sg_resource_state
_sg_d3d11_create_attachments(_sg_attachments_t* atts,
                             _sg_image_t** color_images,
                             _sg_image_t** resolve_images,
                             _sg_image_t* ds_img,
                             const sg_attachments_desc* desc) {
  ...
  for (size_t i = 0; i < (size_t)atts->cmn.num_colors; i++) {
    ...
    d3d11_rtv_desc.Format =
        _sg_d3d11_rtv_pixel_format(color_img->cmn.pixel_format);
    ...
    } else if ((color_img->cmn.type == SG_IMAGETYPE_CUBE) ||
               (color_img->cmn.type == SG_IMAGETYPE_ARRAY)) {
      if (msaa) {
        ...
        d3d11_rtv_desc.Texture2DArray.FirstArraySlice =
            (UINT)cmn_color_att->slice;
    ...
    HRESULT hr = _sg_d3d11_CreateRenderTargetView(
        _sg.d3d11.dev, color_img->d3d11.res, &d3d11_rtv_desc,
        &atts->d3d11.colors[i].view.rtv);
  ...
  if (ds_desc->image.id != SG_INVALID_ID) {
    ...
    d3d11_dsv_desc.Format =
        _sg_d3d11_dsv_pixel_format(ds_img->cmn.pixel_format);
    ...
    } else if ((ds_img->cmn.type == SG_IMAGETYPE_CUBE) ||
               (ds_img->cmn.type == SG_IMAGETYPE_ARRAY)) {
      if (msaa) {
        d3d11_dsv_desc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2DMSARRAY;
        d3d11_dsv_desc.Texture2DMSArray.FirstArraySlice =
            (UINT)cmn_ds_att->slice;
    ...
    HRESULT hr = _sg_d3d11_CreateDepthStencilView(
        _sg.d3d11.dev, ds_img->d3d11.res, &d3d11_dsv_desc,
        &atts->d3d11.depth_stencil.view.dsv);
...
_SOKOL_PRIVATE _sg_image_t* _sg_d3d11_attachments_color_image(
    const _sg_attachments_t* atts,
    int index) {
  SOKOL_ASSERT(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t* _sg_d3d11_attachments_ds_image(
    const _sg_attachments_t* atts) {
  SOKOL_ASSERT(atts);
...
_SOKOL_PRIVATE void _sg_d3d11_begin_pass(const sg_pass* pass) {
  ...
  if (atts) {
    ...
    _sg.d3d11.cur_pass.render_view =
        (ID3D11RenderTargetView*)swapchain->d3d11.render_view;
    _sg.d3d11.cur_pass.resolve_view =
        (ID3D11RenderTargetView*)swapchain->d3d11.resolve_view;
  }
  // apply the render-target- and depth-stencil-views
  _sg_d3d11_OMSetRenderTargets(_sg.d3d11.ctx, SG_MAX_COLOR_ATTACHMENTS, rtvs,
                               dsv);
  ...
  for (size_t i = 0; i < (size_t)num_rtvs; i++) {
    if (action->colors[i].load_action == SG_LOADACTION_CLEAR) {
      _sg_d3d11_ClearRenderTargetView(_sg.d3d11.ctx, rtvs[i],
                                      (float*)&action->colors[i].clear_value);
  ...
  if ((0 != ds_flags) && dsv) {
    _sg_d3d11_ClearDepthStencilView(_sg.d3d11.ctx, dsv, ds_flags,
                                    action->depth.clear_value,
                                    action->stencil.clear_value);
...
_SOKOL_PRIVATE UINT _sg_d3d11_calcsubresource(UINT mip_slice,
                                              UINT array_slice,
                                              UINT mip_levels) {
  ...
  if (!_sg.cur_pass.is_compute) {
    ...
    if (_sg.cur_pass.atts_id.id != SG_INVALID_ID) {
      ...
      SOKOL_ASSERT(_sg.cur_pass.atts &&
                   _sg.cur_pass.atts->slot.id == _sg.cur_pass.atts_id.id);
      for (size_t i = 0; i < (size_t)_sg.cur_pass.atts->cmn.num_colors; i++) {
        const _sg_image_t* resolve_img =
            _sg.cur_pass.atts->d3d11.resolves[i].image;
        if (resolve_img) {
          const _sg_image_t* color_img =
              _sg.cur_pass.atts->d3d11.colors[i].image;
          const _sg_attachment_common_t* cmn_color_att =
              &_sg.cur_pass.atts->cmn.colors[i];
          const _sg_attachment_common_t* cmn_resolve_att =
              &_sg.cur_pass.atts->cmn.resolves[i];
          SOKOL_ASSERT(resolve_img->slot.id == cmn_resolve_att->image_id.id);
          SOKOL_ASSERT(color_img &&
                       (color_img->slot.id == cmn_color_att->image_id.id));
          ...
          const UINT src_subres = _sg_d3d11_calcsubresource(
              (UINT)cmn_color_att->mip_level, (UINT)cmn_color_att->slice,
              (UINT)color_img->cmn.num_mipmaps);
          const UINT dst_subres = _sg_d3d11_calcsubresource(
              (UINT)cmn_resolve_att->mip_level, (UINT)cmn_resolve_att->slice,
              (UINT)resolve_img->cmn.num_mipmaps);
          _sg_d3d11_ResolveSubresource(_sg.d3d11.ctx, resolve_img->d3d11.res,
                                       dst_subres, color_img->d3d11.res,
                                       src_subres, color_img->d3d11.format);
      ...
      if (_sg.d3d11.cur_pass.resolve_view) {
        ...
        _sg_d3d11_GetResource((ID3D11View*)_sg.d3d11.cur_pass.render_view,
                              &d3d11_render_res);
        _sg_d3d11_GetResource((ID3D11View*)_sg.d3d11.cur_pass.resolve_view,
                              &d3d11_resolve_res);
        ...
        _sg_d3d11_ResolveSubresource(_sg.d3d11.ctx, d3d11_resolve_res, 0,
                                     d3d11_render_res, 0,
                                     _sg_d3d11_rtv_pixel_format(color_fmt));
...
_SOKOL_PRIVATE void _sg_d3d11_apply_viewport(int x,
                                             int y,
                                             int w,
                                             int h,
                                             bool origin_top_left) {
...
_SOKOL_PRIVATE void _sg_d3d11_apply_scissor_rect(int x,
                                                 int y,
                                                 int w,
                                                 int h,
                                                 bool origin_top_left) {
...
_SOKOL_PRIVATE void _sg_d3d11_apply_pipeline(_sg_pipeline_t* pip) {
  ...
  if (pip->cmn.is_compute) {
    ...
    _sg_d3d11_CSSetConstantBuffers(_sg.d3d11.ctx, 0,
                                   _SG_D3D11_MAX_STAGE_UB_BINDINGS,
                                   pip->shader->d3d11.cs_cbufs);
    ...
    _sg_d3d11_OMSetDepthStencilState(_sg.d3d11.ctx, pip->d3d11.dss,
                                     pip->d3d11.stencil_ref);
    _sg_d3d11_OMSetBlendState(_sg.d3d11.ctx, pip->d3d11.bs,
                              (float*)&pip->cmn.blend_color, 0xFFFFFFFF);
    ...
    _sg_d3d11_VSSetConstantBuffers(_sg.d3d11.ctx, 0,
                                   _SG_D3D11_MAX_STAGE_UB_BINDINGS,
                                   pip->shader->d3d11.vs_cbufs);
    _sg_d3d11_PSSetShader(_sg.d3d11.ctx, pip->shader->d3d11.fs, NULL, 0);
    _sg_d3d11_PSSetConstantBuffers(_sg.d3d11.ctx, 0,
                                   _SG_D3D11_MAX_STAGE_UB_BINDINGS,
                                   pip->shader->d3d11.fs_cbufs);
...
_SOKOL_PRIVATE bool _sg_d3d11_apply_bindings(_sg_bindings_t* bnd) {
  ...
  ID3D11ShaderResourceView* d3d11_vs_srvs[_SG_D3D11_MAX_STAGE_SRV_BINDINGS] = {
      0};
  ID3D11ShaderResourceView* d3d11_fs_srvs[_SG_D3D11_MAX_STAGE_SRV_BINDINGS] = {
      0};
  ID3D11ShaderResourceView* d3d11_cs_srvs[_SG_D3D11_MAX_STAGE_SRV_BINDINGS] = {
      0};
  ...
  ID3D11UnorderedAccessView* d3d11_cs_uavs[_SG_D3D11_MAX_STAGE_UAV_BINDINGS] = {
      0};
  ...
  for (size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
    ...
    switch (stage) {
      case SG_SHADERSTAGE_VERTEX:
        d3d11_vs_srvs[d3d11_slot] = d3d11_srv;
        break;
      case SG_SHADERSTAGE_FRAGMENT:
        d3d11_fs_srvs[d3d11_slot] = d3d11_srv;
        break;
      case SG_SHADERSTAGE_COMPUTE:
        d3d11_cs_srvs[d3d11_slot] = d3d11_srv;
        break;
      default:
        SOKOL_UNREACHABLE;
    }
  ...
  for (size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
    ...
    switch (stage) {
      case SG_SHADERSTAGE_VERTEX:
        d3d11_vs_smps[d3d11_slot] = d3d11_smp;
        break;
      case SG_SHADERSTAGE_FRAGMENT:
        d3d11_fs_smps[d3d11_slot] = d3d11_smp;
        break;
      case SG_SHADERSTAGE_COMPUTE:
        d3d11_cs_smps[d3d11_slot] = d3d11_smp;
        break;
      default:
        SOKOL_UNREACHABLE;
    }
  ...
  if (is_compute) {
    _sg_d3d11_CSSetShaderResources(
        _sg.d3d11.ctx, 0, _SG_D3D11_MAX_STAGE_SRV_BINDINGS, d3d11_cs_srvs);
    _sg_d3d11_CSSetSamplers(_sg.d3d11.ctx, 0, _SG_D3D11_MAX_STAGE_SMP_BINDINGS,
                            d3d11_cs_smps);
    _sg_d3d11_CSSetUnorderedAccessViews(_sg.d3d11.ctx, 0,
                                        _SG_D3D11_MAX_STAGE_UAV_BINDINGS,
                                        d3d11_cs_uavs, NULL);
    ...
    _sg_d3d11_IASetVertexBuffers(_sg.d3d11.ctx, 0,
                                 SG_MAX_VERTEXBUFFER_BINDSLOTS, d3d11_vbs,
                                 bnd->pip->d3d11.vb_strides, d3d11_vb_offsets);
    _sg_d3d11_IASetIndexBuffer(_sg.d3d11.ctx, d3d11_ib,
                               bnd->pip->d3d11.index_format,
                               (UINT)bnd->ib_offset);
    _sg_d3d11_VSSetShaderResources(
        _sg.d3d11.ctx, 0, _SG_D3D11_MAX_STAGE_SRV_BINDINGS, d3d11_vs_srvs);
    _sg_d3d11_PSSetShaderResources(
        _sg.d3d11.ctx, 0, _SG_D3D11_MAX_STAGE_SRV_BINDINGS, d3d11_fs_srvs);
    _sg_d3d11_VSSetSamplers(_sg.d3d11.ctx, 0, _SG_D3D11_MAX_STAGE_SMP_BINDINGS,
                            d3d11_vs_smps);
    _sg_d3d11_PSSetSamplers(_sg.d3d11.ctx, 0, _SG_D3D11_MAX_STAGE_SMP_BINDINGS,
                            d3d11_fs_smps);
...
_SOKOL_PRIVATE void _sg_d3d11_apply_uniforms(int ub_slot,
                                             const sg_range* data) {
  SOKOL_ASSERT(_sg.d3d11.ctx);
  SOKOL_ASSERT((ub_slot >= 0) && (ub_slot < SG_MAX_UNIFORMBLOCK_BINDSLOTS));
  ...
  SOKOL_ASSERT(shd &&
               (shd->slot.id == _sg.d3d11.cur_pipeline->cmn.shader_id.id));
  SOKOL_ASSERT(data->size == shd->cmn.uniform_blocks[ub_slot].size);

  ID3D11Buffer* cbuf = shd->d3d11.all_cbufs[ub_slot];
  SOKOL_ASSERT(cbuf);
  _sg_d3d11_UpdateSubresource(_sg.d3d11.ctx, (ID3D11Resource*)cbuf, 0, NULL,
                              data->ptr, 0, 0);
  _sg_stats_add(d3d11.uniforms.num_update_subresource, 1);
}

_SOKOL_PRIVATE void _sg_d3d11_draw(int base_element,
                                   int num_elements,
                                   int num_instances) {
  const bool use_instanced_draw =
      (num_instances > 1) || (_sg.d3d11.use_instanced_draw);
  if (_sg.d3d11.use_indexed_draw) {
    if (use_instanced_draw) {
      _sg_d3d11_DrawIndexedInstanced(_sg.d3d11.ctx, (UINT)num_elements,
                                     (UINT)num_instances, (UINT)base_element, 0,
                                     0);
      _sg_stats_add(d3d11.draw.num_draw_indexed_instanced, 1);
    } else {
      _sg_d3d11_DrawIndexed(_sg.d3d11.ctx, (UINT)num_elements,
                            (UINT)base_element, 0);
      _sg_stats_add(d3d11.draw.num_draw_indexed, 1);
    }
  } else {
    if (use_instanced_draw) {
      _sg_d3d11_DrawInstanced(_sg.d3d11.ctx, (UINT)num_elements,
                              (UINT)num_instances, (UINT)base_element, 0);
...
_SOKOL_PRIVATE void _sg_d3d11_dispatch(int num_groups_x,
                                       int num_groups_y,
                                       int num_groups_z) {
  _sg_d3d11_Dispatch(_sg.d3d11.ctx, (UINT)num_groups_x, (UINT)num_groups_y,
                     (UINT)num_groups_z);
}
...
_SOKOL_PRIVATE void _sg_d3d11_update_buffer(_sg_buffer_t* buf,
                                            const sg_range* data) {
  SOKOL_ASSERT(buf && data && data->ptr && (data->size > 0));
  SOKOL_ASSERT(_sg.d3d11.ctx);
  SOKOL_ASSERT(buf->d3d11.buf);
  D3D11_MAPPED_SUBRESOURCE d3d11_msr;
  HRESULT hr = _sg_d3d11_Map(_sg.d3d11.ctx, (ID3D11Resource*)buf->d3d11.buf, 0,
...
_SOKOL_PRIVATE void _sg_d3d11_append_buffer(_sg_buffer_t* buf,
                                            const sg_range* data,
                                            bool new_frame) {
  SOKOL_ASSERT(buf && data && data->ptr && (data->size > 0));
  SOKOL_ASSERT(_sg.d3d11.ctx);
...
// also see:
// https://learn.microsoft.com/en-us/windows/win32/api/d3d11/nf-d3d11-d3d11calcsubresource
_SOKOL_PRIVATE void _sg_d3d11_update_image(_sg_image_t* img,
                                           const sg_image_data* data) {
  SOKOL_ASSERT(img && data);
  SOKOL_ASSERT(_sg.d3d11.ctx);
  ...
  const int num_slices =
      (img->cmn.type == SG_IMAGETYPE_ARRAY) ? img->cmn.num_slices : 1;
  const int num_depth_slices =
      (img->cmn.type == SG_IMAGETYPE_3D) ? img->cmn.num_slices : 1;
  UINT subres_index = 0;
  HRESULT hr;
  D3D11_MAPPED_SUBRESOURCE d3d11_msr;
  for (int face_index = 0; face_index < num_faces; face_index++) {
...
_SOKOL_PRIVATE MTLStoreAction _sg_mtl_store_action(sg_store_action a,
                                                   bool resolve) {
  switch (a) {
    ...
    default:
      SOKOL_UNREACHABLE;
      return (MTLStoreAction)0;
  }
}

_SOKOL_PRIVATE MTLResourceOptions
_sg_mtl_resource_options_storage_mode_managed_or_shared(void) {
...
_SOKOL_PRIVATE MTLResourceOptions
_sg_mtl_buffer_resource_options(sg_usage usg) {
  switch (usg) {
    case SG_USAGE_IMMUTABLE:
...
_SOKOL_PRIVATE MTLVertexStepFunction
_sg_mtl_step_function(sg_vertex_step step) {
  switch (step) {
    case SG_VERTEXSTEP_PER_VERTEX:
...
_SOKOL_PRIVATE API_AVAILABLE(ios(14.0), macos(12.0)) MTLSamplerBorderColor
    _sg_mtl_border_color(sg_border_color c) {
  switch (c) {
    case SG_BORDERCOLOR_TRANSPARENT_BLACK:
...
_SOKOL_PRIVATE void _sg_mtl_init_pool(const sg_desc* desc) {
  _sg.mtl.idpool.num_slots =
      2 * (2 * desc->buffer_pool_size + 4 * desc->image_pool_size +
           1 * desc->sampler_pool_size + 4 * desc->shader_pool_size +
           2 * desc->pipeline_pool_size + desc->attachments_pool_size + 128);
  _sg.mtl.idpool.pool =
      [NSMutableArray arrayWithCapacity:(NSUInteger)_sg.mtl.idpool.num_slots];
  _SG_OBJC_RETAIN(_sg.mtl.idpool.pool);
  NSNull* null = [NSNull null];
  for (int i = 0; i < _sg.mtl.idpool.num_slots; i++) {
    [_sg.mtl.idpool.pool addObject:null];
  }
  SOKOL_ASSERT([_sg.mtl.idpool.pool count] ==
               (NSUInteger)_sg.mtl.idpool.num_slots);
  // a queue of currently free slot indices
  _sg.mtl.idpool.free_queue_top = 0;
  _sg.mtl.idpool.free_queue =
      (int*)_sg_malloc_clear((size_t)_sg.mtl.idpool.num_slots * sizeof(int));
  ...
  // a circular queue which holds release items (frame index when a resource is
  // to be released, and the resource's pool index
  _sg.mtl.idpool.release_queue_front = 0;
  _sg.mtl.idpool.release_queue_back = 0;
  _sg.mtl.idpool.release_queue = (_sg_mtl_release_item_t*)_sg_malloc_clear(
      (size_t)_sg.mtl.idpool.num_slots * sizeof(_sg_mtl_release_item_t));
  for (int i = 0; i < _sg.mtl.idpool.num_slots; i++) {
...
_SOKOL_PRIVATE void _sg_mtl_destroy_pool(void) {
  _sg_free(_sg.mtl.idpool.release_queue);
  _sg.mtl.idpool.release_queue = 0;
  _sg_free(_sg.mtl.idpool.free_queue);
...
_SOKOL_PRIVATE int _sg_mtl_alloc_pool_slot(void) {
  ...
  const int slot_index =
      _sg.mtl.idpool.free_queue[--_sg.mtl.idpool.free_queue_top];
...
/*  mark an MTLResource for release, this will put the resource into the
    deferred-release queue, and the resource will then be released N frames
   later, the special pool index 0 will be ignored (this means that a nil value
   was provided to _sg_mtl_add_resource()
*/
_SOKOL_PRIVATE void _sg_mtl_release_resource(uint32_t frame_index,
                                             int slot_index) {
  ...
  // release queue full?
  SOKOL_ASSERT(_sg.mtl.idpool.release_queue_front !=
  ...
  const uint32_t safe_to_release_frame_index =
      frame_index + SG_NUM_INFLIGHT_FRAMES + 1;
  _sg.mtl.idpool.release_queue[release_index].frame_index =
...
_SOKOL_PRIVATE void _sg_mtl_garbage_collect(uint32_t frame_index) {
  while (_sg.mtl.idpool.release_queue_back !=
         _sg.mtl.idpool.release_queue_front) {
    if (frame_index <
        _sg.mtl.idpool.release_queue[_sg.mtl.idpool.release_queue_back]
            .frame_index) {
    ...
    const int slot_index =
        _sg.mtl.idpool.release_queue[_sg.mtl.idpool.release_queue_back]
            .slot_index;
    SOKOL_ASSERT((slot_index > 0) && (slot_index < _sg.mtl.idpool.num_slots));
    // note: the NSMutableArray takes ownership of its items, assigning an
    // NSNull object will release the object, no matter if using ARC or not
...
_SOKOL_PRIVATE void _sg_mtl_init_caps(void) {
  ...
  if (@available(macOS 12.0, iOS 14.0, *)) {
    _sg.features.image_clamp_to_border =
        [_sg.mtl.device supportsFamily:MTLGPUFamilyApple7] ||
    ...
    if (!_sg.features.image_clamp_to_border) {
      if (@available(macOS 13.0, iOS 16.0, *)) {
        _sg.features.image_clamp_to_border =
            [_sg.mtl.device supportsFamily:MTLGPUFamilyMetal3];
      }
    }
#endif
  }
...
_SOKOL_PRIVATE void _sg_mtl_setup_backend(const sg_desc* desc) {
...
#if defined(SOKOL_DEBUG)
    _sg.mtl.uniform_buffers[i].label =
        [NSString stringWithFormat:@"sg-uniform-buffer.%d", i];
#endif
  }
  ...
  if (desc->mtl_force_managed_storage_mode) {
    ...
    const bool is_apple_gpu =
        [_sg.mtl.device supportsFamily:MTLGPUFamilyApple1];
    if (!is_apple_gpu) {
      _sg.mtl.use_shared_storage_mode = false;
    } else {
...
_SOKOL_PRIVATE void _sg_mtl_discard_backend(void) {
  ...
  // NOTE: MTLCommandBuffer, MTLRenderCommandEncoder and
  // MTLComputeCommandEncoder are auto-released
...
_SOKOL_PRIVATE sg_resource_state
_sg_mtl_create_buffer(_sg_buffer_t* buf, const sg_buffer_desc* desc) {
  SOKOL_ASSERT(buf && desc);
  SOKOL_ASSERT(buf->cmn.size > 0);
  const bool injected = (0 != desc->mtl_buffers[0]);
  MTLResourceOptions mtl_options =
      _sg_mtl_buffer_resource_options(buf->cmn.usage);
  for (int slot = 0; slot < buf->cmn.num_slots; slot++) {
    ...
    if (injected) {
      ...
      if (desc->data.ptr) {
        ...
        mtl_buf = [_sg.mtl.device newBufferWithBytes:desc->data.ptr
                                              length:(NSUInteger)buf->cmn.size
                                             options:mtl_options];
      } else {
        // this is guaranteed to zero-initialize the buffer
        mtl_buf = [_sg.mtl.device newBufferWithLength:(NSUInteger)buf->cmn.size
                                              options:mtl_options];
      }
      if (nil == mtl_buf) {
...
_SOKOL_PRIVATE void _sg_mtl_copy_image_data(
  ...
  for (int face_index = 0; face_index < num_faces; face_index++) {
    for (int mip_index = 0; mip_index < img->cmn.num_mipmaps; mip_index++) {
      ...
      int bytes_per_slice =
          _sg_surface_pitch(img->cmn.pixel_format, mip_width, mip_height, 1);
      /* bytesPerImage special case:
         https://developer.apple.com/documentation/metal/mtltexture/1515679-replaceregion

          "Supply a nonzero value only when you copy data to a MTLTextureType3D
         type texture"
      */
      MTLRegion region;
      int bytes_per_image;
      if (img->cmn.type == SG_IMAGETYPE_3D) {
        ...
        region = MTLRegionMake3D(0, 0, 0, (NSUInteger)mip_width,
                                 (NSUInteger)mip_height, (NSUInteger)mip_depth);
        bytes_per_image = bytes_per_slice;
        // FIXME: apparently the minimal bytes_per_image size for 3D texture is
        // 4 KByte... somehow need to handle this
      } else {
      ...
      for (int slice_index = 0; slice_index < num_slices; slice_index++) {
        const int mtl_slice_index =
            (img->cmn.type == SG_IMAGETYPE_CUBE) ? face_index : slice_index;
...
_SOKOL_PRIVATE bool _sg_mtl_init_texdesc_common(MTLTextureDescriptor* mtl_desc,
                                                _sg_image_t* img) {
...
_SOKOL_PRIVATE void _sg_mtl_init_texdesc_rt(MTLTextureDescriptor* mtl_desc,
                                            _sg_image_t* img) {
...
_SOKOL_PRIVATE void _sg_mtl_init_texdesc_rt_msaa(MTLTextureDescriptor* mtl_desc,
                                                 _sg_image_t* img) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_mtl_create_image(_sg_image_t* img, const sg_image_desc* desc) {
  SOKOL_ASSERT(img && desc);
...
_SOKOL_PRIVATE sg_resource_state
_sg_mtl_create_sampler(_sg_sampler_t* smp, const sg_sampler_desc* desc) {
  SOKOL_ASSERT(smp && desc);
...
_SOKOL_PRIVATE id<MTLLibrary> _sg_mtl_compile_library(const char* src) {
  NSError* err = NULL;
  id<MTLLibrary> lib =
      [_sg.mtl.device newLibraryWithSource:[NSString stringWithUTF8String:src]
                                   options:nil
                                     error:&err];
  if (err) {
    _SG_ERROR(METAL_SHADER_COMPILATION_FAILED);
    _SG_LOGMSG(METAL_SHADER_COMPILATION_OUTPUT,
               [err.localizedDescription UTF8String]);
  }
  return lib;
}

_SOKOL_PRIVATE id<MTLLibrary> _sg_mtl_library_from_bytecode(const void* ptr,
                                                            size_t num_bytes) {
  NSError* err = NULL;
...
_SOKOL_PRIVATE bool _sg_mtl_create_shader_func(const sg_shader_function* func,
                                               const char* label,
                                               const char* label_ext,
                                               _sg_mtl_shader_func_t* res) {
  ...
  id<MTLFunction> mtl_func =
      [mtl_lib newFunctionWithName:[NSString stringWithUTF8String:func->entry]];
  if (mtl_func == nil) {
...
_SOKOL_PRIVATE void _sg_mtl_discard_shader_func(
    const _sg_mtl_shader_func_t* func) {
...
// NOTE: this is an out-of-range check for MSL bindslots that's also active in
// release mode
_SOKOL_PRIVATE bool _sg_mtl_ensure_msl_bindslot_ranges(
    const sg_shader_desc* desc) {
  SOKOL_ASSERT(desc);
  for (size_t i = 0; i < SG_MAX_UNIFORMBLOCK_BINDSLOTS; i++) {
  ...
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
    if (desc->storage_buffers[i].msl_buffer_n >=
        _SG_MTL_MAX_STAGE_UB_SBUF_BINDINGS) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_mtl_create_shader(_sg_shader_t* shd, const sg_shader_desc* desc) {
  SOKOL_ASSERT(shd && desc);
  ...
  shd->mtl.threads_per_threadgroup =
      MTLSizeMake((NSUInteger)desc->mtl_threads_per_threadgroup.x,
  ...
  if (desc->vertex_func.source || desc->vertex_func.bytecode.ptr) {
    shd_valid &= _sg_mtl_create_shader_func(&desc->vertex_func, desc->label,
                                            "vs", &shd->mtl.vertex_func);
  }
  if (desc->fragment_func.source || desc->fragment_func.bytecode.ptr) {
    shd_valid &= _sg_mtl_create_shader_func(&desc->fragment_func, desc->label,
                                            "fs", &shd->mtl.fragment_func);
  }
  if (desc->compute_func.source || desc->compute_func.bytecode.ptr) {
    shd_valid &= _sg_mtl_create_shader_func(&desc->compute_func, desc->label,
                                            "cs", &shd->mtl.compute_func);
  }
  if (!shd_valid) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_mtl_create_pipeline(_sg_pipeline_t* pip,
                        _sg_shader_t* shd,
                        const sg_pipeline_desc* desc) {
  SOKOL_ASSERT(pip && shd && desc);
  SOKOL_ASSERT(desc->shader.id == shd->slot.id);
  ...
  if (pip->cmn.is_compute) {
    NSError* err = NULL;
    MTLComputePipelineDescriptor* cp_desc =
    ...
    if (nil == mtl_cps) {
      ...
      _SG_LOGMSG(METAL_CREATE_CPS_OUTPUT,
                 [err.localizedDescription UTF8String]);
      return SG_RESOURCESTATE_FAILED;
    }
    ...
    for (NSUInteger attr_index = 0; attr_index < SG_MAX_VERTEX_ATTRIBUTES;
         attr_index++) {
      const sg_vertex_attr_state* a_state = &desc->layout.attrs[attr_index];
      if (a_state->format == SG_VERTEXFORMAT_INVALID) {
        break;
      ...
      vtx_desc.attributes[attr_index].format =
          _sg_mtl_vertex_format(a_state->format);
      vtx_desc.attributes[attr_index].offset = (NSUInteger)a_state->offset;
      vtx_desc.attributes[attr_index].bufferIndex =
          _sg_mtl_vertexbuffer_bindslot((size_t)a_state->buffer_index);
    }
    for (NSUInteger layout_index = 0;
         layout_index < SG_MAX_VERTEXBUFFER_BINDSLOTS; layout_index++) {
      if (pip->cmn.vertex_buffer_layout_active[layout_index]) {
        const sg_vertex_buffer_layout_state* l_state =
            &desc->layout.buffers[layout_index];
        const NSUInteger mtl_vb_slot =
            _sg_mtl_vertexbuffer_bindslot(layout_index);
        SOKOL_ASSERT(l_state->stride > 0);
        vtx_desc.layouts[mtl_vb_slot].stride = (NSUInteger)l_state->stride;
        vtx_desc.layouts[mtl_vb_slot].stepFunction =
            _sg_mtl_step_function(l_state->step_func);
    ...
    rp_desc.depthAttachmentPixelFormat =
        _sg_mtl_pixel_format(desc->depth.pixel_format);
    ...
    id<MTLRenderPipelineState> mtl_rps =
        [_sg.mtl.device newRenderPipelineStateWithDescriptor:rp_desc
                                                       error:&err];
    _SG_OBJC_RELEASE(rp_desc);
    if (nil == mtl_rps) {
      SOKOL_ASSERT(err);
      _SG_ERROR(METAL_CREATE_RPS_FAILED);
      _SG_LOGMSG(METAL_CREATE_RPS_OUTPUT,
                 [err.localizedDescription UTF8String]);
      return SG_RESOURCESTATE_FAILED;
    }
    ...
    id<MTLDepthStencilState> mtl_dss =
        [_sg.mtl.device newDepthStencilStateWithDescriptor:ds_desc];
...
_SOKOL_PRIVATE sg_resource_state
_sg_mtl_create_attachments(_sg_attachments_t* atts,
                           _sg_image_t** color_images,
                           _sg_image_t** resolve_images,
                           _sg_image_t* ds_img,
                           const sg_attachments_desc* desc) {
...
_SOKOL_PRIVATE _sg_image_t* _sg_mtl_attachments_color_image(
    const _sg_attachments_t* atts,
    int index) {
...
_SOKOL_PRIVATE _sg_image_t* _sg_mtl_attachments_resolve_image(
    const _sg_attachments_t* atts,
    int index) {
  // NOTE: may return null
  SOKOL_ASSERT(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
...
_SOKOL_PRIVATE _sg_image_t* _sg_mtl_attachments_ds_image(
    const _sg_attachments_t* atts) {
  // NOTE: may return null
  SOKOL_ASSERT(atts);
...
_SOKOL_PRIVATE void _sg_mtl_bind_uniform_buffers(void) {
  // In the Metal backend, uniform buffer bindings happen once in
  // sg_begin_pass() and remain valid for the entire pass. Only binding offsets
  // will be updated in sg_apply_uniforms()
  if (_sg.cur_pass.is_compute) {
    ...
    for (size_t slot = 0; slot < SG_MAX_UNIFORMBLOCK_BINDSLOTS; slot++) {
      [_sg.mtl.render_cmd_encoder
          setVertexBuffer:_sg.mtl
                              .uniform_buffers[_sg.mtl.cur_frame_rotate_index]
                   offset:0
                  atIndex:slot];
      [_sg.mtl.render_cmd_encoder
          setFragmentBuffer:_sg.mtl
                                .uniform_buffers[_sg.mtl.cur_frame_rotate_index]
                     offset:0
                    atIndex:slot];
    }
...
_SOKOL_PRIVATE void _sg_mtl_begin_compute_pass(const sg_pass* pass) {
  ...
  // NOTE: we actually want
  // computeCommandEncoderWithDispatchType:MTLDispatchTypeConcurrent, but that
  // requires bumping the macOS base version to 10.14
...
#if defined(SOKOL_DEBUG)
  if (pass->label) {
    _sg.mtl.compute_cmd_encoder.label =
        [NSString stringWithUTF8String:pass->label];
  }
#endif
}
...
_SOKOL_PRIVATE void _sg_mtl_begin_render_pass(const sg_pass* pass) {
  ...
  if (atts) {
    ...
    for (NSUInteger i = 0; i < (NSUInteger)atts->cmn.num_colors; i++) {
      ...
      SOKOL_ASSERT(color_att_img->mtl.tex[color_att_img->cmn.active_slot] !=
                   _SG_MTL_INVALID_SLOT_INDEX);
      pass_desc.colorAttachments[i].loadAction =
          _sg_mtl_load_action(action->colors[i].load_action);
      pass_desc.colorAttachments[i].storeAction = _sg_mtl_store_action(
          action->colors[i].store_action, resolve_att_img != 0);
      ...
      pass_desc.colorAttachments[i].texture =
          _sg_mtl_id(color_att_img->mtl.tex[color_att_img->cmn.active_slot]);
      pass_desc.colorAttachments[i].level =
          (NSUInteger)cmn_color_att->mip_level;
      switch (color_att_img->cmn.type) {
        case SG_IMAGETYPE_CUBE:
        case SG_IMAGETYPE_ARRAY:
          pass_desc.colorAttachments[i].slice =
              (NSUInteger)cmn_color_att->slice;
          break;
        case SG_IMAGETYPE_3D:
      ...
      if (resolve_att_img) {
        ...
        SOKOL_ASSERT(
            resolve_att_img->mtl.tex[resolve_att_img->cmn.active_slot] !=
            _SG_MTL_INVALID_SLOT_INDEX);
        pass_desc.colorAttachments[i].resolveTexture = _sg_mtl_id(
            resolve_att_img->mtl.tex[resolve_att_img->cmn.active_slot]);
        pass_desc.colorAttachments[i].resolveLevel =
            (NSUInteger)cmn_resolve_att->mip_level;
        switch (resolve_att_img->cmn.type) {
          case SG_IMAGETYPE_CUBE:
          case SG_IMAGETYPE_ARRAY:
            pass_desc.colorAttachments[i].resolveSlice =
                (NSUInteger)cmn_resolve_att->slice;
            break;
          case SG_IMAGETYPE_3D:
    ...
    if (0 != ds_att_img) {
      ...
      SOKOL_ASSERT(ds_att_img->mtl.tex[ds_att_img->cmn.active_slot] !=
                   _SG_MTL_INVALID_SLOT_INDEX);
      pass_desc.depthAttachment.texture =
          _sg_mtl_id(ds_att_img->mtl.tex[ds_att_img->cmn.active_slot]);
      pass_desc.depthAttachment.loadAction =
          _sg_mtl_load_action(action->depth.load_action);
      pass_desc.depthAttachment.storeAction =
          _sg_mtl_store_action(action->depth.store_action, false);
      ...
      switch (ds_att_img->cmn.type) {
        ...
        case SG_IMAGETYPE_ARRAY:
          ...
          break;
        case SG_IMAGETYPE_3D:
          pass_desc.depthAttachment.resolveDepthPlane =
              (NSUInteger)cmn_ds_att->slice;
          break;
        default:
          break;
      }
      if (_sg_is_depth_stencil_format(ds_att_img->cmn.pixel_format)) {
        ...
        pass_desc.stencilAttachment.loadAction =
            _sg_mtl_load_action(action->stencil.load_action);
        pass_desc.stencilAttachment.storeAction =
            _sg_mtl_store_action(action->depth.store_action, false);
        ...
        switch (ds_att_img->cmn.type) {
          ...
          case SG_IMAGETYPE_ARRAY:
            ...
            break;
          case SG_IMAGETYPE_3D:
            pass_desc.stencilAttachment.resolveDepthPlane =
                (NSUInteger)cmn_ds_att->slice;
            break;
          default:
            break;
    ...
    // ===
    // an MTKView current_drawable will not be valid if window is minimized,
    // don't do any rendering in this case
    if (0 == swapchain->metal.current_drawable) {
      _sg.cur_pass.valid = false;
      return;
    }
    // pin the swapchain resources into memory so that they outlive their
    // command buffer (this is necessary because the command buffer doesn't
    // retain references)
    int pass_desc_ref = _sg_mtl_add_resource(pass_desc);
    _sg_mtl_release_resource(_sg.frame_index, pass_desc_ref);

    _sg.mtl.cur_drawable =
        (__bridge id<CAMetalDrawable>)swapchain->metal.current_drawable;
    if (swapchain->sample_count > 1) {
      // multi-sampling: render into msaa texture, resolve into drawable texture
      id<MTLTexture> msaa_tex =
          (__bridge id<MTLTexture>)swapchain->metal.msaa_color_texture;
      SOKOL_ASSERT(msaa_tex != nil);
      pass_desc.colorAttachments[0].texture = msaa_tex;
      pass_desc.colorAttachments[0].resolveTexture =
          _sg.mtl.cur_drawable.texture;
    ...
    pass_desc.colorAttachments[0].loadAction =
        _sg_mtl_load_action(action->colors[0].load_action);
    ...
    if (swapchain->metal.depth_stencil_texture) {
      id<MTLTexture> ds_tex =
          (__bridge id<MTLTexture>)swapchain->metal.depth_stencil_texture;
  ...
  // NOTE: at least in macOS Sonoma, the following is no longer the case, a
  // valid render command encoder is also returned in a minimized window
  // ===
  // create a render command encoder, this might return nil if window is
  // minimized
  _sg.mtl.render_cmd_encoder =
      [_sg.mtl.cmd_buffer renderCommandEncoderWithDescriptor:pass_desc];
...
#if defined(SOKOL_DEBUG)
  if (pass->label) {
    _sg.mtl.render_cmd_encoder.label =
        [NSString stringWithUTF8String:pass->label];
  }
#endif
}
...
_SOKOL_PRIVATE void _sg_mtl_begin_pass(const sg_pass* pass) {
  ...
  // if this is the first pass in the frame, create one command buffer and
  // blit-cmd-encoder for the entire frame
  if (nil == _sg.mtl.cmd_buffer) {
    ...
    if (_sg.desc.mtl_use_command_buffer_with_retained_references) {
      ...
      _sg.mtl.cmd_buffer =
          [_sg.mtl.cmd_queue commandBufferWithUnretainedReferences];
    }
  ...
  if (0 == _sg.mtl.cur_ub_base_ptr) {
    _sg.mtl.cur_ub_base_ptr =
        (uint8_t*)[_sg.mtl.uniform_buffers[_sg.mtl.cur_frame_rotate_index]
            contents];
  }
...
#if defined(_SG_TARGET_MACOS)
    if (_sg_mtl_resource_options_storage_mode_managed_or_shared() ==
        MTLResourceStorageModeManaged) {
      if (_sg.compute.readwrite_sbufs.cur > 0) {
        id<MTLBlitCommandEncoder> blit_cmd_encoder =
            [_sg.mtl.cmd_buffer blitCommandEncoder];
        for (uint32_t i = 0; i < _sg.compute.readwrite_sbufs.cur; i++) {
          _sg_buffer_t* sbuf = _sg_lookup_buffer(
              &_sg.pools, _sg.compute.readwrite_sbufs.items[i]);
          if (sbuf) {
            [blit_cmd_encoder
                synchronizeResource:_sg_mtl_id(
                                        sbuf->mtl.buf[sbuf->cmn.active_slot])];
          }
        }
...
_SOKOL_PRIVATE void _sg_mtl_apply_viewport(int x,
                                           int y,
                                           int w,
                                           int h,
                                           bool origin_top_left) {
...
_SOKOL_PRIVATE void _sg_mtl_apply_scissor_rect(int x,
                                               int y,
                                               int w,
                                               int h,
                                               bool origin_top_left) {
  ...
  const _sg_recti_t clip =
      _sg_clipi(x, y, w, h, _sg.cur_pass.width, _sg.cur_pass.height);
  MTLScissorRect r;
  r.x = (NSUInteger)clip.x;
  r.y =
      (NSUInteger)(origin_top_left ? clip.y
                                   : (_sg.cur_pass.height - (clip.y + clip.h)));
...
_SOKOL_PRIVATE void _sg_mtl_apply_pipeline(_sg_pipeline_t* pip) {
  ...
  if (_sg.mtl.state_cache.cur_pipeline_id.id != pip->slot.id) {
    ...
    if (pip->cmn.is_compute) {
      ...
      [_sg.mtl.compute_cmd_encoder
          setComputePipelineState:_sg_mtl_id(pip->mtl.cps)];
    } else {
      SOKOL_ASSERT(!_sg.cur_pass.is_compute);
      SOKOL_ASSERT(nil != _sg.mtl.render_cmd_encoder);
      sg_color c = pip->cmn.blend_color;
      [_sg.mtl.render_cmd_encoder setBlendColorRed:c.r
                                             green:c.g
                                              blue:c.b
                                             alpha:c.a];
...
_SOKOL_PRIVATE bool _sg_mtl_apply_bindings(_sg_bindings_t* bnd) {
    ...
    for (size_t i = 0; i < SG_MAX_VERTEXBUFFER_BINDSLOTS; i++) {
      ...
      if ((_sg.mtl.state_cache.cur_vs_buffer_ids[mtl_slot].id != vb->slot.id) ||
        ...
        if (_sg.mtl.state_cache.cur_vs_buffer_ids[mtl_slot].id != vb->slot.id) {
          ...
          SOKOL_ASSERT(vb->mtl.buf[vb->cmn.active_slot] !=
                       _SG_MTL_INVALID_SLOT_INDEX);
          [_sg.mtl.render_cmd_encoder
              setVertexBuffer:_sg_mtl_id(vb->mtl.buf[vb->cmn.active_slot])
                       offset:(NSUInteger)vb_offset
                      atIndex:mtl_slot];
        } else {
          // only vertex buffer offset has changed
          [_sg.mtl.render_cmd_encoder
              setVertexBufferOffset:(NSUInteger)vb_offset
                            atIndex:mtl_slot];
        }
  ...
  for (size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
    ...
    SOKOL_ASSERT(img->mtl.tex[img->cmn.active_slot] !=
                 _SG_MTL_INVALID_SLOT_INDEX);
    const sg_shader_stage stage = shd->cmn.images[i].stage;
    SOKOL_ASSERT((stage == SG_SHADERSTAGE_VERTEX) ||
                 (stage == SG_SHADERSTAGE_FRAGMENT) ||
    ...
    if (stage == SG_SHADERSTAGE_VERTEX) {
      ...
      if (_sg.mtl.state_cache.cur_vs_image_ids[mtl_slot].id != img->slot.id) {
        ...
        [_sg.mtl.render_cmd_encoder
            setVertexTexture:_sg_mtl_id(img->mtl.tex[img->cmn.active_slot])
                     atIndex:mtl_slot];
      ...
      if (_sg.mtl.state_cache.cur_fs_image_ids[mtl_slot].id != img->slot.id) {
        ...
        [_sg.mtl.render_cmd_encoder
            setFragmentTexture:_sg_mtl_id(img->mtl.tex[img->cmn.active_slot])
                       atIndex:mtl_slot];
      ...
      if (_sg.mtl.state_cache.cur_cs_image_ids[mtl_slot].id != img->slot.id) {
        ...
        [_sg.mtl.compute_cmd_encoder
            setTexture:_sg_mtl_id(img->mtl.tex[img->cmn.active_slot])
               atIndex:mtl_slot];
  ...
  for (size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
    ...
    if (stage == SG_SHADERSTAGE_VERTEX) {
      ...
      if (_sg.mtl.state_cache.cur_vs_sampler_ids[mtl_slot].id != smp->slot.id) {
        ...
        [_sg.mtl.render_cmd_encoder
            setVertexSamplerState:_sg_mtl_id(smp->mtl.sampler_state)
                          atIndex:mtl_slot];
      ...
      if (_sg.mtl.state_cache.cur_fs_sampler_ids[mtl_slot].id != smp->slot.id) {
        ...
        [_sg.mtl.render_cmd_encoder
            setFragmentSamplerState:_sg_mtl_id(smp->mtl.sampler_state)
                            atIndex:mtl_slot];
      ...
      if (_sg.mtl.state_cache.cur_cs_sampler_ids[mtl_slot].id != smp->slot.id) {
        ...
        [_sg.mtl.compute_cmd_encoder
            setSamplerState:_sg_mtl_id(smp->mtl.sampler_state)
                    atIndex:mtl_slot];
  ...
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
    ...
    SOKOL_ASSERT(sbuf->mtl.buf[sbuf->cmn.active_slot] !=
                 _SG_MTL_INVALID_SLOT_INDEX);
    const sg_shader_stage stage = shd->cmn.storage_buffers[i].stage;
    SOKOL_ASSERT((stage == SG_SHADERSTAGE_VERTEX) ||
                 (stage == SG_SHADERSTAGE_FRAGMENT) ||
    ...
    if (stage == SG_SHADERSTAGE_VERTEX) {
      ...
      if (_sg.mtl.state_cache.cur_vs_buffer_ids[mtl_slot].id != sbuf->slot.id) {
        ...
        [_sg.mtl.render_cmd_encoder
            setVertexBuffer:_sg_mtl_id(sbuf->mtl.buf[sbuf->cmn.active_slot])
                     offset:0
                    atIndex:mtl_slot];
        _sg_stats_add(metal.bindings.num_set_vertex_buffer, 1);
      }
      ...
      if (_sg.mtl.state_cache.cur_fs_buffer_ids[mtl_slot].id != sbuf->slot.id) {
        ...
        [_sg.mtl.render_cmd_encoder
            setFragmentBuffer:_sg_mtl_id(sbuf->mtl.buf[sbuf->cmn.active_slot])
                       offset:0
                      atIndex:mtl_slot];
      ...
      if (_sg.mtl.state_cache.cur_cs_buffer_ids[mtl_slot].id != sbuf->slot.id) {
        ...
        [_sg.mtl.compute_cmd_encoder
            setBuffer:_sg_mtl_id(sbuf->mtl.buf[sbuf->cmn.active_slot])
               offset:0
              atIndex:mtl_slot];
...
_SOKOL_PRIVATE void _sg_mtl_apply_uniforms(int ub_slot, const sg_range* data) {
  SOKOL_ASSERT((ub_slot >= 0) && (ub_slot < SG_MAX_UNIFORMBLOCK_BINDSLOTS));
  SOKOL_ASSERT(((size_t)_sg.mtl.cur_ub_offset + data->size) <=
               (size_t)_sg.mtl.ub_size);
  ...
  // copy to global uniform buffer, record offset into cmd encoder, and advance
  // offset
  ...
  if (stage == SG_SHADERSTAGE_VERTEX) {
    SOKOL_ASSERT(nil != _sg.mtl.render_cmd_encoder);
    [_sg.mtl.render_cmd_encoder
        setVertexBufferOffset:(NSUInteger)_sg.mtl.cur_ub_offset
                      atIndex:mtl_slot];
    _sg_stats_add(metal.uniforms.num_set_vertex_buffer_offset, 1);
  } else if (stage == SG_SHADERSTAGE_FRAGMENT) {
    SOKOL_ASSERT(nil != _sg.mtl.render_cmd_encoder);
    [_sg.mtl.render_cmd_encoder
        setFragmentBufferOffset:(NSUInteger)_sg.mtl.cur_ub_offset
                        atIndex:mtl_slot];
    _sg_stats_add(metal.uniforms.num_set_fragment_buffer_offset, 1);
  } else if (stage == SG_SHADERSTAGE_COMPUTE) {
    SOKOL_ASSERT(nil != _sg.mtl.compute_cmd_encoder);
    [_sg.mtl.compute_cmd_encoder
        setBufferOffset:(NSUInteger)_sg.mtl.cur_ub_offset
                atIndex:mtl_slot];
  ...
  _sg.mtl.cur_ub_offset =
      _sg_roundup(_sg.mtl.cur_ub_offset + (int)data->size, _SG_MTL_UB_ALIGN);
}

_SOKOL_PRIVATE void _sg_mtl_draw(int base_element,
                                 int num_elements,
                                 int num_instances) {
  SOKOL_ASSERT(nil != _sg.mtl.render_cmd_encoder);
  SOKOL_ASSERT(_sg.mtl.state_cache.cur_pipeline &&
               (_sg.mtl.state_cache.cur_pipeline->slot.id ==
  ...
  if (SG_INDEXTYPE_NONE != _sg.mtl.state_cache.cur_pipeline->cmn.index_type) {
    ...
    SOKOL_ASSERT(ib->mtl.buf[ib->cmn.active_slot] !=
                 _SG_MTL_INVALID_SLOT_INDEX);
    const NSUInteger index_buffer_offset =
        (NSUInteger)(_sg.mtl.state_cache.cur_indexbuffer_offset +
                     base_element *
                         _sg.mtl.state_cache.cur_pipeline->mtl.index_size);
    [_sg.mtl.render_cmd_encoder
    ...
    // non-indexed rendering
    [_sg.mtl.render_cmd_encoder
        drawPrimitives:_sg.mtl.state_cache.cur_pipeline->mtl.prim_type
...
_SOKOL_PRIVATE void _sg_mtl_dispatch(int num_groups_x,
                                     int num_groups_y,
                                     int num_groups_z) {
  SOKOL_ASSERT(nil != _sg.mtl.compute_cmd_encoder);
  SOKOL_ASSERT(_sg.mtl.state_cache.cur_pipeline &&
  ...
  const MTLSize thread_groups =
      MTLSizeMake((NSUInteger)num_groups_x, (NSUInteger)num_groups_y,
                  (NSUInteger)num_groups_z);
  const MTLSize threads_per_threadgroup = cur_pip->mtl.threads_per_threadgroup;
  [_sg.mtl.compute_cmd_encoder dispatchThreadgroups:thread_groups
                              threadsPerThreadgroup:threads_per_threadgroup];
}

_SOKOL_PRIVATE void _sg_mtl_update_buffer(_sg_buffer_t* buf,
                                          const sg_range* data) {
  SOKOL_ASSERT(buf && data && data->ptr && (data->size > 0));
  if (++buf->cmn.active_slot >= buf->cmn.num_slots) {
    buf->cmn.active_slot = 0;
  ...
  __unsafe_unretained id<MTLBuffer> mtl_buf =
      _sg_mtl_id(buf->mtl.buf[buf->cmn.active_slot]);
...
_SOKOL_PRIVATE void _sg_mtl_append_buffer(_sg_buffer_t* buf,
                                          const sg_range* data,
                                          bool new_frame) {
  SOKOL_ASSERT(buf && data && data->ptr && (data->size > 0));
  if (new_frame) {
...
_SOKOL_PRIVATE void _sg_mtl_update_image(_sg_image_t* img,
                                         const sg_image_data* data) {
  SOKOL_ASSERT(img && data);
  if (++img->cmn.active_slot >= img->cmn.num_slots) {
    img->cmn.active_slot = 0;
  ...
  __unsafe_unretained id<MTLTexture> mtl_tex =
      _sg_mtl_id(img->mtl.tex[img->cmn.active_slot]);
...
_SOKOL_PRIVATE void _sg_mtl_push_debug_group(const char* name) {
  ...
  if (_sg.mtl.render_cmd_encoder) {
    [_sg.mtl.render_cmd_encoder
        pushDebugGroup:[NSString stringWithUTF8String:name]];
...
_SOKOL_PRIVATE WGPUBufferUsage _sg_wgpu_buffer_usage(sg_buffer_type t,
                                                     sg_usage u) {
...
_SOKOL_PRIVATE WGPULoadOp _sg_wgpu_load_op(WGPUTextureView view,
                                           sg_load_action a) {
  if (0 == view) {
...
_SOKOL_PRIVATE WGPUStoreOp _sg_wgpu_store_op(WGPUTextureView view,
                                             sg_store_action a) {
  if (0 == view) {
...
_SOKOL_PRIVATE WGPUTextureViewDimension
_sg_wgpu_texture_view_dimension(sg_image_type t) {
  switch (t) {
    case SG_IMAGETYPE_2D:
...
_SOKOL_PRIVATE WGPUTextureDimension
_sg_wgpu_texture_dimension(sg_image_type t) {
...
_sg_wgpu_texture_sample_type(sg_image_sample_type t, bool msaa) {
  switch (t) {
    case SG_IMAGESAMPLETYPE_FLOAT:
      return msaa ? WGPUTextureSampleType_UnfilterableFloat
...
_SOKOL_PRIVATE WGPUSamplerBindingType
_sg_wgpu_sampler_binding_type(sg_sampler_type t) {
  switch (t) {
...
_SOKOL_PRIVATE WGPUMipmapFilterMode
_sg_wgpu_sampler_mipmap_filter(sg_filter f) {
  switch (f) {
    case SG_FILTER_NEAREST:
...
_SOKOL_PRIVATE WGPUIndexFormat _sg_wgpu_indexformat(sg_index_type t) {
  ...
  return (t == SG_INDEXTYPE_UINT16) ? WGPUIndexFormat_Uint16
                                    : WGPUIndexFormat_Uint32;
}

_SOKOL_PRIVATE WGPUIndexFormat
_sg_wgpu_stripindexformat(sg_primitive_type prim_type, sg_index_type idx_type) {
  if (idx_type == SG_INDEXTYPE_NONE) {
    return WGPUIndexFormat_Undefined;
  } else if ((prim_type == SG_PRIMITIVETYPE_LINE_STRIP) ||
             (prim_type == SG_PRIMITIVETYPE_TRIANGLE_STRIP)) {
...
_SOKOL_PRIVATE WGPUVertexStepMode _sg_wgpu_stepmode(sg_vertex_step s) {
  return (s == SG_VERTEXSTEP_PER_VERTEX) ? WGPUVertexStepMode_Vertex
...
_SOKOL_PRIVATE void _sg_wgpu_init_caps(void) {
  ...
  _sg.limits.max_image_size_cube =
      (int)l
          ->maxTextureDimension2D;  // not a bug, see:
  ...
  if (wgpuDeviceHasFeature(_sg.wgpu.dev,
...
_SOKOL_PRIVATE void _sg_wgpu_uniform_buffer_init(const sg_desc* desc) {
  ...
  _sg.wgpu.uniform.num_bytes =
      (uint32_t)(desc->uniform_buffer_size + _SG_WGPU_MAX_UNIFORM_UPDATE_SIZE);
...
_SOKOL_PRIVATE void _sg_wgpu_uniform_buffer_on_commit(void) {
  wgpuQueueWriteBuffer(_sg.wgpu.queue, _sg.wgpu.uniform.buf, 0,
                       _sg.wgpu.uniform.staging, _sg.wgpu.uniform.offset);
  _sg_stats_add(wgpu.uniforms.size_write_buffer, _sg.wgpu.uniform.offset);
  _sg.wgpu.uniform.offset = 0;
  _sg_clear(_sg.wgpu.uniform.bind_offsets,
            sizeof(_sg.wgpu.uniform.bind_offsets));
}

_SOKOL_PRIVATE void _sg_wgpu_bindgroups_pool_init(const sg_desc* desc) {
  SOKOL_ASSERT((desc->wgpu_bindgroups_cache_size > 0) &&
               (desc->wgpu_bindgroups_cache_size < _SG_MAX_POOL_SIZE));
...
_SOKOL_PRIVATE _sg_wgpu_bindgroup_t* _sg_wgpu_bindgroup_at(uint32_t bg_id) {
  ...
  SOKOL_ASSERT((slot_index > _SG_INVALID_SLOT_INDEX) &&
               (slot_index < p->pool.size));
...
_SOKOL_PRIVATE _sg_wgpu_bindgroup_handle_t _sg_wgpu_alloc_bindgroup(void) {
  ...
  if (_SG_INVALID_SLOT_INDEX != slot_index) {
    res.id =
        _sg_slot_alloc(&p->pool, &p->bindgroups[slot_index].slot, slot_index);
  } else {
...
_SOKOL_PRIVATE void _sg_wgpu_dealloc_bindgroup(_sg_wgpu_bindgroup_t* bg) {
  SOKOL_ASSERT(bg && (bg->slot.state == SG_RESOURCESTATE_ALLOC) &&
               (bg->slot.id != SG_INVALID_ID));
  _sg_wgpu_bindgroups_pool_t* p = &_sg.wgpu.bindgroups_pool;
  _sg_pool_free_index(&p->pool, _sg_slot_index(bg->slot.id));
  _sg_slot_reset(&bg->slot);
}

_SOKOL_PRIVATE void _sg_wgpu_reset_bindgroup_to_alloc_state(
    _sg_wgpu_bindgroup_t* bg) {
  SOKOL_ASSERT(bg);
...
// MurmurHash64B (see:
// https://github.com/aappleby/smhasher/blob/61a0530f28277f2e850bfc39600ce61d02b518de/src/MurmurHash2.cpp#L142)
_SOKOL_PRIVATE uint64_t _sg_wgpu_hash(const void* key, int len, uint64_t seed) {
  ...
  switch (len) {
    case 3:
      h2 ^= (uint32_t)(((unsigned char*)data)[2] << 16);
    case 2:
      h2 ^= (uint32_t)(((unsigned char*)data)[1] << 8);
    case 1:
...
_SOKOL_PRIVATE uint64_t
_sg_wgpu_bindgroups_cache_item(_sg_wgpu_bindgroups_cache_item_type_t type,
                               uint8_t wgpu_binding,
                               uint32_t id) {
...
_SOKOL_PRIVATE uint64_t _sg_wgpu_bindgroups_cache_pip_item(uint32_t id) {
  return _sg_wgpu_bindgroups_cache_item(
      _SG_WGPU_BINDGROUPSCACHEITEMTYPE_PIPELINE, 0xFF, id);
}

_SOKOL_PRIVATE uint64_t
_sg_wgpu_bindgroups_cache_image_item(uint8_t wgpu_binding, uint32_t id) {
  return _sg_wgpu_bindgroups_cache_item(_SG_WGPU_BINDGROUPSCACHEITEMTYPE_IMAGE,
                                        wgpu_binding, id);
}

_SOKOL_PRIVATE uint64_t
_sg_wgpu_bindgroups_cache_sampler_item(uint8_t wgpu_binding, uint32_t id) {
  return _sg_wgpu_bindgroups_cache_item(
      _SG_WGPU_BINDGROUPSCACHEITEMTYPE_SAMPLER, wgpu_binding, id);
}

_SOKOL_PRIVATE uint64_t
_sg_wgpu_bindgroups_cache_sbuf_item(uint8_t wgpu_binding, uint32_t id) {
  return _sg_wgpu_bindgroups_cache_item(
      _SG_WGPU_BINDGROUPSCACHEITEMTYPE_STORAGEBUFFER, wgpu_binding, id);
}

_SOKOL_PRIVATE void _sg_wgpu_init_bindgroups_cache_key(
    _sg_wgpu_bindgroups_cache_key_t* key,
    const _sg_bindings_t* bnd) {
  SOKOL_ASSERT(bnd);
  SOKOL_ASSERT(bnd->pip);
  ...
  for (size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
    ...
    key->items[item_idx] =
        _sg_wgpu_bindgroups_cache_image_item(wgpu_binding, id);
  }
  for (size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
    ...
    key->items[item_idx] =
        _sg_wgpu_bindgroups_cache_sampler_item(wgpu_binding, id);
  }
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
    if (shd->cmn.storage_buffers[i].stage == SG_SHADERSTAGE_NONE) {
      continue;
    ...
    const size_t item_idx =
        i + 1 + SG_MAX_IMAGE_BINDSLOTS + SG_MAX_SAMPLER_BINDSLOTS;
    SOKOL_ASSERT(item_idx < _SG_WGPU_BINDGROUPSCACHEKEY_NUM_ITEMS);
    SOKOL_ASSERT(0 == key->items[item_idx]);
    const uint8_t wgpu_binding = shd->wgpu.sbuf_grp1_bnd_n[i];
    const uint32_t id = bnd->sbufs[i]->slot.id;
    key->items[item_idx] =
        _sg_wgpu_bindgroups_cache_sbuf_item(wgpu_binding, id);
  }
  key->hash =
      _sg_wgpu_hash(&key->items, (int)sizeof(key->items), 0x1234567887654321);
}

_SOKOL_PRIVATE bool _sg_wgpu_compare_bindgroups_cache_key(
    _sg_wgpu_bindgroups_cache_key_t* k0,
    _sg_wgpu_bindgroups_cache_key_t* k1) {
  SOKOL_ASSERT(k0 && k1);
  if (k0->hash != k1->hash) {
    return false;
  }
...
_SOKOL_PRIVATE _sg_wgpu_bindgroup_t* _sg_wgpu_create_bindgroup(
    _sg_bindings_t* bnd) {
...
_SOKOL_PRIVATE void _sg_wgpu_bindgroups_cache_init(const sg_desc* desc) {
  ...
  _sg.wgpu.bindgroups_cache.items =
      (_sg_wgpu_bindgroup_handle_t*)_sg_malloc_clear(size_in_bytes);
}
...
_SOKOL_PRIVATE void _sg_wgpu_bindgroups_cache_set(uint64_t hash,
                                                  uint32_t bg_id) {
...
_SOKOL_PRIVATE void _sg_wgpu_bindgroups_cache_invalidate(
    _sg_wgpu_bindgroups_cache_item_type_t type,
    uint32_t id) {
  const uint64_t key_mask = 0x0000FFFFFFFFFFFF;
  const uint64_t key_item =
      _sg_wgpu_bindgroups_cache_item(type, 0, id) & key_mask;
  SOKOL_ASSERT(_sg.wgpu.bindgroups_cache.items);
  for (uint32_t cache_item_idx = 0;
       cache_item_idx < _sg.wgpu.bindgroups_cache.num; cache_item_idx++) {
    ...
    if (bg_id != SG_INVALID_ID) {
      ...
      // check if resource is in bindgroup, if yes discard bindgroup and
      // invalidate cache slot
      bool invalidate_cache_item = false;
      for (int key_item_idx = 0;
...
_SOKOL_PRIVATE bool _sg_wgpu_bindings_cache_vb_dirty(size_t index,
                                                     const _sg_buffer_t* vb,
                                                     uint64_t offset) {
...
_SOKOL_PRIVATE void _sg_wgpu_bindings_cache_vb_update(size_t index,
                                                      const _sg_buffer_t* vb,
                                                      uint64_t offset) {
  SOKOL_ASSERT((index >= 0) && (index < SG_MAX_VERTEXBUFFER_BINDSLOTS));
  if (vb) {
...
_SOKOL_PRIVATE bool _sg_wgpu_bindings_cache_ib_dirty(const _sg_buffer_t* ib,
                                                     uint64_t offset) {
  if (ib) {
...
_SOKOL_PRIVATE void _sg_wgpu_bindings_cache_ib_update(const _sg_buffer_t* ib,
                                                      uint64_t offset) {
  if (ib) {
...
_SOKOL_PRIVATE bool _sg_wgpu_bindings_cache_bg_dirty(
    const _sg_wgpu_bindgroup_t* bg) {
  if (bg) {
...
_SOKOL_PRIVATE void _sg_wgpu_bindings_cache_bg_update(
    const _sg_wgpu_bindgroup_t* bg) {
  if (bg) {
...
_SOKOL_PRIVATE void _sg_wgpu_set_img_smp_sbuf_bindgroup(
    _sg_wgpu_bindgroup_t* bg) {
    ...
    if (_sg.cur_pass.is_compute) {
      ...
      if (bg) {
        ...
        wgpuComputePassEncoderSetBindGroup(
            _sg.wgpu.cpass_enc, _SG_WGPU_IMG_SMP_SBUF_BINDGROUP_INDEX,
            bg->bindgroup, 0, 0);
      } else {
        wgpuComputePassEncoderSetBindGroup(
            _sg.wgpu.cpass_enc, _SG_WGPU_IMG_SMP_SBUF_BINDGROUP_INDEX,
            _sg.wgpu.empty_bind_group, 0, 0);
      }
    } else {
      ...
      if (bg) {
        ...
        wgpuRenderPassEncoderSetBindGroup(_sg.wgpu.rpass_enc,
                                          _SG_WGPU_IMG_SMP_SBUF_BINDGROUP_INDEX,
                                          bg->bindgroup, 0, 0);
      } else {
        wgpuRenderPassEncoderSetBindGroup(_sg.wgpu.rpass_enc,
                                          _SG_WGPU_IMG_SMP_SBUF_BINDGROUP_INDEX,
...
_SOKOL_PRIVATE bool _sg_wgpu_apply_bindgroup(_sg_bindings_t* bnd) {
  if (!_sg.desc.wgpu_disable_bindgroups_cache) {
    ...
    if (bg == 0) {
      // either no cache entry yet, or cache collision, create new bindgroup and
      // store in cache
    ...
    // bindgroups cache disabled, create and destroy bindgroup on the fly
    // (expensive!)
...
_SOKOL_PRIVATE bool _sg_wgpu_apply_index_buffer(_sg_bindings_t* bnd) {
  ...
  if (_sg_wgpu_bindings_cache_ib_dirty(ib, offset)) {
    ...
    if (ib) {
      const WGPUIndexFormat format =
          _sg_wgpu_indexformat(bnd->pip->cmn.index_type);
      const uint64_t buf_size = (uint64_t)ib->cmn.size;
      SOKOL_ASSERT(buf_size > offset);
      const uint64_t max_bytes = buf_size - offset;
      wgpuRenderPassEncoderSetIndexBuffer(_sg.wgpu.rpass_enc, ib->wgpu.buf,
                                          format, offset, max_bytes);
      /* FIXME: the else-pass should actually set a null index buffer, but that
      doesn't seem to work yet } else {
          wgpuRenderPassEncoderSetIndexBuffer(_sg.wgpu.rpass_enc, 0,
      WGPUIndexFormat_Undefined, 0, 0);
      */
    }
  ...
  for (uint32_t slot = 0; slot < SG_MAX_VERTEXBUFFER_BINDSLOTS; slot++) {
    ...
    if (_sg_wgpu_bindings_cache_vb_dirty(slot, vb, offset)) {
      ...
      if (vb) {
        ...
        SOKOL_ASSERT(buf_size > offset);
        const uint64_t max_bytes = buf_size - offset;
        wgpuRenderPassEncoderSetVertexBuffer(_sg.wgpu.rpass_enc, slot,
                                             vb->wgpu.buf, offset, max_bytes);
        /* FIXME: the else-pass should actually set a null vertex buffer, but
        that doesn't seem to work yet } else {
            wgpuRenderPassEncoderSetVertexBuffer(_sg.wgpu.rpass_enc, slot, 0, 0,
        0);
        */
      }
...
_SOKOL_PRIVATE void _sg_wgpu_setup_backend(const sg_desc* desc) {
  ...
  WGPUBindGroupLayout empty_bgl =
      wgpuDeviceCreateBindGroupLayout(_sg.wgpu.dev, &bgl_desc);
  ...
  _sg.wgpu.cmd_enc =
      wgpuDeviceCreateCommandEncoder(_sg.wgpu.dev, &cmd_enc_desc);
...
_SOKOL_PRIVATE sg_resource_state
_sg_wgpu_create_buffer(_sg_buffer_t* buf, const sg_buffer_desc* desc) {
  SOKOL_ASSERT(buf && desc);
  SOKOL_ASSERT(buf->cmn.size > 0);
  ...
  if (injected) {
    ...
    // buffer mapping size must be multiple of 4, so round up buffer size (only
    // a problem with index buffers containing odd number of indices)
    const uint64_t wgpu_buf_size = _sg_roundup_u64((uint64_t)buf->cmn.size, 4);
    const bool map_at_creation =
        (SG_USAGE_IMMUTABLE == buf->cmn.usage) && (desc->data.ptr);
...
_SOKOL_PRIVATE void _sg_wgpu_discard_buffer(_sg_buffer_t* buf) {
  ...
  if (buf->cmn.type == SG_BUFFERTYPE_STORAGEBUFFER) {
    _sg_wgpu_bindgroups_cache_invalidate(
        _SG_WGPU_BINDGROUPSCACHEITEMTYPE_STORAGEBUFFER, buf->slot.id);
  }
  if (buf->wgpu.buf) {
...
_SOKOL_PRIVATE void _sg_wgpu_copy_buffer_data(const _sg_buffer_t* buf,
                                              uint64_t offset,
                                              const sg_range* data) {
  SOKOL_ASSERT((offset + data->size) <= (size_t)buf->cmn.size);
  // WebGPU's write-buffer requires the size to be a multiple of four, so we may
  // need to split the copy operation into two writeBuffer calls
  uint64_t clamped_size = data->size & ~3UL;
  uint64_t extra_size = data->size & 3UL;
  SOKOL_ASSERT(extra_size < 4);
  wgpuQueueWriteBuffer(_sg.wgpu.queue, buf->wgpu.buf, offset, data->ptr,
                       clamped_size);
  if (extra_size > 0) {
    ...
    wgpuQueueWriteBuffer(_sg.wgpu.queue, buf->wgpu.buf, extra_dst_offset,
                         extra_src_ptr, 4);
  }
}

_SOKOL_PRIVATE void _sg_wgpu_copy_image_data(const _sg_image_t* img,
  ...
  for (int face_index = 0; face_index < num_faces; face_index++) {
    for (int mip_index = 0; mip_index < img->cmn.num_mipmaps; mip_index++) {
      ...
      wgpuQueueWriteTexture(_sg.wgpu.queue, &wgpu_copy_tex, mip_data->ptr,
                            mip_data->size, &wgpu_layout, &wgpu_extent);
    }
  }
}

_SOKOL_PRIVATE sg_resource_state
_sg_wgpu_create_image(_sg_image_t* img, const sg_image_desc* desc) {
  SOKOL_ASSERT(img && desc);
  ...
  if (injected) {
    ...
    wgpu_texview_desc.dimension =
        _sg_wgpu_texture_view_dimension(img->cmn.type);
...
_SOKOL_PRIVATE void _sg_wgpu_discard_image(_sg_image_t* img) {
  SOKOL_ASSERT(img);
  _sg_wgpu_bindgroups_cache_invalidate(_SG_WGPU_BINDGROUPSCACHEITEMTYPE_IMAGE,
                                       img->slot.id);
  if (img->wgpu.view) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_wgpu_create_sampler(_sg_sampler_t* smp, const sg_sampler_desc* desc) {
  SOKOL_ASSERT(smp && desc);
  SOKOL_ASSERT(_sg.wgpu.dev);
  ...
  if (injected) {
    ...
    wgpu_desc.mipmapFilter =
        _sg_wgpu_sampler_mipmap_filter(desc->mipmap_filter);
...
_SOKOL_PRIVATE void _sg_wgpu_discard_sampler(_sg_sampler_t* smp) {
  SOKOL_ASSERT(smp);
  _sg_wgpu_bindgroups_cache_invalidate(_SG_WGPU_BINDGROUPSCACHEITEMTYPE_SAMPLER,
                                       smp->slot.id);
  if (smp->wgpu.smp) {
...
_SOKOL_PRIVATE _sg_wgpu_shader_func_t
_sg_wgpu_create_shader_func(const sg_shader_function* func, const char* label) {
  SOKOL_ASSERT(func);
...
typedef struct {
  uint8_t sokol_slot, wgpu_slot;
} _sg_wgpu_dynoffset_mapping_t;

_SOKOL_PRIVATE int _sg_wgpu_dynoffset_cmp(const void* a, const void* b) {
  const _sg_wgpu_dynoffset_mapping_t* aa =
      (const _sg_wgpu_dynoffset_mapping_t*)a;
  const _sg_wgpu_dynoffset_mapping_t* bb =
      (const _sg_wgpu_dynoffset_mapping_t*)b;
  if (aa->wgpu_slot < bb->wgpu_slot)
    return -1;
  else if (aa->wgpu_slot > bb->wgpu_slot)
    return 1;
  return 0;
}

// NOTE: this is an out-of-range check for WGSL bindslots that's also active in
// release mode
_SOKOL_PRIVATE bool _sg_wgpu_ensure_wgsl_bindslot_ranges(
    const sg_shader_desc* desc) {
  SOKOL_ASSERT(desc);
  for (size_t i = 0; i < SG_MAX_UNIFORMBLOCK_BINDSLOTS; i++) {
    if (desc->uniform_blocks[i].wgsl_group0_binding_n >=
        _SG_WGPU_MAX_UB_BINDGROUP_BIND_SLOTS) {
      _SG_ERROR(WGPU_UNIFORMBLOCK_WGSL_GROUP0_BINDING_OUT_OF_RANGE);
      return false;
    }
  }
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
    if (desc->storage_buffers[i].wgsl_group1_binding_n >=
        _SG_WGPU_MAX_IMG_SMP_SBUF_BIND_SLOTS) {
      _SG_ERROR(WGPU_STORAGEBUFFER_WGSL_GROUP1_BINDING_OUT_OF_RANGE);
      return false;
    }
  }
  for (size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
    if (desc->images[i].wgsl_group1_binding_n >=
        _SG_WGPU_MAX_IMG_SMP_SBUF_BIND_SLOTS) {
  ...
  for (size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
    if (desc->samplers[i].wgsl_group1_binding_n >=
        _SG_WGPU_MAX_IMG_SMP_SBUF_BIND_SLOTS) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_wgpu_create_shader(_sg_shader_t* shd, const sg_shader_desc* desc) {
  SOKOL_ASSERT(shd && desc);
  ...
  if (desc->vertex_func.source) {
    shd->wgpu.vertex_func =
        _sg_wgpu_create_shader_func(&desc->vertex_func, desc->label);
    shd_valid &= shd->wgpu.vertex_func.module != 0;
  }
  if (desc->fragment_func.source) {
    shd->wgpu.fragment_func =
        _sg_wgpu_create_shader_func(&desc->fragment_func, desc->label);
    shd_valid &= shd->wgpu.fragment_func.module != 0;
  }
  if (desc->compute_func.source) {
    shd->wgpu.compute_func =
        _sg_wgpu_create_shader_func(&desc->compute_func, desc->label);
  ...
  SOKOL_ASSERT(_SG_WGPU_MAX_UB_BINDGROUP_ENTRIES <=
               _SG_WGPU_MAX_IMG_SMP_SBUF_BINDGROUP_ENTRIES);
  WGPUBindGroupLayoutEntry
      bgl_entries[_SG_WGPU_MAX_IMG_SMP_SBUF_BINDGROUP_ENTRIES];
  ...
  for (size_t i = 0; i < SG_MAX_UNIFORMBLOCK_BINDSLOTS; i++) {
    ...
    bgl_entry->visibility =
        _sg_wgpu_shader_stage(shd->cmn.uniform_blocks[i].stage);
  ...
  qsort(dynoffset_map, bgl_index, sizeof(_sg_wgpu_dynoffset_mapping_t),
        _sg_wgpu_dynoffset_cmp);
  ...
  for (size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
    ...
    bgl_entry->texture.viewDimension =
        _sg_wgpu_texture_view_dimension(shd->cmn.images[i].image_type);
    bgl_entry->texture.sampleType =
        _sg_wgpu_texture_sample_type(shd->cmn.images[i].sample_type, msaa);
  ...
  for (size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
    ...
    bgl_entry->sampler.type =
        _sg_wgpu_sampler_binding_type(shd->cmn.samplers[i].sampler_type);
    bgl_index += 1;
  }
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
    if (shd->cmn.storage_buffers[i].stage == SG_SHADERSTAGE_NONE) {
    ...
    shd->wgpu.sbuf_grp1_bnd_n[i] =
        desc->storage_buffers[i].wgsl_group1_binding_n;
    WGPUBindGroupLayoutEntry* bgl_entry = &bgl_entries[bgl_index];
    bgl_entry->binding = shd->wgpu.sbuf_grp1_bnd_n[i];
    bgl_entry->visibility =
        _sg_wgpu_shader_stage(shd->cmn.storage_buffers[i].stage);
    if (shd->cmn.storage_buffers[i].readonly) {
...
_SOKOL_PRIVATE sg_resource_state
_sg_wgpu_create_pipeline(_sg_pipeline_t* pip,
                         _sg_shader_t* shd,
                         const sg_pipeline_desc* desc) {
  SOKOL_ASSERT(pip && shd && desc);
  SOKOL_ASSERT(desc->shader.id == shd->slot.id);
  ...
  const WGPUPipelineLayout wgpu_pip_layout =
      wgpuDeviceCreatePipelineLayout(_sg.wgpu.dev, &wgpu_pl_desc);
  ...
  if (pip->cmn.is_compute) {
    ...
    pip->wgpu.cpip =
        wgpuDeviceCreateComputePipeline(_sg.wgpu.dev, &wgpu_pip_desc);
    ...
    WGPUVertexAttribute wgpu_vtx_attrs[SG_MAX_VERTEXBUFFER_BINDSLOTS]
                                      [SG_MAX_VERTEX_ATTRIBUTES];
    _sg_clear(wgpu_vtx_attrs, sizeof(wgpu_vtx_attrs));
    int wgpu_vb_num = 0;
    for (int vb_idx = 0; vb_idx < SG_MAX_VERTEXBUFFER_BINDSLOTS;
         vb_idx++, wgpu_vb_num++) {
      const sg_vertex_buffer_layout_state* vbl_state =
          &desc->layout.buffers[vb_idx];
      if (0 == vbl_state->stride) {
        break;
      }
      wgpu_vb_layouts[vb_idx].arrayStride = (uint64_t)vbl_state->stride;
      wgpu_vb_layouts[vb_idx].stepMode =
          _sg_wgpu_stepmode(vbl_state->step_func);
    ...
    for (int va_idx = 0; va_idx < SG_MAX_VERTEX_ATTRIBUTES; va_idx++) {
      ...
      wgpu_vtx_attrs[vb_idx][wgpu_attr_idx].format =
          _sg_wgpu_vertexformat(va_state->format);
    ...
    wgpu_pip_desc.primitive.stripIndexFormat =
        _sg_wgpu_stripindexformat(desc->primitive_type, desc->index_type);
    ...
    if (SG_PIXELFORMAT_NONE != desc->depth.pixel_format) {
      ...
      wgpu_ds_state.depthWriteEnabled =
          _sg_wgpu_optional_bool(desc->depth.write_enabled);
      wgpu_ds_state.depthCompare = _sg_wgpu_comparefunc(desc->depth.compare);
      wgpu_ds_state.stencilFront.compare =
          _sg_wgpu_comparefunc(desc->stencil.front.compare);
      wgpu_ds_state.stencilFront.failOp =
          _sg_wgpu_stencilop(desc->stencil.front.fail_op);
      wgpu_ds_state.stencilFront.depthFailOp =
          _sg_wgpu_stencilop(desc->stencil.front.depth_fail_op);
      wgpu_ds_state.stencilFront.passOp =
          _sg_wgpu_stencilop(desc->stencil.front.pass_op);
      wgpu_ds_state.stencilBack.compare =
          _sg_wgpu_comparefunc(desc->stencil.back.compare);
      wgpu_ds_state.stencilBack.failOp =
          _sg_wgpu_stencilop(desc->stencil.back.fail_op);
      wgpu_ds_state.stencilBack.depthFailOp =
          _sg_wgpu_stencilop(desc->stencil.back.depth_fail_op);
      wgpu_ds_state.stencilBack.passOp =
          _sg_wgpu_stencilop(desc->stencil.back.pass_op);
    ...
    wgpu_pip_desc.multisample.alphaToCoverageEnabled =
    ...
    if (desc->color_count > 0) {
      ...
      for (int i = 0; i < desc->color_count; i++) {
        SOKOL_ASSERT(i < SG_MAX_COLOR_ATTACHMENTS);
        wgpu_ctgt_state[i].format =
            _sg_wgpu_textureformat(desc->colors[i].pixel_format);
        wgpu_ctgt_state[i].writeMask =
            _sg_wgpu_colorwritemask(desc->colors[i].write_mask);
        if (desc->colors[i].blend.enabled) {
          wgpu_ctgt_state[i].blend = &wgpu_blend_state[i];
          wgpu_blend_state[i].color.operation =
              _sg_wgpu_blendop(desc->colors[i].blend.op_rgb);
          wgpu_blend_state[i].color.srcFactor =
              _sg_wgpu_blendfactor(desc->colors[i].blend.src_factor_rgb);
          wgpu_blend_state[i].color.dstFactor =
              _sg_wgpu_blendfactor(desc->colors[i].blend.dst_factor_rgb);
          wgpu_blend_state[i].alpha.operation =
              _sg_wgpu_blendop(desc->colors[i].blend.op_alpha);
          wgpu_blend_state[i].alpha.srcFactor =
              _sg_wgpu_blendfactor(desc->colors[i].blend.src_factor_alpha);
          wgpu_blend_state[i].alpha.dstFactor =
              _sg_wgpu_blendfactor(desc->colors[i].blend.dst_factor_alpha);
        }
      }
      wgpu_pip_desc.fragment = &wgpu_frag_state;
    }
    pip->wgpu.rpip =
        wgpuDeviceCreateRenderPipeline(_sg.wgpu.dev, &wgpu_pip_desc);
    wgpuPipelineLayoutRelease(wgpu_pip_layout);
    if (0 == pip->wgpu.rpip) {
...
_SOKOL_PRIVATE void _sg_wgpu_discard_pipeline(_sg_pipeline_t* pip) {
  SOKOL_ASSERT(pip);
  _sg_wgpu_bindgroups_cache_invalidate(
      _SG_WGPU_BINDGROUPSCACHEITEMTYPE_PIPELINE, pip->slot.id);
...
_SOKOL_PRIVATE sg_resource_state
_sg_wgpu_create_attachments(_sg_attachments_t* atts,
                            _sg_image_t** color_images,
                            _sg_image_t** resolve_images,
                            _sg_image_t* ds_img,
                            const sg_attachments_desc* desc) {
  ...
  for (int i = 0; i < atts->cmn.num_colors; i++) {
    ...
    atts->wgpu.colors[i].view =
        wgpuTextureCreateView(color_images[i]->wgpu.tex, &wgpu_color_view_desc);
    ...
    if (resolve_desc->image.id != SG_INVALID_ID) {
      ...
      atts->wgpu.resolves[i].view = wgpuTextureCreateView(
          resolve_images[i]->wgpu.tex, &wgpu_resolve_view_desc);
  ...
  if (ds_desc->image.id != SG_INVALID_ID) {
    ...
    atts->wgpu.depth_stencil.view =
        wgpuTextureCreateView(ds_img->wgpu.tex, &wgpu_ds_view_desc);
...
_SOKOL_PRIVATE _sg_image_t* _sg_wgpu_attachments_color_image(
    const _sg_attachments_t* atts,
    int index) {
  SOKOL_ASSERT(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
  // NOTE: may return null
...
_SOKOL_PRIVATE _sg_image_t* _sg_wgpu_attachments_resolve_image(
    const _sg_attachments_t* atts,
    int index) {
  SOKOL_ASSERT(atts && (index >= 0) && (index < SG_MAX_COLOR_ATTACHMENTS));
  // NOTE: may return null
...
_SOKOL_PRIVATE _sg_image_t* _sg_wgpu_attachments_ds_image(
    const _sg_attachments_t* atts) {
  // NOTE: may return null
  SOKOL_ASSERT(atts);
...
_SOKOL_PRIVATE void _sg_wgpu_init_color_att(
    WGPURenderPassColorAttachment* wgpu_att,
    const sg_color_attachment_action* action,
...
_SOKOL_PRIVATE void _sg_wgpu_init_ds_att(
    WGPURenderPassDepthStencilAttachment* wgpu_att,
  ...
  if (_sg_is_depth_stencil_format(fmt)) {
    wgpu_att->stencilLoadOp =
        _sg_wgpu_load_op(view, action->stencil.load_action);
    wgpu_att->stencilStoreOp =
        _sg_wgpu_store_op(view, action->stencil.store_action);
  } else {
...
_SOKOL_PRIVATE void _sg_wgpu_begin_compute_pass(const sg_pass* pass) {
  ...
  _sg.wgpu.cpass_enc =
      wgpuCommandEncoderBeginComputePass(_sg.wgpu.cmd_enc, &wgpu_pass_desc);
  SOKOL_ASSERT(_sg.wgpu.cpass_enc);
...
_SOKOL_PRIVATE void _sg_wgpu_begin_render_pass(const sg_pass* pass) {
  ...
  if (atts) {
    SOKOL_ASSERT(atts->slot.state == SG_RESOURCESTATE_VALID);
    for (int i = 0; i < atts->cmn.num_colors; i++) {
      _sg_wgpu_init_color_att(&wgpu_color_att[i], &action->colors[i],
                              atts->wgpu.colors[i].view,
                              atts->wgpu.resolves[i].view);
    }
    ...
    if (atts->wgpu.depth_stencil.image) {
      _sg_wgpu_init_ds_att(&wgpu_ds_att, action,
                           atts->wgpu.depth_stencil.image->cmn.pixel_format,
                           atts->wgpu.depth_stencil.view);
      wgpu_pass_desc.depthStencilAttachment = &wgpu_ds_att;
    }
  } else {
    WGPUTextureView wgpu_color_view =
        (WGPUTextureView)swapchain->wgpu.render_view;
    WGPUTextureView wgpu_resolve_view =
        (WGPUTextureView)swapchain->wgpu.resolve_view;
    WGPUTextureView wgpu_depth_stencil_view =
        (WGPUTextureView)swapchain->wgpu.depth_stencil_view;
    _sg_wgpu_init_color_att(&wgpu_color_att[0], &action->colors[0],
                            wgpu_color_view, wgpu_resolve_view);
    wgpu_pass_desc.colorAttachmentCount = 1;
    wgpu_pass_desc.colorAttachments = &wgpu_color_att[0];
    if (wgpu_depth_stencil_view) {
      SOKOL_ASSERT(swapchain->depth_format > SG_PIXELFORMAT_NONE);
      _sg_wgpu_init_ds_att(&wgpu_ds_att, action, swapchain->depth_format,
                           wgpu_depth_stencil_view);
      wgpu_pass_desc.depthStencilAttachment = &wgpu_ds_att;
    }
  }
  _sg.wgpu.rpass_enc =
      wgpuCommandEncoderBeginRenderPass(_sg.wgpu.cmd_enc, &wgpu_pass_desc);
  SOKOL_ASSERT(_sg.wgpu.rpass_enc);
...
_SOKOL_PRIVATE void _sg_wgpu_commit(void) {
  ...
  WGPUCommandBuffer wgpu_cmd_buf =
      wgpuCommandEncoderFinish(_sg.wgpu.cmd_enc, &cmd_buf_desc);
...
_SOKOL_PRIVATE void _sg_wgpu_apply_viewport(int x,
                                            int y,
                                            int w,
                                            int h,
                                            bool origin_top_left) {
  ...
  // (but currently required because WebGPU insists that the viewport rectangle
  // must be fully contained inside the framebuffer, but this doesn't make any
  // sense, and also isn't required by the backend APIs)
  const _sg_recti_t clip =
      _sg_clipi(x, y, w, h, _sg.cur_pass.width, _sg.cur_pass.height);
  float xf = (float)clip.x;
  float yf =
      (float)(origin_top_left ? clip.y
                              : (_sg.cur_pass.height - (clip.y + clip.h)));
  float wf = (float)clip.w;
  float hf = (float)clip.h;
  wgpuRenderPassEncoderSetViewport(_sg.wgpu.rpass_enc, xf, yf, wf, hf, 0.0f,
                                   1.0f);
}

_SOKOL_PRIVATE void _sg_wgpu_apply_scissor_rect(int x,
                                                int y,
                                                int w,
                                                int h,
                                                bool origin_top_left) {
  ...
  uint32_t sy =
      (uint32_t)(origin_top_left ? clip.y
                                 : (_sg.cur_pass.height - (clip.y + clip.h)));
...
_SOKOL_PRIVATE void _sg_wgpu_set_ub_bindgroup(const _sg_shader_t* shd) {
  ...
  if (_sg.cur_pass.is_compute) {
    SOKOL_ASSERT(_sg.wgpu.cpass_enc);
    wgpuComputePassEncoderSetBindGroup(
        _sg.wgpu.cpass_enc, _SG_WGPU_UB_BINDGROUP_INDEX, shd->wgpu.bg_ub,
        shd->wgpu.ub_num_dynoffsets, dyn_offsets);
  } else {
    SOKOL_ASSERT(_sg.wgpu.rpass_enc);
    wgpuRenderPassEncoderSetBindGroup(
        _sg.wgpu.rpass_enc, _SG_WGPU_UB_BINDGROUP_INDEX, shd->wgpu.bg_ub,
        shd->wgpu.ub_num_dynoffsets, dyn_offsets);
  }
}
...
_SOKOL_PRIVATE void _sg_wgpu_apply_pipeline(_sg_pipeline_t* pip) {
  ...
  if (pip->cmn.is_compute) {
    ...
    wgpuRenderPassEncoderSetBlendConstant(_sg.wgpu.rpass_enc,
                                          &pip->wgpu.blend_color);
    wgpuRenderPassEncoderSetStencilReference(_sg.wgpu.rpass_enc,
                                             pip->cmn.stencil.ref);
  }
  // bind groups must be set because pipelines without uniform blocks or
  // resource bindings will still create 'empty' BindGroupLayouts
  _sg_wgpu_set_ub_bindgroup(pip->shader);
  _sg_wgpu_set_img_smp_sbuf_bindgroup(
      0);  // this will set the 'empty bind group'
}

_SOKOL_PRIVATE bool _sg_wgpu_apply_bindings(_sg_bindings_t* bnd) {
  SOKOL_ASSERT(bnd);
  SOKOL_ASSERT(bnd->pip->shader &&
               (bnd->pip->cmn.shader_id.id == bnd->pip->shader->slot.id));
  bool retval = true;
  if (!_sg.cur_pass.is_compute) {
...
_SOKOL_PRIVATE void _sg_wgpu_apply_uniforms(int ub_slot, const sg_range* data) {
  const uint32_t alignment =
      _sg.wgpu.limits.limits.minUniformBufferOffsetAlignment;
  SOKOL_ASSERT(_sg.wgpu.uniform.staging);
  SOKOL_ASSERT((ub_slot >= 0) && (ub_slot < SG_MAX_UNIFORMBLOCK_BINDSLOTS));
  SOKOL_ASSERT((_sg.wgpu.uniform.offset + data->size) <=
               _sg.wgpu.uniform.num_bytes);
  ...
  memcpy(_sg.wgpu.uniform.staging + _sg.wgpu.uniform.offset, data->ptr,
         data->size);
  _sg.wgpu.uniform.bind_offsets[ub_slot] = _sg.wgpu.uniform.offset;
  _sg.wgpu.uniform.offset = _sg_roundup_u32(
      _sg.wgpu.uniform.offset + (uint32_t)data->size, alignment);

  _sg_wgpu_set_ub_bindgroup(shd);
}

_SOKOL_PRIVATE void _sg_wgpu_draw(int base_element,
                                  int num_elements,
                                  int num_instances) {
  SOKOL_ASSERT(_sg.wgpu.rpass_enc);
  SOKOL_ASSERT(_sg.wgpu.cur_pipeline &&
               (_sg.wgpu.cur_pipeline->slot.id == _sg.wgpu.cur_pipeline_id.id));
  if (SG_INDEXTYPE_NONE != _sg.wgpu.cur_pipeline->cmn.index_type) {
    wgpuRenderPassEncoderDrawIndexed(_sg.wgpu.rpass_enc, (uint32_t)num_elements,
                                     (uint32_t)num_instances,
                                     (uint32_t)base_element, 0, 0);
  } else {
    wgpuRenderPassEncoderDraw(_sg.wgpu.rpass_enc, (uint32_t)num_elements,
                              (uint32_t)num_instances, (uint32_t)base_element,
                              0);
  }
}

_SOKOL_PRIVATE void _sg_wgpu_dispatch(int num_groups_x,
                                      int num_groups_y,
                                      int num_groups_z) {
  SOKOL_ASSERT(_sg.wgpu.cpass_enc);
  wgpuComputePassEncoderDispatchWorkgroups(
      _sg.wgpu.cpass_enc, (uint32_t)num_groups_x, (uint32_t)num_groups_y,
      (uint32_t)num_groups_z);
}

_SOKOL_PRIVATE void _sg_wgpu_update_buffer(_sg_buffer_t* buf,
                                           const sg_range* data) {
...
_SOKOL_PRIVATE void _sg_wgpu_append_buffer(_sg_buffer_t* buf,
                                           const sg_range* data,
                                           bool new_frame) {
  SOKOL_ASSERT(data && data->ptr && (data->size > 0));
...
_SOKOL_PRIVATE void _sg_wgpu_update_image(_sg_image_t* img,
                                          const sg_image_data* data) {
  SOKOL_ASSERT(img && data);
...
static inline sg_resource_state _sg_create_buffer(_sg_buffer_t* buf,
                                                  const sg_buffer_desc* desc) {
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state _sg_create_image(_sg_image_t* img,
                                                 const sg_image_desc* desc) {
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state _sg_create_sampler(
    _sg_sampler_t* smp,
    const sg_sampler_desc* desc) {
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state _sg_create_shader(_sg_shader_t* shd,
                                                  const sg_shader_desc* desc) {
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state _sg_create_pipeline(
    _sg_pipeline_t* pip,
    _sg_shader_t* shd,
    const sg_pipeline_desc* desc) {
#if defined(_SOKOL_ANY_GL)
...
static inline sg_resource_state _sg_create_attachments(
    ...
    const sg_attachments_desc* desc) {
#if defined(_SOKOL_ANY_GL)
  return _sg_gl_create_attachments(atts, color_images, resolve_images, ds_image,
                                   desc);
#elif defined(SOKOL_METAL)
  return _sg_mtl_create_attachments(atts, color_images, resolve_images,
                                    ds_image, desc);
#elif defined(SOKOL_D3D11)
  return _sg_d3d11_create_attachments(atts, color_images, resolve_images,
                                      ds_image, desc);
#elif defined(SOKOL_WGPU)
  return _sg_wgpu_create_attachments(atts, color_images, resolve_images,
                                     ds_image, desc);
#elif defined(SOKOL_DUMMY_BACKEND)
  return _sg_dummy_create_attachments(atts, color_images, resolve_images,
                                      ds_image, desc);
#else
#error("INVALID BACKEND");
#endif
...
static inline _sg_image_t* _sg_attachments_color_image(
    const _sg_attachments_t* atts,
    int index) {
#if defined(_SOKOL_ANY_GL)
...
static inline _sg_image_t* _sg_attachments_resolve_image(
    const _sg_attachments_t* atts,
    int index) {
#if defined(_SOKOL_ANY_GL)
...
static inline _sg_image_t* _sg_attachments_ds_image(
    const _sg_attachments_t* atts) {
#if defined(_SOKOL_ANY_GL)
...
static inline void _sg_apply_viewport(int x,
                                      int y,
                                      int w,
                                      int h,
                                      bool origin_top_left) {
...
static inline void _sg_apply_scissor_rect(int x,
                                          int y,
                                          int w,
                                          int h,
                                          bool origin_top_left) {
...
static inline void _sg_draw(int base_element,
                            int num_elements,
                            int num_instances) {
#if defined(_SOKOL_ANY_GL)
...
static inline void _sg_dispatch(int num_groups_x,
                                int num_groups_y,
                                int num_groups_z) {
#if defined(_SOKOL_ANY_GL)
...
static inline void _sg_append_buffer(_sg_buffer_t* buf,
                                     const sg_range* data,
                                     bool new_frame) {
#if defined(_SOKOL_ANY_GL)
...
static inline void _sg_update_image(_sg_image_t* img,
                                    const sg_image_data* data) {
#if defined(_SOKOL_ANY_GL)
...
_SOKOL_PRIVATE void _sg_reset_attachments_to_alloc_state(
    _sg_attachments_t* atts) {
  SOKOL_ASSERT(atts);
...
_SOKOL_PRIVATE void _sg_setup_pools(_sg_pools_t* p, const sg_desc* desc) {
  ...
  SOKOL_ASSERT((desc->buffer_pool_size > 0) &&
               (desc->buffer_pool_size < _SG_MAX_POOL_SIZE));
  _sg_pool_init(&p->buffer_pool, desc->buffer_pool_size);
  size_t buffer_pool_byte_size =
      sizeof(_sg_buffer_t) * (size_t)p->buffer_pool.size;
  p->buffers = (_sg_buffer_t*)_sg_malloc_clear(buffer_pool_byte_size);

  SOKOL_ASSERT((desc->image_pool_size > 0) &&
               (desc->image_pool_size < _SG_MAX_POOL_SIZE));
  _sg_pool_init(&p->image_pool, desc->image_pool_size);
  size_t image_pool_byte_size =
      sizeof(_sg_image_t) * (size_t)p->image_pool.size;
  p->images = (_sg_image_t*)_sg_malloc_clear(image_pool_byte_size);

  SOKOL_ASSERT((desc->sampler_pool_size > 0) &&
               (desc->sampler_pool_size < _SG_MAX_POOL_SIZE));
  _sg_pool_init(&p->sampler_pool, desc->sampler_pool_size);
  size_t sampler_pool_byte_size =
      sizeof(_sg_sampler_t) * (size_t)p->sampler_pool.size;
  p->samplers = (_sg_sampler_t*)_sg_malloc_clear(sampler_pool_byte_size);

  SOKOL_ASSERT((desc->shader_pool_size > 0) &&
               (desc->shader_pool_size < _SG_MAX_POOL_SIZE));
  _sg_pool_init(&p->shader_pool, desc->shader_pool_size);
  size_t shader_pool_byte_size =
      sizeof(_sg_shader_t) * (size_t)p->shader_pool.size;
  p->shaders = (_sg_shader_t*)_sg_malloc_clear(shader_pool_byte_size);

  SOKOL_ASSERT((desc->pipeline_pool_size > 0) &&
               (desc->pipeline_pool_size < _SG_MAX_POOL_SIZE));
  _sg_pool_init(&p->pipeline_pool, desc->pipeline_pool_size);
  size_t pipeline_pool_byte_size =
      sizeof(_sg_pipeline_t) * (size_t)p->pipeline_pool.size;
  p->pipelines = (_sg_pipeline_t*)_sg_malloc_clear(pipeline_pool_byte_size);

  SOKOL_ASSERT((desc->attachments_pool_size > 0) &&
               (desc->attachments_pool_size < _SG_MAX_POOL_SIZE));
  _sg_pool_init(&p->attachments_pool, desc->attachments_pool_size);
  size_t attachments_pool_byte_size =
      sizeof(_sg_attachments_t) * (size_t)p->attachments_pool.size;
  p->attachments =
      (_sg_attachments_t*)_sg_malloc_clear(attachments_pool_byte_size);
}

_SOKOL_PRIVATE void _sg_discard_pools(_sg_pools_t* p) {
  SOKOL_ASSERT(p);
  _sg_free(p->attachments);
  p->attachments = 0;
...
_SOKOL_PRIVATE _sg_buffer_t* _sg_buffer_at(const _sg_pools_t* p,
                                           uint32_t buf_id) {
...
_SOKOL_PRIVATE _sg_image_t* _sg_image_at(const _sg_pools_t* p,
                                         uint32_t img_id) {
...
_SOKOL_PRIVATE _sg_sampler_t* _sg_sampler_at(const _sg_pools_t* p,
                                             uint32_t smp_id) {
  SOKOL_ASSERT(p && (SG_INVALID_ID != smp_id));
  int slot_index = _sg_slot_index(smp_id);
  SOKOL_ASSERT((slot_index > _SG_INVALID_SLOT_INDEX) &&
               (slot_index < p->sampler_pool.size));
  return &p->samplers[slot_index];
}

_SOKOL_PRIVATE _sg_shader_t* _sg_shader_at(const _sg_pools_t* p,
                                           uint32_t shd_id) {
...
_SOKOL_PRIVATE _sg_pipeline_t* _sg_pipeline_at(const _sg_pools_t* p,
                                               uint32_t pip_id) {
...
_SOKOL_PRIVATE _sg_attachments_t* _sg_attachments_at(const _sg_pools_t* p,
                                                     uint32_t atts_id) {
  SOKOL_ASSERT(p && (SG_INVALID_ID != atts_id));
  int slot_index = _sg_slot_index(atts_id);
  SOKOL_ASSERT((slot_index > _SG_INVALID_SLOT_INDEX) &&
               (slot_index < p->attachments_pool.size));
  return &p->attachments[slot_index];
}

// returns pointer to resource with matching id check, may return 0
_SOKOL_PRIVATE _sg_buffer_t* _sg_lookup_buffer(const _sg_pools_t* p,
                                               uint32_t buf_id) {
...
_SOKOL_PRIVATE _sg_image_t* _sg_lookup_image(const _sg_pools_t* p,
                                             uint32_t img_id) {
...
_SOKOL_PRIVATE _sg_sampler_t* _sg_lookup_sampler(const _sg_pools_t* p,
                                                 uint32_t smp_id) {
...
_SOKOL_PRIVATE _sg_shader_t* _sg_lookup_shader(const _sg_pools_t* p,
                                               uint32_t shd_id) {
  SOKOL_ASSERT(p);
...
_SOKOL_PRIVATE _sg_pipeline_t* _sg_lookup_pipeline(const _sg_pools_t* p,
                                                   uint32_t pip_id) {
  SOKOL_ASSERT(p);
...
_SOKOL_PRIVATE _sg_attachments_t* _sg_lookup_attachments(const _sg_pools_t* p,
                                                         uint32_t atts_id) {
  SOKOL_ASSERT(p);
...
_SOKOL_PRIVATE bool _sg_validate_buffer_desc(const sg_buffer_desc* desc) {
  ...
  if (!injected && (desc->usage == SG_USAGE_IMMUTABLE)) {
    if (desc->data.ptr) {
      _SG_VALIDATE(desc->size == desc->data.size,
                   VALIDATE_BUFFERDESC_EXPECT_MATCHING_DATA_SIZE);
    } else {
      _SG_VALIDATE(desc->data.size == 0,
  ...
  if (desc->type == SG_BUFFERTYPE_STORAGEBUFFER) {
    _SG_VALIDATE(_sg.features.compute,
                 VALIDATE_BUFFERDESC_STORAGEBUFFER_SUPPORTED);
    _SG_VALIDATE(_sg_multiple_u64(desc->size, 4),
...
_SOKOL_PRIVATE void _sg_validate_image_data(const sg_image_data* data,
                                            sg_pixel_format fmt,
                                            int width,
                                            int height,
                                            int num_faces,
  ...
  for (int face_index = 0; face_index < num_faces; face_index++) {
    for (int mip_index = 0; mip_index < num_mips; mip_index++) {
      ...
      const int bytes_per_slice =
          _sg_surface_pitch(fmt, mip_width, mip_height, 1);
      const int expected_size = bytes_per_slice * num_slices;
      _SG_VALIDATE(
          expected_size == (int)data->subimage[face_index][mip_index].size,
          VALIDATE_IMAGEDATA_DATA_SIZE);
    }
  }
#endif
...
_SOKOL_PRIVATE bool _sg_validate_image_desc(const sg_image_desc* desc) {
  ...
  if (_sg_is_depth_or_depth_stencil_format(fmt)) {
    _SG_VALIDATE(desc->type != SG_IMAGETYPE_3D,
  ...
  if (desc->render_target) {
    ...
    _SG_VALIDATE(desc->data.subimage[0][0].ptr == 0,
                 VALIDATE_IMAGEDESC_RT_NO_DATA);
    if (desc->sample_count > 1) {
      _SG_VALIDATE(_sg.formats[fmt].msaa,
      ...
      _SG_VALIDATE(desc->type != SG_IMAGETYPE_CUBE,
    ...
    const bool is_compressed =
        _sg_is_compressed_pixel_format(desc->pixel_format);
    ...
    if (!injected && is_immutable) {
      ...
      _sg_validate_image_data(&desc->data, desc->pixel_format, desc->width,
                              desc->height,
...
_SOKOL_PRIVATE bool _sg_validate_slot_bits(_sg_u128_t bits,
                                           sg_shader_stage stage,
                                           uint8_t slot) {
...
_SOKOL_PRIVATE bool _sg_validate_shader_desc(const sg_shader_desc* desc) {
  ...
  SOKOL_ASSERT(desc);
  bool is_compute_shader = (desc->compute_func.source != 0) ||
                           (desc->compute_func.bytecode.ptr != 0);
  _sg_validate_begin();
  ...
  if (is_compute_shader) {
    _SG_VALIDATE(0 != desc->compute_func.source,
                 VALIDATE_SHADERDESC_COMPUTE_SOURCE);
  } else {
    _SG_VALIDATE(0 != desc->vertex_func.source,
                 VALIDATE_SHADERDESC_VERTEX_SOURCE);
    _SG_VALIDATE(0 != desc->fragment_func.source,
  ...
  if (is_compute_shader) {
    _SG_VALIDATE((0 != desc->compute_func.source) ||
                     (0 != desc->compute_func.bytecode.ptr),
                 VALIDATE_SHADERDESC_COMPUTE_SOURCE_OR_BYTECODE);
  } else {
    _SG_VALIDATE((0 != desc->vertex_func.source) ||
                     (0 != desc->vertex_func.bytecode.ptr),
                 VALIDATE_SHADERDESC_VERTEX_SOURCE_OR_BYTECODE);
    _SG_VALIDATE((0 != desc->fragment_func.source) ||
                     (0 != desc->fragment_func.bytecode.ptr),
...
#if defined(SOKOL_METAL)
  if (is_compute_shader) {
    _SG_VALIDATE(desc->mtl_threads_per_threadgroup.x > 0,
                 VALIDATE_SHADERDESC_METAL_THREADS_PER_THREADGROUP);
    _SG_VALIDATE(desc->mtl_threads_per_threadgroup.y > 0,
                 VALIDATE_SHADERDESC_METAL_THREADS_PER_THREADGROUP);
    _SG_VALIDATE(desc->mtl_threads_per_threadgroup.z > 0,
                 VALIDATE_SHADERDESC_METAL_THREADS_PER_THREADGROUP);
  }
#endif
  for (size_t i = 0; i < SG_MAX_VERTEX_ATTRIBUTES; i++) {
    if (desc->attrs[i].glsl_name) {
      _SG_VALIDATE(strlen(desc->attrs[i].glsl_name) < _SG_STRING_SIZE,
                   VALIDATE_SHADERDESC_ATTR_STRING_TOO_LONG);
    }
    if (desc->attrs[i].hlsl_sem_name) {
      _SG_VALIDATE(strlen(desc->attrs[i].hlsl_sem_name) < _SG_STRING_SIZE,
                   VALIDATE_SHADERDESC_ATTR_STRING_TOO_LONG);
    }
  }
  // if shader byte code, the size must also be provided
  if (0 != desc->vertex_func.bytecode.ptr) {
    _SG_VALIDATE(desc->vertex_func.bytecode.size > 0,
                 VALIDATE_SHADERDESC_NO_BYTECODE_SIZE);
  }
  if (0 != desc->fragment_func.bytecode.ptr) {
    _SG_VALIDATE(desc->fragment_func.bytecode.size > 0,
                 VALIDATE_SHADERDESC_NO_BYTECODE_SIZE);
  }
  if (0 != desc->compute_func.bytecode.ptr) {
    _SG_VALIDATE(desc->compute_func.bytecode.size > 0,
                 VALIDATE_SHADERDESC_NO_BYTECODE_SIZE);
  }
  ...
  for (size_t ub_idx = 0; ub_idx < SG_MAX_UNIFORMBLOCK_BINDSLOTS; ub_idx++) {
    ...
    _SG_VALIDATE(ub_desc->size > 0,
                 VALIDATE_SHADERDESC_UNIFORMBLOCK_SIZE_IS_ZERO);
#if defined(SOKOL_METAL)
    _SG_VALIDATE(
        ub_desc->msl_buffer_n < _SG_MTL_MAX_STAGE_UB_BINDINGS,
        VALIDATE_SHADERDESC_UNIFORMBLOCK_METAL_BUFFER_SLOT_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(msl_buf_bits, ub_desc->stage,
                                        ub_desc->msl_buffer_n),
                 VALIDATE_SHADERDESC_UNIFORMBLOCK_METAL_BUFFER_SLOT_COLLISION);
    msl_buf_bits = _sg_validate_set_slot_bit(msl_buf_bits, ub_desc->stage,
                                             ub_desc->msl_buffer_n);
#elif defined(SOKOL_D3D11)
    _SG_VALIDATE(ub_desc->hlsl_register_b_n < _SG_D3D11_MAX_STAGE_UB_BINDINGS,
                 VALIDATE_SHADERDESC_UNIFORMBLOCK_HLSL_REGISTER_B_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(hlsl_buf_bits, ub_desc->stage,
                                        ub_desc->hlsl_register_b_n),
                 VALIDATE_SHADERDESC_UNIFORMBLOCK_HLSL_REGISTER_B_COLLISION);
    hlsl_buf_bits = _sg_validate_set_slot_bit(hlsl_buf_bits, ub_desc->stage,
                                              ub_desc->hlsl_register_b_n);
#elif defined(SOKOL_WGPU)
    _SG_VALIDATE(
        ub_desc->wgsl_group0_binding_n < _SG_WGPU_MAX_UB_BINDGROUP_BIND_SLOTS,
        VALIDATE_SHADERDESC_UNIFORMBLOCK_WGSL_GROUP0_BINDING_OUT_OF_RANGE);
    _SG_VALIDATE(
        _sg_validate_slot_bits(wgsl_group0_bits, SG_SHADERSTAGE_NONE,
                               ub_desc->wgsl_group0_binding_n),
        VALIDATE_SHADERDESC_UNIFORMBLOCK_WGSL_GROUP0_BINDING_COLLISION);
    wgsl_group0_bits = _sg_validate_set_slot_bit(
        wgsl_group0_bits, SG_SHADERSTAGE_NONE, ub_desc->wgsl_group0_binding_n);
#endif
#if defined(_SOKOL_ANY_GL)
    ...
    for (size_t u_index = 0; u_index < SG_MAX_UNIFORMBLOCK_MEMBERS; u_index++) {
      const sg_glsl_shader_uniform* u_desc = &ub_desc->glsl_uniforms[u_index];
      if (u_desc->type != SG_UNIFORMTYPE_INVALID) {
        _SG_VALIDATE(uniforms_continuous,
                     VALIDATE_SHADERDESC_UNIFORMBLOCK_NO_CONT_MEMBERS);
        _SG_VALIDATE(u_desc->glsl_name,
                     VALIDATE_SHADERDESC_UNIFORMBLOCK_UNIFORM_GLSL_NAME);
        const int array_count = u_desc->array_count;
        _SG_VALIDATE(array_count > 0,
                     VALIDATE_SHADERDESC_UNIFORMBLOCK_ARRAY_COUNT);
        const uint32_t u_align =
            _sg_uniform_alignment(u_desc->type, array_count, ub_desc->layout);
        const uint32_t u_size =
            _sg_uniform_size(u_desc->type, array_count, ub_desc->layout);
        ...
        if (ub_desc->layout == SG_UNIFORMLAYOUT_STD140) {
          if (array_count > 1) {
            _SG_VALIDATE((u_desc->type == SG_UNIFORMTYPE_FLOAT4) ||
                             (u_desc->type == SG_UNIFORMTYPE_INT4) ||
                             (u_desc->type == SG_UNIFORMTYPE_MAT4),
                         VALIDATE_SHADERDESC_UNIFORMBLOCK_STD140_ARRAY_TYPE);
          }
        }
      } else {
    ...
    _SG_VALIDATE((size_t)uniform_offset == ub_desc->size,
                 VALIDATE_SHADERDESC_UNIFORMBLOCK_SIZE_MISMATCH);
    _SG_VALIDATE(num_uniforms > 0, VALIDATE_SHADERDESC_UNIFORMBLOCK_NO_MEMBERS);
#endif
  }

  for (size_t sbuf_idx = 0; sbuf_idx < SG_MAX_STORAGEBUFFER_BINDSLOTS;
       sbuf_idx++) {
    const sg_shader_storage_buffer* sbuf_desc =
        &desc->storage_buffers[sbuf_idx];
    if (sbuf_desc->stage == SG_SHADERSTAGE_NONE) {
      continue;
    }
#if defined(SOKOL_METAL)
    _SG_VALIDATE(
        (sbuf_desc->msl_buffer_n >= _SG_MTL_MAX_STAGE_UB_BINDINGS) &&
            (sbuf_desc->msl_buffer_n < _SG_MTL_MAX_STAGE_UB_SBUF_BINDINGS),
        VALIDATE_SHADERDESC_STORAGEBUFFER_METAL_BUFFER_SLOT_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(msl_buf_bits, sbuf_desc->stage,
                                        sbuf_desc->msl_buffer_n),
                 VALIDATE_SHADERDESC_STORAGEBUFFER_METAL_BUFFER_SLOT_COLLISION);
    msl_buf_bits = _sg_validate_set_slot_bit(msl_buf_bits, sbuf_desc->stage,
                                             sbuf_desc->msl_buffer_n);
#elif defined(SOKOL_D3D11)
    if (sbuf_desc->readonly) {
      _SG_VALIDATE(
          sbuf_desc->hlsl_register_t_n < _SG_D3D11_MAX_STAGE_SRV_BINDINGS,
          VALIDATE_SHADERDESC_STORAGEBUFFER_HLSL_REGISTER_T_OUT_OF_RANGE);
      _SG_VALIDATE(_sg_validate_slot_bits(hlsl_srv_bits, sbuf_desc->stage,
                                          sbuf_desc->hlsl_register_t_n),
                   VALIDATE_SHADERDESC_STORAGEBUFFER_HLSL_REGISTER_T_COLLISION);
      hlsl_srv_bits = _sg_validate_set_slot_bit(hlsl_srv_bits, sbuf_desc->stage,
                                                sbuf_desc->hlsl_register_t_n);
    } else {
      _SG_VALIDATE(
          sbuf_desc->hlsl_register_u_n < _SG_D3D11_MAX_STAGE_UAV_BINDINGS,
          VALIDATE_SHADERDESC_STORAGEBUFFER_HLSL_REGISTER_U_OUT_OF_RANGE);
      _SG_VALIDATE(_sg_validate_slot_bits(hlsl_uav_bits, sbuf_desc->stage,
                                          sbuf_desc->hlsl_register_u_n),
                   VALIDATE_SHADERDESC_STORAGEBUFFER_HLSL_REGISTER_U_COLLISION);
      hlsl_uav_bits = _sg_validate_set_slot_bit(hlsl_uav_bits, sbuf_desc->stage,
                                                sbuf_desc->hlsl_register_u_n);
    }
#elif defined(_SOKOL_ANY_GL)
    _SG_VALIDATE(sbuf_desc->glsl_binding_n < _SG_GL_MAX_SBUF_BINDINGS,
                 VALIDATE_SHADERDESC_STORAGEBUFFER_GLSL_BINDING_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(glsl_bnd_bits, SG_SHADERSTAGE_NONE,
                                        sbuf_desc->glsl_binding_n),
                 VALIDATE_SHADERDESC_STORAGEBUFFER_GLSL_BINDING_COLLISION);
    glsl_bnd_bits = _sg_validate_set_slot_bit(
        glsl_bnd_bits, SG_SHADERSTAGE_NONE, sbuf_desc->glsl_binding_n);
#elif defined(SOKOL_WGPU)
    _SG_VALIDATE(
        sbuf_desc->wgsl_group1_binding_n < _SG_WGPU_MAX_IMG_SMP_SBUF_BIND_SLOTS,
        VALIDATE_SHADERDESC_STORAGEBUFFER_WGSL_GROUP1_BINDING_OUT_OF_RANGE);
    _SG_VALIDATE(
        _sg_validate_slot_bits(wgsl_group1_bits, SG_SHADERSTAGE_NONE,
                               sbuf_desc->wgsl_group1_binding_n),
        VALIDATE_SHADERDESC_STORAGEBUFFER_WGSL_GROUP1_BINDING_COLLISION);
    wgsl_group1_bits =
        _sg_validate_set_slot_bit(wgsl_group1_bits, SG_SHADERSTAGE_NONE,
                                  sbuf_desc->wgsl_group1_binding_n);
#endif
  }
...
#if defined(SOKOL_METAL)
    _SG_VALIDATE(img_desc->msl_texture_n < _SG_MTL_MAX_STAGE_IMAGE_BINDINGS,
                 VALIDATE_SHADERDESC_IMAGE_METAL_TEXTURE_SLOT_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(msl_tex_bits, img_desc->stage,
                                        img_desc->msl_texture_n),
                 VALIDATE_SHADERDESC_IMAGE_METAL_TEXTURE_SLOT_COLLISION);
    msl_tex_bits = _sg_validate_set_slot_bit(msl_tex_bits, img_desc->stage,
                                             img_desc->msl_texture_n);
#elif defined(SOKOL_D3D11)
    _SG_VALIDATE(img_desc->hlsl_register_t_n < _SG_D3D11_MAX_STAGE_SRV_BINDINGS,
                 VALIDATE_SHADERDESC_IMAGE_HLSL_REGISTER_T_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(hlsl_srv_bits, img_desc->stage,
                                        img_desc->hlsl_register_t_n),
                 VALIDATE_SHADERDESC_IMAGE_HLSL_REGISTER_T_COLLISION);
    hlsl_srv_bits = _sg_validate_set_slot_bit(hlsl_srv_bits, img_desc->stage,
                                              img_desc->hlsl_register_t_n);
#elif defined(SOKOL_WGPU)
    _SG_VALIDATE(
        img_desc->wgsl_group1_binding_n < _SG_WGPU_MAX_IMG_SMP_SBUF_BIND_SLOTS,
        VALIDATE_SHADERDESC_IMAGE_WGSL_GROUP1_BINDING_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(wgsl_group1_bits, SG_SHADERSTAGE_NONE,
                                        img_desc->wgsl_group1_binding_n),
                 VALIDATE_SHADERDESC_IMAGE_WGSL_GROUP1_BINDING_COLLISION);
    wgsl_group1_bits = _sg_validate_set_slot_bit(
        wgsl_group1_bits, SG_SHADERSTAGE_NONE, img_desc->wgsl_group1_binding_n);
#endif
  }
...
#if defined(SOKOL_METAL)
    _SG_VALIDATE(smp_desc->msl_sampler_n < _SG_MTL_MAX_STAGE_SAMPLER_BINDINGS,
                 VALIDATE_SHADERDESC_SAMPLER_METAL_SAMPLER_SLOT_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(msl_smp_bits, smp_desc->stage,
                                        smp_desc->msl_sampler_n),
                 VALIDATE_SHADERDESC_SAMPLER_METAL_SAMPLER_SLOT_COLLISION);
    msl_smp_bits = _sg_validate_set_slot_bit(msl_smp_bits, smp_desc->stage,
                                             smp_desc->msl_sampler_n);
#elif defined(SOKOL_D3D11)
    _SG_VALIDATE(smp_desc->hlsl_register_s_n < _SG_D3D11_MAX_STAGE_SMP_BINDINGS,
                 VALIDATE_SHADERDESC_SAMPLER_HLSL_REGISTER_S_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(hlsl_smp_bits, smp_desc->stage,
                                        smp_desc->hlsl_register_s_n),
                 VALIDATE_SHADERDESC_SAMPLER_HLSL_REGISTER_S_COLLISION);
    hlsl_smp_bits = _sg_validate_set_slot_bit(hlsl_smp_bits, smp_desc->stage,
                                              smp_desc->hlsl_register_s_n);
#elif defined(SOKOL_WGPU)
    _SG_VALIDATE(
        smp_desc->wgsl_group1_binding_n < _SG_WGPU_MAX_IMG_SMP_SBUF_BIND_SLOTS,
        VALIDATE_SHADERDESC_SAMPLER_WGSL_GROUP1_BINDING_OUT_OF_RANGE);
    _SG_VALIDATE(_sg_validate_slot_bits(wgsl_group1_bits, SG_SHADERSTAGE_NONE,
                                        smp_desc->wgsl_group1_binding_n),
                 VALIDATE_SHADERDESC_SAMPLER_WGSL_GROUP1_BINDING_COLLISION);
    wgsl_group1_bits = _sg_validate_set_slot_bit(
        wgsl_group1_bits, SG_SHADERSTAGE_NONE, smp_desc->wgsl_group1_binding_n);
#endif
  }

  uint32_t ref_img_slot_mask = 0;
  uint32_t ref_smp_slot_mask = 0;
  for (size_t img_smp_idx = 0; img_smp_idx < SG_MAX_IMAGE_SAMPLER_PAIRS;
       img_smp_idx++) {
    const sg_shader_image_sampler_pair* img_smp_desc =
        &desc->image_sampler_pairs[img_smp_idx];
    if (img_smp_desc->stage == SG_SHADERSTAGE_NONE) {
      continue;
    }
#if defined(_SOKOL_ANY_GL)
    _SG_VALIDATE(img_smp_desc->glsl_name != 0,
                 VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_GLSL_NAME);
#endif
    const bool img_slot_in_range =
        img_smp_desc->image_slot < SG_MAX_IMAGE_BINDSLOTS;
    const bool smp_slot_in_range =
        img_smp_desc->sampler_slot < SG_MAX_SAMPLER_BINDSLOTS;
    _SG_VALIDATE(
        img_slot_in_range,
        VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_IMAGE_SLOT_OUT_OF_RANGE);
    _SG_VALIDATE(
    ...
    if (img_slot_in_range && smp_slot_in_range) {
      ...
      const sg_shader_sampler* smp_desc =
          &desc->samplers[img_smp_desc->sampler_slot];
      _SG_VALIDATE(img_desc->stage == img_smp_desc->stage,
                   VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_IMAGE_STAGE_MISMATCH);
      _SG_VALIDATE(
          smp_desc->stage == img_smp_desc->stage,
          VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_SAMPLER_STAGE_MISMATCH);
      const bool needs_nonfiltering =
          (img_desc->sample_type == SG_IMAGESAMPLETYPE_UINT) ||
          (img_desc->sample_type == SG_IMAGESAMPLETYPE_SINT) ||
          (img_desc->sample_type == SG_IMAGESAMPLETYPE_UNFILTERABLE_FLOAT);
      const bool needs_comparison =
          img_desc->sample_type == SG_IMAGESAMPLETYPE_DEPTH;
      ...
      if (needs_comparison) {
        _SG_VALIDATE(needs_comparison &&
                         (smp_desc->sampler_type == SG_SAMPLERTYPE_COMPARISON),
                     VALIDATE_SHADERDESC_COMPARISON_SAMPLER_REQUIRED);
      }
    }
  }
  // each image and sampler must be referenced by an image sampler
  _SG_VALIDATE(img_slot_mask == ref_img_slot_mask,
               VALIDATE_SHADERDESC_IMAGE_NOT_REFERENCED_BY_IMAGE_SAMPLER_PAIRS);
  _SG_VALIDATE(
      smp_slot_mask == ref_smp_slot_mask,
      VALIDATE_SHADERDESC_SAMPLER_NOT_REFERENCED_BY_IMAGE_SAMPLER_PAIRS);
...
_SOKOL_PRIVATE bool _sg_validate_pipeline_desc(const sg_pipeline_desc* desc) {
  ...
  if (shd) {
    _SG_VALIDATE(shd->slot.state == SG_RESOURCESTATE_VALID,
                 VALIDATE_PIPELINEDESC_SHADER);
    if (desc->compute) {
      _SG_VALIDATE(shd->cmn.is_compute,
                   VALIDATE_PIPELINEDESC_COMPUTE_SHADER_EXPECTED);
    } else {
      _SG_VALIDATE(!shd->cmn.is_compute,
                   VALIDATE_PIPELINEDESC_NO_COMPUTE_SHADER_EXPECTED);
      bool attrs_cont = true;
      for (size_t attr_index = 0; attr_index < SG_MAX_VERTEX_ATTRIBUTES;
           attr_index++) {
        const sg_vertex_attr_state* a_state = &desc->layout.attrs[attr_index];
        if (a_state->format == SG_VERTEXFORMAT_INVALID) {
          attrs_cont = false;
        ...
        SOKOL_ASSERT(a_state->buffer_index < SG_MAX_VERTEXBUFFER_BINDSLOTS);
        // vertex format must match expected shader attribute base type (if
        // provided)
        if (shd->cmn.attrs[attr_index].base_type !=
            SG_SHADERATTRBASETYPE_UNDEFINED) {
          if (_sg_vertexformat_basetype(a_state->format) !=
              shd->cmn.attrs[attr_index].base_type) {
            ...
            _SG_LOGMSG(VALIDATE_PIPELINEDESC_ATTR_BASETYPE_MISMATCH,
                       "shader attr base type:");
            _SG_LOGMSG(VALIDATE_PIPELINEDESC_ATTR_BASETYPE_MISMATCH,
                       _sg_shaderattrbasetype_to_string(
                           shd->cmn.attrs[attr_index].base_type));
          }
        }
#if defined(SOKOL_D3D11)
        // on D3D11, semantic names (and semantic indices) must be provided
        _SG_VALIDATE(!_sg_strempty(&shd->d3d11.attrs[attr_index].sem_name),
      ...
      for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
        if (shd->cmn.storage_buffers[i].stage != SG_SHADERSTAGE_NONE) {
          _SG_VALIDATE(shd->cmn.storage_buffers[i].readonly,
                       VALIDATE_PIPELINEDESC_SHADER_READONLY_STORAGEBUFFERS);
        }
      }
      for (int buf_index = 0; buf_index < SG_MAX_VERTEXBUFFER_BINDSLOTS;
           buf_index++) {
        const sg_vertex_buffer_layout_state* l_state =
            &desc->layout.buffers[buf_index];
        if (l_state->stride == 0) {
          continue;
        }
        _SG_VALIDATE(_sg_multiple_u64((uint64_t)l_state->stride, 4),
                     VALIDATE_PIPELINEDESC_LAYOUT_STRIDE4);
      }
    }
  }
  for (size_t color_index = 0; color_index < (size_t)desc->color_count;
       color_index++) {
...
_SOKOL_PRIVATE bool _sg_validate_attachments_desc(
    const sg_attachments_desc* desc) {
#if !defined(SOKOL_DEBUG)
  _SOKOL_UNUSED(desc);
  return true;
  ...
  for (int att_index = 0; att_index < SG_MAX_COLOR_ATTACHMENTS; att_index++) {
    ...
    if (0 != img) {
      _SG_VALIDATE(img->slot.state == SG_RESOURCESTATE_VALID,
                   VALIDATE_ATTACHMENTSDESC_IMAGE);
      _SG_VALIDATE(img->cmn.render_target,
                   VALIDATE_ATTACHMENTSDESC_IMAGE_NO_RT);
      _SG_VALIDATE(att->mip_level < img->cmn.num_mipmaps,
                   VALIDATE_ATTACHMENTSDESC_MIPLEVEL);
      if (img->cmn.type == SG_IMAGETYPE_CUBE) {
        _SG_VALIDATE(att->slice < 6, VALIDATE_ATTACHMENTSDESC_FACE);
      } else if (img->cmn.type == SG_IMAGETYPE_ARRAY) {
        _SG_VALIDATE(att->slice < img->cmn.num_slices,
                     VALIDATE_ATTACHMENTSDESC_LAYER);
      ...
      if (att_index == 0) {
        ...
        _SG_VALIDATE(
            color_height == _sg_miplevel_dim(img->cmn.height, att->mip_level),
            VALIDATE_ATTACHMENTSDESC_IMAGE_SIZES);
        _SG_VALIDATE(color_sample_count == img->cmn.sample_count,
                     VALIDATE_ATTACHMENTSDESC_IMAGE_SAMPLE_COUNTS);
      }
      _SG_VALIDATE(
          _sg_is_valid_rendertarget_color_format(img->cmn.pixel_format),
          VALIDATE_ATTACHMENTSDESC_COLOR_INV_PIXELFORMAT);
      ...
      if (res_att->image.id != SG_INVALID_ID) {
        ...
        _SG_VALIDATE(img->cmn.sample_count > 1,
                     VALIDATE_ATTACHMENTSDESC_RESOLVE_COLOR_IMAGE_MSAA);
        const _sg_image_t* res_img =
            _sg_lookup_image(&_sg.pools, res_att->image.id);
        _SG_VALIDATE(res_img, VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE);
        if (res_img != 0) {
          _SG_VALIDATE(res_img->slot.state == SG_RESOURCESTATE_VALID,
                       VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE);
          _SG_VALIDATE(res_img->cmn.render_target,
                       VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_NO_RT);
          _SG_VALIDATE(res_img->cmn.sample_count == 1,
                       VALIDATE_ATTACHMENTSDESC_RESOLVE_SAMPLE_COUNT);
          _SG_VALIDATE(res_att->mip_level < res_img->cmn.num_mipmaps,
                       VALIDATE_ATTACHMENTSDESC_RESOLVE_MIPLEVEL);
          if (res_img->cmn.type == SG_IMAGETYPE_CUBE) {
            _SG_VALIDATE(res_att->slice < 6,
                         VALIDATE_ATTACHMENTSDESC_RESOLVE_FACE);
          } else if (res_img->cmn.type == SG_IMAGETYPE_ARRAY) {
            _SG_VALIDATE(res_att->slice < res_img->cmn.num_slices,
                         VALIDATE_ATTACHMENTSDESC_RESOLVE_LAYER);
          } else if (res_img->cmn.type == SG_IMAGETYPE_3D) {
            _SG_VALIDATE(res_att->slice < res_img->cmn.num_slices,
                         VALIDATE_ATTACHMENTSDESC_RESOLVE_SLICE);
          }
          _SG_VALIDATE(img->cmn.pixel_format == res_img->cmn.pixel_format,
                       VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_FORMAT);
          _SG_VALIDATE(color_width == _sg_miplevel_dim(res_img->cmn.width,
                                                       res_att->mip_level),
                       VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_SIZES);
          _SG_VALIDATE(color_height == _sg_miplevel_dim(res_img->cmn.height,
                                                        res_att->mip_level),
                       VALIDATE_ATTACHMENTSDESC_RESOLVE_IMAGE_SIZES);
        }
      }
  ...
  if (desc->depth_stencil.image.id != SG_INVALID_ID) {
    ...
    if (img) {
      ...
      if (img->cmn.type == SG_IMAGETYPE_CUBE) {
        ...
        // NOTE: this can't actually happen because of
        // VALIDATE_IMAGEDESC_DEPTH_3D_IMAGE
        _SG_VALIDATE(att->slice < img->cmn.num_slices,
                     VALIDATE_ATTACHMENTSDESC_DEPTH_SLICE);
      }
      ...
      _SG_VALIDATE((color_height == -1) ||
                       (color_height ==
                        _sg_miplevel_dim(img->cmn.height, att->mip_level)),
                   VALIDATE_ATTACHMENTSDESC_DEPTH_IMAGE_SIZES);
      _SG_VALIDATE((color_sample_count == -1) ||
                       (color_sample_count == img->cmn.sample_count),
  ...
  _SG_VALIDATE(has_color_atts || has_depth_stencil_att,
               VALIDATE_ATTACHMENTSDESC_NO_ATTACHMENTS);
  return _sg_validate_end();
#endif
}
...
_SOKOL_PRIVATE bool _sg_validate_begin_pass(const sg_pass* pass) {
  ...
  const bool is_swapchain_pass =
      !is_compute_pass && (pass->attachments.id == SG_INVALID_ID);
...
// NOTE: the GL framebuffer handle may actually be 0
    ...
    if (pass->swapchain.sample_count > 1) {
      _SG_VALIDATE(pass->swapchain.metal.msaa_color_texture != 0,
                   VALIDATE_BEGINPASS_SWAPCHAIN_METAL_EXPECT_MSAACOLORTEXTURE);
    } else {
      _SG_VALIDATE(
          pass->swapchain.metal.msaa_color_texture == 0,
          VALIDATE_BEGINPASS_SWAPCHAIN_METAL_EXPECT_MSAACOLORTEXTURE_NOTSET);
    }
#elif defined(SOKOL_D3D11)
    _SG_VALIDATE(pass->swapchain.d3d11.render_view != 0,
                 VALIDATE_BEGINPASS_SWAPCHAIN_D3D11_EXPECT_RENDERVIEW);
    if (pass->swapchain.depth_format == SG_PIXELFORMAT_NONE) {
      _SG_VALIDATE(
          pass->swapchain.d3d11.depth_stencil_view == 0,
          VALIDATE_BEGINPASS_SWAPCHAIN_D3D11_EXPECT_DEPTHSTENCILVIEW_NOTSET);
    } else {
      _SG_VALIDATE(pass->swapchain.d3d11.depth_stencil_view != 0,
                   VALIDATE_BEGINPASS_SWAPCHAIN_D3D11_EXPECT_DEPTHSTENCILVIEW);
    }
    if (pass->swapchain.sample_count > 1) {
      _SG_VALIDATE(pass->swapchain.d3d11.resolve_view != 0,
                   VALIDATE_BEGINPASS_SWAPCHAIN_D3D11_EXPECT_RESOLVEVIEW);
    } else {
      _SG_VALIDATE(
          pass->swapchain.d3d11.resolve_view == 0,
          VALIDATE_BEGINPASS_SWAPCHAIN_D3D11_EXPECT_RESOLVEVIEW_NOTSET);
    }
#elif defined(SOKOL_WGPU)
    _SG_VALIDATE(pass->swapchain.wgpu.render_view != 0,
                 VALIDATE_BEGINPASS_SWAPCHAIN_WGPU_EXPECT_RENDERVIEW);
    if (pass->swapchain.depth_format == SG_PIXELFORMAT_NONE) {
      _SG_VALIDATE(
          pass->swapchain.wgpu.depth_stencil_view == 0,
          VALIDATE_BEGINPASS_SWAPCHAIN_WGPU_EXPECT_DEPTHSTENCILVIEW_NOTSET);
    } else {
      _SG_VALIDATE(pass->swapchain.wgpu.depth_stencil_view != 0,
                   VALIDATE_BEGINPASS_SWAPCHAIN_WGPU_EXPECT_DEPTHSTENCILVIEW);
    }
    if (pass->swapchain.sample_count > 1) {
      _SG_VALIDATE(pass->swapchain.wgpu.resolve_view != 0,
                   VALIDATE_BEGINPASS_SWAPCHAIN_WGPU_EXPECT_RESOLVEVIEW);
    } else {
      _SG_VALIDATE(pass->swapchain.wgpu.resolve_view == 0,
                   VALIDATE_BEGINPASS_SWAPCHAIN_WGPU_EXPECT_RESOLVEVIEW_NOTSET);
    ...
    const _sg_attachments_t* atts =
        _sg_lookup_attachments(&_sg.pools, pass->attachments.id);
    if (atts) {
      _SG_VALIDATE(atts->slot.state == SG_RESOURCESTATE_VALID,
                   VALIDATE_BEGINPASS_ATTACHMENTS_VALID);
      for (int i = 0; i < SG_MAX_COLOR_ATTACHMENTS; i++) {
        const _sg_attachment_common_t* color_att = &atts->cmn.colors[i];
        const _sg_image_t* color_img = _sg_attachments_color_image(atts, i);
        if (color_img) {
          _SG_VALIDATE(color_img->slot.state == SG_RESOURCESTATE_VALID,
                       VALIDATE_BEGINPASS_COLOR_ATTACHMENT_IMAGE);
          _SG_VALIDATE(color_img->slot.id == color_att->image_id.id,
                       VALIDATE_BEGINPASS_COLOR_ATTACHMENT_IMAGE);
        }
        ...
        if (resolve_img) {
          _SG_VALIDATE(resolve_img->slot.state == SG_RESOURCESTATE_VALID,
                       VALIDATE_BEGINPASS_RESOLVE_ATTACHMENT_IMAGE);
          _SG_VALIDATE(resolve_img->slot.id == resolve_att->image_id.id,
                       VALIDATE_BEGINPASS_RESOLVE_ATTACHMENT_IMAGE);
        }
      }
      ...
      if (ds_img) {
        ...
        _SG_VALIDATE(ds_img->slot.state == SG_RESOURCESTATE_VALID,
                     VALIDATE_BEGINPASS_DEPTHSTENCIL_ATTACHMENT_IMAGE);
        _SG_VALIDATE(ds_img->slot.id == att->image_id.id,
                     VALIDATE_BEGINPASS_DEPTHSTENCIL_ATTACHMENT_IMAGE);
      }
    } else {
  ...
  if (is_compute_pass || is_offscreen_pass) {
    ...
    _SG_VALIDATE(pass->swapchain.sample_count == 0,
                 VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_SAMPLECOUNT_NOTSET);
    _SG_VALIDATE(pass->swapchain.color_format == _SG_PIXELFORMAT_DEFAULT,
                 VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_COLORFORMAT_NOTSET);
    _SG_VALIDATE(pass->swapchain.depth_format == _SG_PIXELFORMAT_DEFAULT,
                 VALIDATE_BEGINPASS_SWAPCHAIN_EXPECT_DEPTHFORMAT_NOTSET);
#if defined(SOKOL_METAL)
    _SG_VALIDATE(
        pass->swapchain.metal.current_drawable == 0,
        VALIDATE_BEGINPASS_SWAPCHAIN_METAL_EXPECT_CURRENTDRAWABLE_NOTSET);
    _SG_VALIDATE(
        pass->swapchain.metal.depth_stencil_texture == 0,
        VALIDATE_BEGINPASS_SWAPCHAIN_METAL_EXPECT_DEPTHSTENCILTEXTURE_NOTSET);
    ...
    _SG_VALIDATE(pass->swapchain.wgpu.resolve_view == 0,
                 VALIDATE_BEGINPASS_SWAPCHAIN_WGPU_EXPECT_RESOLVEVIEW_NOTSET);
#elif defined(_SOKOL_ANY_GL)
    _SG_VALIDATE(pass->swapchain.gl.framebuffer == 0,
                 VALIDATE_BEGINPASS_SWAPCHAIN_GL_EXPECT_FRAMEBUFFER_NOTSET);
#endif
  }
  return _sg_validate_end();
...
_SOKOL_PRIVATE bool _sg_validate_apply_viewport(int x,
                                                int y,
                                                int width,
                                                int height,
                                                bool origin_top_left) {
  ...
  _sg_validate_begin();
  _SG_VALIDATE(_sg.cur_pass.in_pass && !_sg.cur_pass.is_compute,
               VALIDATE_AVP_RENDERPASS_EXPECTED);
  return _sg_validate_end();
#endif
}

_SOKOL_PRIVATE bool _sg_validate_apply_scissor_rect(int x,
                                                    int y,
                                                    int width,
                                                    int height,
                                                    bool origin_top_left) {
...
_SOKOL_PRIVATE bool _sg_validate_apply_pipeline(sg_pipeline pip_id) {
  ...
  _SG_VALIDATE(pip->slot.state == SG_RESOURCESTATE_VALID,
  ...
  _SG_VALIDATE(pip->shader->slot.id == pip->cmn.shader_id.id,
               VALIDATE_APIP_SHADER_EXISTS);
  _SG_VALIDATE(pip->shader->slot.state == SG_RESOURCESTATE_VALID,
  ...
  if (pip->cmn.is_compute) {
    ...
    if (_sg.cur_pass.atts_id.id != SG_INVALID_ID) {
      ...
      const _sg_attachments_t* atts = _sg.cur_pass.atts;
      SOKOL_ASSERT(atts);
      _SG_VALIDATE(atts->slot.id == _sg.cur_pass.atts_id.id,
                   VALIDATE_APIP_CURPASS_ATTACHMENTS_EXISTS);
      _SG_VALIDATE(atts->slot.state == SG_RESOURCESTATE_VALID,
                   VALIDATE_APIP_CURPASS_ATTACHMENTS_VALID);

      _SG_VALIDATE(pip->cmn.color_count == atts->cmn.num_colors,
                   VALIDATE_APIP_ATT_COUNT);
      for (int i = 0; i < pip->cmn.color_count; i++) {
        const _sg_image_t* att_img = _sg_attachments_color_image(atts, i);
        _SG_VALIDATE(
            pip->cmn.colors[i].pixel_format == att_img->cmn.pixel_format,
            VALIDATE_APIP_COLOR_FORMAT);
        _SG_VALIDATE(pip->cmn.sample_count == att_img->cmn.sample_count,
                     VALIDATE_APIP_SAMPLE_COUNT);
      }
      const _sg_image_t* att_dsimg = _sg_attachments_ds_image(atts);
      if (att_dsimg) {
        _SG_VALIDATE(pip->cmn.depth.pixel_format == att_dsimg->cmn.pixel_format,
      ...
      _SG_VALIDATE(
          pip->cmn.colors[0].pixel_format == _sg.cur_pass.swapchain.color_fmt,
          VALIDATE_APIP_COLOR_FORMAT);
      _SG_VALIDATE(
          pip->cmn.depth.pixel_format == _sg.cur_pass.swapchain.depth_fmt,
          VALIDATE_APIP_DEPTH_FORMAT);
      _SG_VALIDATE(pip->cmn.sample_count == _sg.cur_pass.swapchain.sample_count,
                   VALIDATE_APIP_SAMPLE_COUNT);
    }
  }
  return _sg_validate_end();
...
_SOKOL_PRIVATE bool _sg_validate_apply_bindings(const sg_bindings* bindings) {
  ...
  const _sg_pipeline_t* pip =
      _sg_lookup_pipeline(&_sg.pools, _sg.cur_pipeline.id);
  ...
  if (_sg.cur_pass.is_compute) {
    for (size_t i = 0; i < SG_MAX_VERTEXBUFFER_BINDSLOTS; i++) {
      _SG_VALIDATE(bindings->vertex_buffers[i].id == SG_INVALID_ID,
    ...
    for (size_t i = 0; i < SG_MAX_VERTEXBUFFER_BINDSLOTS; i++) {
      if (pip->cmn.vertex_buffer_layout_active[i]) {
        ...
        // buffers in vertex-buffer-slots must be of type
        // SG_BUFFERTYPE_VERTEXBUFFER
        if (bindings->vertex_buffers[i].id != SG_INVALID_ID) {
          const _sg_buffer_t* buf =
              _sg_lookup_buffer(&_sg.pools, bindings->vertex_buffers[i].id);
          _SG_VALIDATE(buf != 0, VALIDATE_ABND_VB_EXISTS);
          if (buf && buf->slot.state == SG_RESOURCESTATE_VALID) {
            _SG_VALIDATE(SG_BUFFERTYPE_VERTEXBUFFER == buf->cmn.type,
  ...
  if (_sg.cur_pass.is_compute) {
    _SG_VALIDATE(bindings->index_buffer.id == SG_INVALID_ID,
                 VALIDATE_ABND_COMPUTE_EXPECTED_NO_IB);
  } else {
    ...
    if (bindings->index_buffer.id != SG_INVALID_ID) {
      ...
      const _sg_buffer_t* buf =
          _sg_lookup_buffer(&_sg.pools, bindings->index_buffer.id);
      _SG_VALIDATE(buf != 0, VALIDATE_ABND_IB_EXISTS);
      if (buf && buf->slot.state == SG_RESOURCESTATE_VALID) {
        _SG_VALIDATE(SG_BUFFERTYPE_INDEXBUFFER == buf->cmn.type,
  ...
  for (size_t i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
    if (shd->cmn.images[i].stage != SG_SHADERSTAGE_NONE) {
      _SG_VALIDATE(bindings->images[i].id != SG_INVALID_ID,
                   VALIDATE_ABND_EXPECTED_IMAGE_BINDING);
      if (bindings->images[i].id != SG_INVALID_ID) {
        const _sg_image_t* img =
            _sg_lookup_image(&_sg.pools, bindings->images[i].id);
        _SG_VALIDATE(img != 0, VALIDATE_ABND_IMG_EXISTS);
        if (img && img->slot.state == SG_RESOURCESTATE_VALID) {
          _SG_VALIDATE(img->cmn.type == shd->cmn.images[i].image_type,
                       VALIDATE_ABND_IMAGE_TYPE_MISMATCH);
          ...
          const _sg_pixelformat_info_t* info =
              &_sg.formats[img->cmn.pixel_format];
          switch (shd->cmn.images[i].sample_type) {
            case SG_IMAGESAMPLETYPE_FLOAT:
              _SG_VALIDATE(info->filter,
  ...
  for (size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
    if (shd->cmn.samplers[i].stage != SG_SHADERSTAGE_NONE) {
      _SG_VALIDATE(bindings->samplers[i].id != SG_INVALID_ID,
                   VALIDATE_ABND_EXPECTED_SAMPLER_BINDING);
      if (bindings->samplers[i].id != SG_INVALID_ID) {
        const _sg_sampler_t* smp =
            _sg_lookup_sampler(&_sg.pools, bindings->samplers[i].id);
        _SG_VALIDATE(smp != 0, VALIDATE_ABND_SMP_EXISTS);
        if (smp) {
          if (shd->cmn.samplers[i].sampler_type == SG_SAMPLERTYPE_COMPARISON) {
            _SG_VALIDATE(smp->cmn.compare != SG_COMPAREFUNC_NEVER,
                         VALIDATE_ABND_UNEXPECTED_SAMPLER_COMPARE_NEVER);
          } else {
            _SG_VALIDATE(smp->cmn.compare == SG_COMPAREFUNC_NEVER,
                         VALIDATE_ABND_EXPECTED_SAMPLER_COMPARE_NEVER);
          }
          if (shd->cmn.samplers[i].sampler_type ==
              SG_SAMPLERTYPE_NONFILTERING) {
            const bool nonfiltering =
                (smp->cmn.min_filter != SG_FILTER_LINEAR) &&
                (smp->cmn.mag_filter != SG_FILTER_LINEAR) &&
                (smp->cmn.mipmap_filter != SG_FILTER_LINEAR);
            _SG_VALIDATE(nonfiltering,
                         VALIDATE_ABND_EXPECTED_NONFILTERING_SAMPLER);
          }
        }
      }
...
_SOKOL_PRIVATE bool _sg_validate_apply_uniforms(int ub_slot,
                                                const sg_range* data) {
#if !defined(SOKOL_DEBUG)
  _SOKOL_UNUSED(ub_slot);
  _SOKOL_UNUSED(data);
  ...
  const _sg_shader_t* shd = pip->shader;
  _SG_VALIDATE(shd->cmn.uniform_blocks[ub_slot].stage != SG_SHADERSTAGE_NONE,
               VALIDATE_AU_NO_UNIFORMBLOCK_AT_SLOT);
  _SG_VALIDATE(data->size == shd->cmn.uniform_blocks[ub_slot].size,
               VALIDATE_AU_SIZE);
...
_SOKOL_PRIVATE bool _sg_validate_draw(int base_element,
                                      int num_elements,
                                      int num_instances) {
#if !defined(SOKOL_DEBUG)
  _SOKOL_UNUSED(base_element);
  ...
  _SG_VALIDATE(
      _sg.required_bindings_and_uniforms == _sg.applied_bindings_and_uniforms,
      VALIDATE_DRAW_REQUIRED_BINDINGS_OR_UNIFORMS_MISSING);
  return _sg_validate_end();
#endif
}

_SOKOL_PRIVATE bool _sg_validate_dispatch(int num_groups_x,
                                          int num_groups_y,
                                          int num_groups_z) {
#if !defined(SOKOL_DEBUG)
  _SOKOL_UNUSED(num_groups_x);
  ...
  _SG_VALIDATE((num_groups_x >= 0) && (num_groups_x < (1 << 16)),
               VALIDATE_DISPATCH_NUMGROUPSX);
  _SG_VALIDATE((num_groups_y >= 0) && (num_groups_y < (1 << 16)),
               VALIDATE_DISPATCH_NUMGROUPSY);
  _SG_VALIDATE((num_groups_z >= 0) && (num_groups_z < (1 << 16)),
...
_SOKOL_PRIVATE bool _sg_validate_update_buffer(const _sg_buffer_t* buf,
                                               const sg_range* data) {
#if !defined(SOKOL_DEBUG)
  _SOKOL_UNUSED(buf);
  _SOKOL_UNUSED(data);
  ...
  _SG_VALIDATE(buf->cmn.update_frame_index != _sg.frame_index,
               VALIDATE_UPDATEBUF_ONCE);
  _SG_VALIDATE(buf->cmn.append_frame_index != _sg.frame_index,
               VALIDATE_UPDATEBUF_APPEND);
  return _sg_validate_end();
#endif
}

_SOKOL_PRIVATE bool _sg_validate_append_buffer(const _sg_buffer_t* buf,
                                               const sg_range* data) {
#if !defined(SOKOL_DEBUG)
  _SOKOL_UNUSED(buf);
  _SOKOL_UNUSED(data);
  ...
  _SG_VALIDATE(buf->cmn.size >= (buf->cmn.append_pos + (int)data->size),
               VALIDATE_APPENDBUF_SIZE);
  _SG_VALIDATE(buf->cmn.update_frame_index != _sg.frame_index,
...
_SOKOL_PRIVATE bool _sg_validate_update_image(const _sg_image_t* img,
                                              const sg_image_data* data) {
#if !defined(SOKOL_DEBUG)
  _SOKOL_UNUSED(img);
  _SOKOL_UNUSED(data);
  ...
  _SG_VALIDATE(img->cmn.upd_frame_index != _sg.frame_index,
               VALIDATE_UPDIMG_ONCE);
...
_sg_buffer_desc_defaults(const sg_buffer_desc* desc) {
...
_SOKOL_PRIVATE sg_image_desc
_sg_image_desc_defaults(const sg_image_desc* desc) {
  ...
  if (desc->render_target) {
    def.pixel_format =
        _sg_def(def.pixel_format, _sg.desc.environment.defaults.color_format);
    def.sample_count =
        _sg_def(def.sample_count, _sg.desc.environment.defaults.sample_count);
  } else {
...
_SOKOL_PRIVATE sg_sampler_desc
_sg_sampler_desc_defaults(const sg_sampler_desc* desc) {
...
_sg_shader_desc_defaults(const sg_shader_desc* desc) {
  ...
  if (def.vertex_func.source) {
    def.vertex_func.d3d11_target =
        _sg_def(def.vertex_func.d3d11_target, "vs_4_0");
  }
  if (def.fragment_func.source) {
    def.fragment_func.d3d11_target =
        _sg_def(def.fragment_func.d3d11_target, "ps_4_0");
  }
  if (def.compute_func.source) {
    def.compute_func.d3d11_target =
        _sg_def(def.fragment_func.d3d11_target, "cs_5_0");
  }
#endif
  def.mtl_threads_per_threadgroup.y =
      _sg_def(desc->mtl_threads_per_threadgroup.y, 1);
  def.mtl_threads_per_threadgroup.z =
      _sg_def(desc->mtl_threads_per_threadgroup.z, 1);
  for (size_t ub_index = 0; ub_index < SG_MAX_UNIFORMBLOCK_BINDSLOTS;
       ub_index++) {
    sg_shader_uniform_block* ub_desc = &def.uniform_blocks[ub_index];
    if (ub_desc->stage != SG_SHADERSTAGE_NONE) {
      ub_desc->layout = _sg_def(ub_desc->layout, SG_UNIFORMLAYOUT_NATIVE);
      for (size_t u_index = 0; u_index < SG_MAX_UNIFORMBLOCK_MEMBERS;
           u_index++) {
  ...
  for (size_t img_index = 0; img_index < SG_MAX_IMAGE_BINDSLOTS; img_index++) {
    ...
    if (img_desc->stage != SG_SHADERSTAGE_NONE) {
      ...
      img_desc->sample_type =
          _sg_def(img_desc->sample_type, SG_IMAGESAMPLETYPE_FLOAT);
    }
  }
  for (size_t smp_index = 0; smp_index < SG_MAX_SAMPLER_BINDSLOTS;
       smp_index++) {
    sg_shader_sampler* smp_desc = &def.samplers[smp_index];
    if (smp_desc->stage != SG_SHADERSTAGE_NONE) {
      smp_desc->sampler_type =
          _sg_def(smp_desc->sampler_type, SG_SAMPLERTYPE_FILTERING);
    }
  }
  return def;
}

_SOKOL_PRIVATE sg_pipeline_desc
_sg_pipeline_desc_defaults(const sg_pipeline_desc* desc) {
  ...
  def.stencil.front.compare =
      _sg_def(def.stencil.front.compare, SG_COMPAREFUNC_ALWAYS);
  def.stencil.front.fail_op =
      _sg_def(def.stencil.front.fail_op, SG_STENCILOP_KEEP);
  def.stencil.front.depth_fail_op =
      _sg_def(def.stencil.front.depth_fail_op, SG_STENCILOP_KEEP);
  def.stencil.front.pass_op =
      _sg_def(def.stencil.front.pass_op, SG_STENCILOP_KEEP);
  def.stencil.back.compare =
      _sg_def(def.stencil.back.compare, SG_COMPAREFUNC_ALWAYS);
  def.stencil.back.fail_op =
      _sg_def(def.stencil.back.fail_op, SG_STENCILOP_KEEP);
  def.stencil.back.depth_fail_op =
      _sg_def(def.stencil.back.depth_fail_op, SG_STENCILOP_KEEP);
  def.stencil.back.pass_op =
      _sg_def(def.stencil.back.pass_op, SG_STENCILOP_KEEP);

  def.depth.compare = _sg_def(def.depth.compare, SG_COMPAREFUNC_ALWAYS);
  def.depth.pixel_format = _sg_def(def.depth.pixel_format,
                                   _sg.desc.environment.defaults.depth_format);
  ...
  for (int i = 0; i < def.color_count; i++) {
    ...
    cs->pixel_format =
        _sg_def(cs->pixel_format, _sg.desc.environment.defaults.color_format);
  ...
  for (int attr_index = 0; attr_index < SG_MAX_VERTEX_ATTRIBUTES;
    ...
    SOKOL_ASSERT(a_state->buffer_index < SG_MAX_VERTEXBUFFER_BINDSLOTS);
    sg_vertex_buffer_layout_state* l_state =
        &def.layout.buffers[a_state->buffer_index];
  ...
  for (int attr_index = 0; attr_index < SG_MAX_VERTEX_ATTRIBUTES;
    ...
    auto_offset[a_state->buffer_index] +=
        _sg_vertexformat_bytesize(a_state->format);
  }
...
_SOKOL_PRIVATE sg_attachments_desc
_sg_attachments_desc_defaults(const sg_attachments_desc* desc) {
...
_SOKOL_PRIVATE sg_buffer _sg_alloc_buffer(void) {
  ...
  if (_SG_INVALID_SLOT_INDEX != slot_index) {
    res.id = _sg_slot_alloc(&_sg.pools.buffer_pool,
                            &_sg.pools.buffers[slot_index].slot, slot_index);
  } else {
    res.id = SG_INVALID_ID;
...
_SOKOL_PRIVATE sg_image _sg_alloc_image(void) {
  ...
  if (_SG_INVALID_SLOT_INDEX != slot_index) {
    res.id = _sg_slot_alloc(&_sg.pools.image_pool,
                            &_sg.pools.images[slot_index].slot, slot_index);
  } else {
    res.id = SG_INVALID_ID;
...
_SOKOL_PRIVATE sg_sampler _sg_alloc_sampler(void) {
  ...
  if (_SG_INVALID_SLOT_INDEX != slot_index) {
    res.id = _sg_slot_alloc(&_sg.pools.sampler_pool,
                            &_sg.pools.samplers[slot_index].slot, slot_index);
  } else {
    res.id = SG_INVALID_ID;
...
_SOKOL_PRIVATE sg_shader _sg_alloc_shader(void) {
  ...
  if (_SG_INVALID_SLOT_INDEX != slot_index) {
    res.id = _sg_slot_alloc(&_sg.pools.shader_pool,
                            &_sg.pools.shaders[slot_index].slot, slot_index);
  } else {
    res.id = SG_INVALID_ID;
...
_SOKOL_PRIVATE sg_pipeline _sg_alloc_pipeline(void) {
  ...
  if (_SG_INVALID_SLOT_INDEX != slot_index) {
    res.id = _sg_slot_alloc(&_sg.pools.pipeline_pool,
                            &_sg.pools.pipelines[slot_index].slot, slot_index);
  } else {
    res.id = SG_INVALID_ID;
...
_SOKOL_PRIVATE sg_attachments _sg_alloc_attachments(void) {
  ...
  if (_SG_INVALID_SLOT_INDEX != slot_index) {
    res.id =
        _sg_slot_alloc(&_sg.pools.attachments_pool,
                       &_sg.pools.attachments[slot_index].slot, slot_index);
  } else {
    res.id = SG_INVALID_ID;
...
_SOKOL_PRIVATE void _sg_dealloc_attachments(_sg_attachments_t* atts) {
  SOKOL_ASSERT(atts && (atts->slot.state == SG_RESOURCESTATE_ALLOC) &&
               (atts->slot.id != SG_INVALID_ID));
  _sg_pool_free_index(&_sg.pools.attachments_pool,
                      _sg_slot_index(atts->slot.id));
  _sg_slot_reset(&atts->slot);
}

_SOKOL_PRIVATE void _sg_init_buffer(_sg_buffer_t* buf,
                                    const sg_buffer_desc* desc) {
  SOKOL_ASSERT(buf && (buf->slot.state == SG_RESOURCESTATE_ALLOC));
  SOKOL_ASSERT(desc);
...
_SOKOL_PRIVATE void _sg_init_image(_sg_image_t* img,
                                   const sg_image_desc* desc) {
  SOKOL_ASSERT(img && (img->slot.state == SG_RESOURCESTATE_ALLOC));
  SOKOL_ASSERT(desc);
...
_SOKOL_PRIVATE void _sg_init_sampler(_sg_sampler_t* smp,
                                     const sg_sampler_desc* desc) {
  SOKOL_ASSERT(smp && (smp->slot.state == SG_RESOURCESTATE_ALLOC));
  SOKOL_ASSERT(desc);
  ...
  SOKOL_ASSERT((smp->slot.state == SG_RESOURCESTATE_VALID) ||
               (smp->slot.state == SG_RESOURCESTATE_FAILED));
}

_SOKOL_PRIVATE void _sg_init_shader(_sg_shader_t* shd,
                                    const sg_shader_desc* desc) {
  SOKOL_ASSERT(shd && (shd->slot.state == SG_RESOURCESTATE_ALLOC));
  SOKOL_ASSERT(desc);
...
_SOKOL_PRIVATE void _sg_init_pipeline(_sg_pipeline_t* pip,
                                      const sg_pipeline_desc* desc) {
  SOKOL_ASSERT(pip && (pip->slot.state == SG_RESOURCESTATE_ALLOC));
  SOKOL_ASSERT(desc);
...
_SOKOL_PRIVATE void _sg_init_attachments(_sg_attachments_t* atts,
                                         const sg_attachments_desc* desc) {
  SOKOL_ASSERT(atts && atts->slot.state == SG_RESOURCESTATE_ALLOC);
  SOKOL_ASSERT(desc);
  if (_sg_validate_attachments_desc(desc)) {
    ...
    for (int i = 0; i < SG_MAX_COLOR_ATTACHMENTS; i++) {
      if (desc->colors[i].image.id) {
        color_images[i] =
            _sg_lookup_image(&_sg.pools, desc->colors[i].image.id);
        if (!(color_images[i] &&
              color_images[i]->slot.state == SG_RESOURCESTATE_VALID)) {
      ...
      if (desc->resolves[i].image.id) {
        resolve_images[i] =
            _sg_lookup_image(&_sg.pools, desc->resolves[i].image.id);
        if (!(resolve_images[i] &&
              resolve_images[i]->slot.state == SG_RESOURCESTATE_VALID)) {
          atts->slot.state = SG_RESOURCESTATE_FAILED;
          return;
        }
      }
    ...
    atts->slot.state = _sg_create_attachments(atts, color_images,
                                              resolve_images, ds_image, desc);
  } else {
    atts->slot.state = SG_RESOURCESTATE_FAILED;
  }
  SOKOL_ASSERT((atts->slot.state == SG_RESOURCESTATE_VALID) ||
...
_SOKOL_PRIVATE void _sg_uninit_sampler(_sg_sampler_t* smp) {
  SOKOL_ASSERT(smp && ((smp->slot.state == SG_RESOURCESTATE_VALID) ||
                       (smp->slot.state == SG_RESOURCESTATE_FAILED)));
...
_SOKOL_PRIVATE void _sg_uninit_attachments(_sg_attachments_t* atts) {
  SOKOL_ASSERT(atts && ((atts->slot.state == SG_RESOURCESTATE_VALID) ||
                        (atts->slot.state == SG_RESOURCESTATE_FAILED)));
...
_SOKOL_PRIVATE void _sg_setup_commit_listeners(const sg_desc* desc) {
  ...
  const size_t size =
      (size_t)_sg.commit_listeners.num * sizeof(sg_commit_listener);
...
_SOKOL_PRIVATE bool _sg_add_commit_listener(
    const sg_commit_listener* new_listener) {
  ...
  for (int i = 0; i < _sg.commit_listeners.upper; i++) {
    ...
    if ((slot->func == new_listener->func) &&
        (slot->user_data == new_listener->user_data)) {
...
_SOKOL_PRIVATE bool _sg_remove_commit_listener(
...
_SOKOL_PRIVATE void _sg_setup_compute(const sg_desc* desc) {
  ...
  const uint32_t max_tracked_sbufs =
      (uint32_t)desc->max_dispatch_calls_per_pass *
      SG_MAX_STORAGEBUFFER_BINDSLOTS;
...
_SOKOL_PRIVATE void _sg_compute_pass_track_storage_buffer(_sg_buffer_t* sbuf,
                                                          bool readonly) {
  SOKOL_ASSERT(sbuf);
  if (!readonly) {
...
#elif defined(SOKOL_METAL) || defined(SOKOL_D3D11)
  res.environment.defaults.color_format =
      _sg_def(res.environment.defaults.color_format, SG_PIXELFORMAT_BGRA8);
#else
  res.environment.defaults.color_format =
      _sg_def(res.environment.defaults.color_format, SG_PIXELFORMAT_RGBA8);
#endif
  res.environment.defaults.depth_format = _sg_def(
      res.environment.defaults.depth_format, SG_PIXELFORMAT_DEPTH_STENCIL);
  res.environment.defaults.sample_count =
      _sg_def(res.environment.defaults.sample_count, 1);
  res.buffer_pool_size =
      _sg_def(res.buffer_pool_size, _SG_DEFAULT_BUFFER_POOL_SIZE);
  res.image_pool_size =
      _sg_def(res.image_pool_size, _SG_DEFAULT_IMAGE_POOL_SIZE);
  res.sampler_pool_size =
      _sg_def(res.sampler_pool_size, _SG_DEFAULT_SAMPLER_POOL_SIZE);
  res.shader_pool_size =
      _sg_def(res.shader_pool_size, _SG_DEFAULT_SHADER_POOL_SIZE);
  res.pipeline_pool_size =
      _sg_def(res.pipeline_pool_size, _SG_DEFAULT_PIPELINE_POOL_SIZE);
  res.attachments_pool_size =
      _sg_def(res.attachments_pool_size, _SG_DEFAULT_ATTACHMENTS_POOL_SIZE);
  res.uniform_buffer_size =
      _sg_def(res.uniform_buffer_size, _SG_DEFAULT_UB_SIZE);
  res.max_dispatch_calls_per_pass = _sg_def(
      res.max_dispatch_calls_per_pass, _SG_DEFAULT_MAX_DISPATCH_CALLS_PER_PASS);
  res.max_commit_listeners =
      _sg_def(res.max_commit_listeners, _SG_DEFAULT_MAX_COMMIT_LISTENERS);
  res.wgpu_bindgroups_cache_size = _sg_def(
      res.wgpu_bindgroups_cache_size, _SG_DEFAULT_WGPU_BINDGROUP_CACHE_SIZE);
  return res;
}
...
_SOKOL_PRIVATE sg_pass _sg_pass_defaults(const sg_pass* pass) {
  ...
  if (!res.compute) {
    if (res.compute && res.attachments.id == SG_INVALID_ID) {
      // this is a swapchain-pass
      res.swapchain.sample_count =
          _sg_def(res.swapchain.sample_count,
                  _sg.desc.environment.defaults.sample_count);
      res.swapchain.color_format =
          _sg_def(res.swapchain.color_format,
                  _sg.desc.environment.defaults.color_format);
      res.swapchain.depth_format =
          _sg_def(res.swapchain.depth_format,
                  _sg.desc.environment.defaults.depth_format);
    }
...
SOKOL_API_IMPL int sg_query_row_pitch(sg_pixel_format fmt,
                                      int width,
                                      int row_align_bytes) {
  SOKOL_ASSERT(_sg.valid);
  SOKOL_ASSERT(width > 0);
  SOKOL_ASSERT((row_align_bytes > 0) && _sg_ispow2(row_align_bytes));
  SOKOL_ASSERT(((int)fmt > SG_PIXELFORMAT_NONE) &&
               ((int)fmt < _SG_PIXELFORMAT_NUM));
  return _sg_row_pitch(fmt, width, row_align_bytes);
}

SOKOL_API_IMPL int sg_query_surface_pitch(sg_pixel_format fmt,
                                          int width,
                                          int height,
                                          int row_align_bytes) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL void sg_init_sampler(sg_sampler smp_id,
                                    const sg_sampler_desc* desc) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL void sg_init_shader(sg_shader shd_id,
                                   const sg_shader_desc* desc) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL void sg_init_pipeline(sg_pipeline pip_id,
                                     const sg_pipeline_desc* desc) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL void sg_init_attachments(sg_attachments atts_id,
                                        const sg_attachments_desc* desc) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL void sg_uninit_sampler(sg_sampler smp_id) {
  ...
  _sg_sampler_t* smp = _sg_lookup_sampler(&_sg.pools, smp_id.id);
  if (smp) {
    if ((smp->slot.state == SG_RESOURCESTATE_VALID) ||
        (smp->slot.state == SG_RESOURCESTATE_FAILED)) {
...
SOKOL_API_IMPL void sg_uninit_attachments(sg_attachments atts_id) {
  ...
  _sg_attachments_t* atts = _sg_lookup_attachments(&_sg.pools, atts_id.id);
  if (atts) {
    if ((atts->slot.state == SG_RESOURCESTATE_VALID) ||
        (atts->slot.state == SG_RESOURCESTATE_FAILED)) {
...
SOKOL_API_IMPL sg_resource_state
sg_query_attachments_state(sg_attachments atts_id) {
  SOKOL_ASSERT(_sg.valid);
  _sg_attachments_t* atts = _sg_lookup_attachments(&_sg.pools, atts_id.id);
...
SOKOL_API_IMPL void sg_begin_pass(const sg_pass* pass) {
  ...
  if (!pass_def.compute) {
    if (pass_def.attachments.id != SG_INVALID_ID) {
      ...
      _sg.cur_pass.atts =
          _sg_lookup_attachments(&_sg.pools, pass_def.attachments.id);
  ...
  _sg.cur_pass.valid =
      true;  // may be overruled by backend begin-pass functions
...
SOKOL_API_IMPL void sg_apply_viewport(int x,
                                      int y,
                                      int width,
                                      int height,
                                      bool origin_top_left) {
...
SOKOL_API_IMPL void sg_apply_viewportf(float x,
                                       float y,
                                       float width,
                                       float height,
                                       bool origin_top_left) {
...
SOKOL_API_IMPL void sg_apply_scissor_rect(int x,
                                          int y,
                                          int width,
                                          int height,
                                          bool origin_top_left) {
...
SOKOL_API_IMPL void sg_apply_scissor_rectf(float x,
                                           float y,
                                           float width,
                                           float height,
                                           bool origin_top_left) {
  sg_apply_scissor_rect((int)x, (int)y, (int)width, (int)height,
                        origin_top_left);
}
...
SOKOL_API_IMPL void sg_apply_pipeline(sg_pipeline pip_id) {
  ...
  _sg.required_bindings_and_uniforms =
      pip->cmn.required_bindings_and_uniforms |
      pip->shader->cmn.required_bindings_and_uniforms;
...
SOKOL_API_IMPL void sg_apply_bindings(const sg_bindings* bindings) {
  ...
  if (!_sg.cur_pass.is_compute) {
    for (size_t i = 0; i < SG_MAX_VERTEXBUFFER_BINDSLOTS; i++) {
      if (bnd.pip->cmn.vertex_buffer_layout_active[i]) {
        ...
        if (bnd.vbs[i]) {
          _sg.next_draw_valid &=
              (SG_RESOURCESTATE_VALID == bnd.vbs[i]->slot.state);
  ...
  for (int i = 0; i < SG_MAX_IMAGE_BINDSLOTS; i++) {
    if (shd->cmn.images[i].stage != SG_SHADERSTAGE_NONE) {
      ...
      if (bnd.imgs[i]) {
        _sg.next_draw_valid &=
            (SG_RESOURCESTATE_VALID == bnd.imgs[i]->slot.state);
      } else {
        _sg.next_draw_valid = false;
      }
    }
  ...
  for (size_t i = 0; i < SG_MAX_SAMPLER_BINDSLOTS; i++) {
    if (shd->cmn.samplers[i].stage != SG_SHADERSTAGE_NONE) {
      ...
      if (bnd.smps[i]) {
        _sg.next_draw_valid &=
            (SG_RESOURCESTATE_VALID == bnd.smps[i]->slot.state);
      } else {
        _sg.next_draw_valid = false;
      }
    }
  ...
  for (size_t i = 0; i < SG_MAX_STORAGEBUFFER_BINDSLOTS; i++) {
    if (shd->cmn.storage_buffers[i].stage != SG_SHADERSTAGE_NONE) {
      ...
      if (bnd.sbufs[i]) {
        _sg.next_draw_valid &=
            (SG_RESOURCESTATE_VALID == bnd.sbufs[i]->slot.state);
        if (_sg.cur_pass.is_compute) {
...
SOKOL_API_IMPL void sg_draw(int base_element,
                            int num_elements,
                            int num_instances) {
  SOKOL_ASSERT(_sg.valid);
#if defined(SOKOL_DEBUG)
...
SOKOL_API_IMPL void sg_dispatch(int num_groups_x,
                                int num_groups_y,
                                int num_groups_z) {
  SOKOL_ASSERT(_sg.valid);
#if defined(SOKOL_DEBUG)
...
SOKOL_API_IMPL bool sg_query_buffer_will_overflow(sg_buffer buf_id,
                                                  size_t size) {
  SOKOL_ASSERT(_sg.valid);
  _sg_buffer_t* buf = _sg_lookup_buffer(&_sg.pools, buf_id.id);
  bool result = false;
...
SOKOL_API_IMPL void sg_update_image(sg_image img_id,
                                    const sg_image_data* data) {
  SOKOL_ASSERT(_sg.valid);
  ...
  for (int face_index = 0; face_index < SG_CUBEFACE_NUM; face_index++) {
    for (int mip_index = 0; mip_index < SG_MAX_MIPMAPS; mip_index++) {
      ...
      _sg_stats_add(size_update_image,
                    (uint32_t)data->subimage[face_index][mip_index].size);
    }
  }
  _sg_image_t* img = _sg_lookup_image(&_sg.pools, img_id.id);
  if (img && img->slot.state == SG_RESOURCESTATE_VALID) {
...
SOKOL_API_IMPL sg_attachments_info
sg_query_attachments_info(sg_attachments atts_id) {
  SOKOL_ASSERT(_sg.valid);
  sg_attachments_info info;
  _sg_clear(&info, sizeof(info));
  const _sg_attachments_t* atts =
      _sg_lookup_attachments(&_sg.pools, atts_id.id);
  if (atts) {
...
SOKOL_API_IMPL sg_shader_desc sg_query_shader_desc(sg_shader shd_id) {
  ...
  if (shd) {
    ...
    for (size_t sbuf_idx = 0; sbuf_idx < SG_MAX_STORAGEBUFFER_BINDSLOTS;
      ...
      const _sg_shader_storage_buffer_t* sbuf =
          &shd->cmn.storage_buffers[sbuf_idx];
    ...
    for (size_t img_smp_idx = 0; img_smp_idx < SG_MAX_IMAGE_SAMPLER_PAIRS;
         img_smp_idx++) {
      sg_shader_image_sampler_pair* img_smp_desc =
          &desc.image_sampler_pairs[img_smp_idx];
      const _sg_shader_image_sampler_t* img_smp =
          &shd->cmn.image_samplers[img_smp_idx];
...
SOKOL_API_IMPL sg_attachments_desc
sg_query_attachments_desc(sg_attachments atts_id) {
  SOKOL_ASSERT(_sg.valid);
  sg_attachments_desc desc;
  _sg_clear(&desc, sizeof(desc));
  const _sg_attachments_t* atts =
...
SOKOL_API_IMPL sg_d3d11_buffer_info
sg_d3d11_query_buffer_info(sg_buffer buf_id) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL sg_d3d11_sampler_info
sg_d3d11_query_sampler_info(sg_sampler smp_id) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL sg_d3d11_shader_info
sg_d3d11_query_shader_info(sg_shader shd_id) {
  SOKOL_ASSERT(_sg.valid);
...
sg_d3d11_query_pipeline_info(sg_pipeline pip_id) {
  SOKOL_ASSERT(_sg.valid);
...
sg_d3d11_query_attachments_info(sg_attachments atts_id) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL sg_mtl_sampler_info
sg_mtl_query_sampler_info(sg_sampler smp_id) {
  SOKOL_ASSERT(_sg.valid);
...
sg_mtl_query_pipeline_info(sg_pipeline pip_id) {
  SOKOL_ASSERT(_sg.valid);
...
sg_wgpu_query_sampler_info(sg_sampler smp_id) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL sg_wgpu_pipeline_info
sg_wgpu_query_pipeline_info(sg_pipeline pip_id) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL sg_wgpu_attachments_info
sg_wgpu_query_attachments_info(sg_attachments atts_id) {
  SOKOL_ASSERT(_sg.valid);
...
SOKOL_API_IMPL sg_gl_attachments_info
sg_gl_query_attachments_info(sg_attachments atts_id) {
  SOKOL_ASSERT(_sg.valid);

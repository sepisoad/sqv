...
/*
   ...
   SOKOL_WIN32_FORCE_MAIN or SOKOL_NO_ENTRY is defined) SOKOL_NO_ENTRY - define
   this if sokol_app.h shouldn't "hijack" the main() function SOKOL_APP_API_DECL
   - public function declaration prefix (default: extern) SOKOL_API_DECL - same
   as SOKOL_APP_API_DECL SOKOL_API_IMPL              - public function
   ...
   code have been taken from GLFW (http://www.glfw.org/).
    ...
    - on Linux with EGL: X11, Xi, Xcursor, EGL, GL (or GLESv2), dl, pthread,
   m(?)
    - on Linux with GLX: X11, Xi, Xcursor, GL, dl, pthread, m(?)
    - on Android: GLESv3, EGL, log, android
    - on Windows with the MSVC or Clang toolchains: no action needed, libs are
   defined in-source via pragma-comment-lib
    - on Windows with MINGW/MSYS2 gcc: compile with '-mwin32' so that _WIN32 is
   defined
   ...
   fetch-call): const void* ptr = response->data.ptr;
            }
            else {
...
typedef struct sapp_event {
  ...
  sapp_keycode
      key_code;        // the virtual key code, only valid in KEY_UP, KEY_DOWN
  ...
  float mouse_y;    // current vertical mouse position in pixels, always valid
                    // except during mouse lock
  float mouse_dx;   // relative horizontal mouse movement since last frame,
                    // always valid
  ...
  float scroll_x;   // horizontal mouse wheel scroll distance, valid in
                    // MOUSE_SCROLL events
  float scroll_y;   // vertical mouse wheel scroll distance, valid in
                    // MOUSE_SCROLL events
  int num_touches;  // number of valid items in the touches[] array
  sapp_touchpoint
      touches[SAPP_MAX_TOUCHPOINTS];  // current touch points, valid in
                                      // TOUCHES_BEGIN, TOUCHES_MOVED,
                                      // TOUCHES_ENDED
...
#define _SAPP_LOG_ITEMS                                                        \
  ...
  _SAPP_LOGITEM_XMACRO(WGPU_SWAPCHAIN_CREATE_SWAPCHAIN_FAILED,                 \
                       "wgpu: failed to create swapchain object")              \
  _SAPP_LOGITEM_XMACRO(                                                        \
...
typedef struct sapp_desc {
  ...
  bool
      enable_dragndrop;  // enable file dropping (drag'n'drop), default is false
  ...
  const char* html5_canvas_selector;  // css selector of the HTML5 canvas
                                      // element, default is "#canvas"
  bool html5_canvas_resize;  // if true, the HTML5 canvas size is set to
                             // sapp_desc.width/height, otherwise canvas size is
                             // tracked
  bool
      html5_preserve_drawing_buffer;  // HTML5 only: whether to preserve default
                                      // framebuffer content between frames
  bool html5_premultiplied_alpha;     // HTML5 only: whether the rendered pixels
                                      // use premultiplied alpha convention
  bool html5_ask_leave_site;          // initial state of the internal
                                      // html5_ask_leave_site flag (see
                                      // sapp_html5_ask_leave_site())
  bool html5_update_document_title;   // if true, update the HTML document.title
                                      // with sapp_desc.window_title
  bool html5_bubble_mouse_events;     // if true, mouse events will bubble up to
                                      // the web page
  bool html5_bubble_touch_events;     // same for touch events
  bool html5_bubble_wheel_events;     // same for wheel events
  bool html5_bubble_key_events;       // if true, bubble up *all* key events to
                                 // browser, not just key events that represent
                                 // characters
  bool html5_bubble_char_events;  // if true, bubble up character events to
                                  // browser
  bool
      html5_use_emsc_set_main_loop;  // if true, use emscripten_set_main_loop()
                                     // instead of
                                     // emscripten_request_animation_frame_loop()
  bool
      html5_emsc_set_main_loop_simulate_infinite_loop;  // this will be passed
                                                        // as the
                                                        // simulate_infinite_loop
                                                        // arg to
                                                        // emscripten_set_main_loop()
  bool ios_keyboard_resizes_canvas;  // if true, showing the iOS keyboard
                                     // shrinks the canvas
} sapp_desc;
...
typedef struct sapp_html5_fetch_request {
  ...
  void (*callback)(
      const sapp_html5_fetch_response*);  // response callback function pointer
                                          // (required)
...
SOKOL_APP_API_DECL void sapp_html5_fetch_dropped_file(
    const sapp_html5_fetch_request* request);

/* Metal: get bridged pointer to Metal device object */
SOKOL_APP_API_DECL const void* sapp_metal_get_device(void);
/* Metal: get bridged pointer to MTKView's current drawable of type
 * CAMetalDrawable */
SOKOL_APP_API_DECL const void* sapp_metal_get_current_drawable(void);
/* Metal: get bridged pointer to MTKView's depth-stencil texture of type
 * MTLTexture */
...
inline void sapp_run(const sapp_desc& desc) {
...
_SOKOL_PRIVATE int64_t _sapp_int64_muldiv(int64_t value,
                                          int64_t numer,
                                          int64_t denom) {
...
typedef BOOL(WINAPI* PFNWGLGETPIXELFORMATATTRIBIVARBPROC)(HDC,
                                                          int,
                                                          int,
                                                          UINT,
                                                          const int*,
...
typedef GLXWindow (*PFNGLXCREATEWINDOWPROC)(Display*,
                                            GLXFBConfig,
                                            Window,
                                            const int*);
...
static void _sapp_log(sapp_log_item log_item,
                      uint32_t log_level,
                      const char* msg,
                      uint32_t line_nr) {
...
_SOKOL_PRIVATE void _sapp_init_state(const sapp_desc* desc) {
  ...
  _sapp_strcpy(_sapp.desc.html5_canvas_selector, _sapp.html5_canvas_selector,
...
_SOKOL_PRIVATE void _sapp_macos_update_dimensions(void) {
  ...
  _sapp.macos.view.layer.contentsScale =
      _sapp.dpi_scale;  // NOTE: needed because we set layerContentsPlacement to
                        // a non-scaling value in windowWillStartLiveResize.
  ...
  if (dim_changed) {
#if defined(SOKOL_METAL)
    CGSize drawable_size = {(CGFloat)_sapp.framebuffer_width,
...
- (void)applicationDidFinishLaunching:(NSNotification*)aNotification {
  ...
  if ((_sapp.window_width == 0) || (_sapp.window_height == 0)) {
    ...
    if (_sapp.window_height == 0) {
      _sapp.window_height =
          (int)roundf((screen_rect.size.height * 4.0f) / 5.0f);
    }
  }
  ...
  const int glVersion =
      _sapp.desc.gl_major_version * 10 + _sapp.desc.gl_minor_version;
  switch (glVersion) {
    case 10:
...
- (void)windowWillStartLiveResize:(NSNotification*)notification {
  // Work around the MTKView resizing glitch by "anchoring" the layer to the
  // window corner opposite to the currently manipulated corner (or edge). This
  // prevents the content stretching back and forth during resizing. This is a
  // workaround for this issue: https://github.com/floooh/sokol/issues/700 Can
  // be removed if/when migrating to CAMetalLayer:
...
_SOKOL_PRIVATE void _sapp_macos_poll_input_events(void) {
  ...
  @autoreleasepool {
      for (;;) {
          // NOTE: using NSDefaultRunLoopMode here causes stuttering in the GL
  backend,
          // see: https://github.com/floooh/sokol/issues/486
          NSEvent* event = [NSApp nextEventMatchingMask:mask untilDate:nil
  inMode:NSEventTrackingRunLoopMode dequeue:YES]; if (event == nil) { break;
          }
...
- (void)flagsChanged:(NSEvent*)event {
  ...
  if (key_code != SAPP_KEYCODE_INVALID) {
    _sapp_macos_key_event(
        down ? SAPP_EVENTTYPE_KEY_DOWN : SAPP_EVENTTYPE_KEY_UP, key_code, false,
        _sapp_macos_mods(event));
  }
}
@end
...
_SOKOL_PRIVATE void _sapp_ios_update_dimensions(void) {
  ...
  _sapp.framebuffer_width =
      (int)roundf(screen_rect.size.width * _sapp.dpi_scale);
  _sapp.framebuffer_height =
      (int)roundf(screen_rect.size.height * _sapp.dpi_scale);
...
#else
    // nothing to do here, GLKView correctly respects the view's
    // contentScaleFactor
#endif
    if (!_sapp.first_frame) {
...
EMSCRIPTEN_KEEPALIVE void _sapp_emsc_invoke_fetch_cb(
    ...
    int success,
    int error_code,
    _sapp_html5_fetch_callback callback,
...
_SOKOL_PRIVATE EM_BOOL _sapp_emsc_pointerlockchange_cb(
    int emsc_type,
    const EmscriptenPointerlockChangeEvent* emsc_event,
    void* user_data) {
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_size_changed(int event_type,
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_mouse_cb(int emsc_type,
                    const EmscriptenMouseEvent* emsc_event,
                    void* user_data) {
  _SOKOL_UNUSED(user_data);
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_wheel_cb(int emsc_type,
                    const EmscriptenWheelEvent* emsc_event,
                    void* user_data) {
  _SOKOL_UNUSED(emsc_type);
  _SOKOL_UNUSED(user_data);
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_key_cb(int emsc_type,
                  const EmscriptenKeyboardEvent* emsc_event,
                  void* user_data) {
  _SOKOL_UNUSED(user_data);
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_touch_cb(int emsc_type,
                    const EmscriptenTouchEvent* emsc_event,
                    void* user_data) {
  _SOKOL_UNUSED(user_data);
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_focus_cb(int emsc_type,
...
_SOKOL_PRIVATE EM_BOOL
_sapp_emsc_blur_cb(int emsc_type,
                   const EmscriptenFocusEvent* emsc_event,
                   void* user_data) {
  _SOKOL_UNUSED(emsc_type);
  _SOKOL_UNUSED(emsc_event);
...
_SOKOL_PRIVATE void _sapp_emsc_wgpu_create_swapchain(void) {
  ...
  _sapp.wgpu.render_format =
      wgpuSurfaceGetPreferredFormat(_sapp.wgpu.surface, _sapp.wgpu.adapter);
  ...
  _sapp.wgpu.swapchain = wgpuDeviceCreateSwapChain(
      _sapp.wgpu.device, _sapp.wgpu.surface, &sc_desc);
...
_SOKOL_PRIVATE void _sapp_emsc_wgpu_request_device_cb(
...
_SOKOL_PRIVATE void _sapp_emsc_wgpu_request_adapter_cb(
...
#undef _SAPP_WGPU_MAX_REQUESTED_FEATURES
  ...
  wgpuAdapterRequestDevice(adapter, &dev_desc,
                           _sapp_emsc_wgpu_request_device_cb, 0);
}
...
_SOKOL_PRIVATE void _sapp_emsc_wgpu_init(void) {
  ...
  wgpuInstanceRequestAdapter(_sapp.wgpu.instance, 0,
                             _sapp_emsc_wgpu_request_adapter_cb, 0);
}

_SOKOL_PRIVATE void _sapp_emsc_wgpu_frame(void) {
  if (_sapp.wgpu.async_init_done) {
    _sapp.wgpu.swapchain_view =
        wgpuSwapChainGetCurrentTextureView(_sapp.wgpu.swapchain);
    _sapp_frame();
...
_SOKOL_PRIVATE void _sapp_emsc_run(const sapp_desc* desc) {
  ...
  const char* document_title =
      desc->html5_update_document_title ? _sapp.window_title : 0;
  ...
  if (_sapp.desc.html5_use_emsc_set_main_loop) {
    emscripten_set_main_loop(
        ...
        _sapp.desc.html5_emsc_set_main_loop_simulate_infinite_loop);
  } else {
...
_SOKOL_PRIVATE const _sapp_gl_fbconfig* _sapp_gl_choose_fbconfig(
    const _sapp_gl_fbconfig* desired,
    const _sapp_gl_fbconfig* alternatives,
    int count) {
...
// >>windows
#if defined(_SAPP_WIN32)
_SOKOL_PRIVATE bool _sapp_win32_utf8_to_wide(const char* src,
                                             wchar_t* dst,
                                             int dst_num_bytes) {
...
static inline HRESULT _sapp_dxgi_GetBuffer(IDXGISwapChain* self,
                                           UINT Buffer,
                                           REFIID riid,
                                           void** ppSurface) {
#if defined(__cplusplus)
...
static inline HRESULT _sapp_d3d11_QueryInterface(ID3D11Device* self,
                                                 REFIID riid,
                                                 void** ppvObject) {
#if defined(__cplusplus)
...
static inline HRESULT _sapp_d3d11_CreateRenderTargetView(
    ID3D11Device* self,
...
static inline HRESULT _sapp_dxgi_GetFrameStatistics(
    IDXGISwapChain* self,
...
static inline HRESULT _sapp_dxgi_GetParent(IDXGIObject* self,
                                           REFIID riid,
                                           void** ppParent) {
#if defined(__cplusplus)
  ...
  if (SUCCEEDED(hr) && _sapp.d3d11.dxgi_device) {
    ...
    if (SUCCEEDED(hr) && dxgi_adapter) {
      ...
      hr = _sapp_dxgi_GetParent((IDXGIObject*)dxgi_adapter,
                                ...
                                (void**)&dxgi_factory);
      if (SUCCEEDED(hr)) {
        _sapp_dxgi_MakeWindowAssociation(
            dxgi_factory, _sapp.win32.hwnd,
...
_SOKOL_PRIVATE void _sapp_d3d11_create_default_render_target(void) {
  ...
  hr = _sapp_d3d11_CreateRenderTargetView(_sapp.d3d11.device,
                                          (ID3D11Resource*)_sapp.d3d11.rt, NULL,
                                          &_sapp.d3d11.rtv);
  ...
  hr = _sapp_d3d11_CreateDepthStencilView(_sapp.d3d11.device,
                                          (ID3D11Resource*)_sapp.d3d11.ds, NULL,
                                          &_sapp.d3d11.dsv);
...
_SOKOL_PRIVATE void _sapp_wgl_attribiv(int pixel_format,
                                       int num_attribs,
                                       const int* attribs,
                                       int* results) {
  SOKOL_ASSERT(_sapp.wgl.arb_pixel_format);
...
#define _sapp_wgl_num_query_tags (12)
  ...
  for (int i = 0; i < native_count; i++) {
    ...
    u.samples =
        query_results[result_samples_index];  // NOTE: If arb_multisample is not
                                              // supported  - just takes the
                                              // default 0
...
_SOKOL_PRIVATE bool _sapp_win32_wide_to_utf8(const wchar_t* src,
                                             char* dst,
                                             int dst_num_bytes) {
  SOKOL_ASSERT(src && dst && (dst_num_bytes > 1));
  _sapp_clear(dst, (size_t)dst_num_bytes);
...
_SOKOL_PRIVATE void _sapp_win32_key_event(sapp_event_type type,
                                          int vk,
                                          bool repeat) {
  ...
  if (_sapp.d3d11.use_dxgi_frame_stats) {
    ...
    if (SUCCEEDED(hr)) {
      if (dxgi_stats.SyncRefreshCount != _sapp.d3d11.sync_refresh_count) {
        if ((_sapp.d3d11.sync_refresh_count + 1) !=
            dxgi_stats.SyncRefreshCount) {
...
_SOKOL_PRIVATE LRESULT CALLBACK _sapp_win32_wndproc(HWND hWnd,
                                                    UINT uMsg,
                                                    WPARAM wParam,
                                                    LPARAM lParam) {
    ...
    switch (uMsg) {
      ...
      case WM_SETCURSOR:
        if (LOWORD(lParam) == HTCLIENT) {
          _sapp_win32_update_cursor(_sapp.mouse.current_cursor,
                                    _sapp.mouse.shown, true);
          return TRUE;
        }
        break;
      ...
      case WM_RBUTTONUP:
        _sapp_win32_mouse_update(lParam);
        _sapp_win32_mouse_event(SAPP_EVENTTYPE_MOUSE_UP,
                                SAPP_MOUSEBUTTON_RIGHT);
        ...
        if (_sapp.mouse.locked) {
          ...
          if (raw_mouse_data->data.mouse.usFlags & MOUSE_MOVE_ABSOLUTE) {
            /* mouse only reports absolute position
               ...
               Remote Desktop sessions. (such remote desktop sessions are
               setting the MOUSE_MOVE_ABSOLUTE flag). See:
            ...
            */
            ...
            if (_sapp.win32.mouse.raw_input.pos_valid) {
              _sapp.mouse.dx =
                  (float)(new_x - _sapp.win32.mouse.raw_input.pos_x);
              _sapp.mouse.dy =
                  (float)(new_y - _sapp.win32.mouse.raw_input.pos_y);
            }
...
_SOKOL_PRIVATE void _sapp_win32_init_dpi(void) {
  ...
  if (shcore) {
    fn_setprocessdpiawareness = (SETPROCESSDPIAWARENESS_T)(void*)GetProcAddress(
...
_SOKOL_PRIVATE HICON
_sapp_win32_create_icon_from_image(const sapp_image_desc* desc) {
  BITMAPV5HEADER bi;
...
_SOKOL_PRIVATE void _sapp_win32_set_icon(const sapp_icon_desc* icon_desc,
  ...
  HICON big_icon =
      _sapp_win32_create_icon_from_image(&icon_desc->images[big_img_index]);
  HICON sml_icon =
      _sapp_win32_create_icon_from_image(&icon_desc->images[sml_img_index]);
...
_SOKOL_PRIVATE char** _sapp_win32_command_line_to_utf8_argv(
...
_SOKOL_PRIVATE void* _sapp_android_on_save_instance_state(
    ANativeActivity* activity,
    size_t* out_size) {
  _SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void _sapp_android_on_window_focus_changed(
    ANativeActivity* activity,
    int has_focus) {
  _SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void _sapp_android_on_native_window_created(
    ANativeActivity* activity,
    ANativeWindow* window) {
  _SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void _sapp_android_on_native_window_destroyed(
    ANativeActivity* activity,
    ANativeWindow* window) {
  _SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void _sapp_android_on_input_queue_created(
    ANativeActivity* activity,
    AInputQueue* queue) {
  _SOKOL_UNUSED(activity);
...
_SOKOL_PRIVATE void _sapp_android_on_input_queue_destroyed(
    ANativeActivity* activity,
    AInputQueue* queue) {
  _SOKOL_UNUSED(activity);
  _SOKOL_UNUSED(queue);
...
_SOKOL_PRIVATE void _sapp_x11_send_event(Atom type,
                                         int a,
                                         int b,
                                         int c,
                                         int d,
...
_SOKOL_PRIVATE unsigned long _sapp_x11_get_window_property(
    Window window,
    Atom property,
    Atom type,
    unsigned char** value) {
...
_SOKOL_PRIVATE void _sapp_x11_key_event(sapp_event_type type,
                                        sapp_keycode key,
                                        bool repeat,
                                        uint32_t mods) {
  if (_sapp_events_enabled()) {
...
_SOKOL_PRIVATE void _sapp_x11_char_event(uint32_t chr,
                                         bool repeat,
                                         uint32_t mods) {
  if (_sapp_events_enabled()) {
    _sapp_init_event(SAPP_EVENTTYPE_CHAR);
...
_SOKOL_PRIVATE void _sapp_x11_on_clientmessage(XEvent* event) {
  ...
  if (event->xclient.message_type == _sapp.x11.WM_PROTOCOLS) {
    ...
    Atom* formats = 0;
    if (is_list) {
      count = _sapp_x11_get_window_property(_sapp.x11.xdnd.source,
                                            _sapp.x11.xdnd.XdndTypeList,
                                            XA_ATOM, (unsigned char**)&formats);
    } else {
      count = 3;
...
_SOKOL_PRIVATE void _sapp_x11_on_selectionrequest(XEvent* event) {
  ...
  if (req->target == _sapp.x11.UTF8_STRING) {
    ...
    XChangeProperty(_sapp.x11.display, req->requestor, req->property, XA_ATOM,
                    32, PropModeReplace, (unsigned char*)&_sapp.x11.UTF8_STRING,
                    1);
  } else {
    reply.property = None;
  }
...
SOKOL_API_IMPL void sapp_html5_fetch_dropped_file(
    const sapp_html5_fetch_request* request) {
  SOKOL_ASSERT(_sapp.drop.enabled);
  ...
  if (SAPP_HTML5_FETCH_ERROR_NO_ERROR != error_code) {
    _sapp_emsc_invoke_fetch_cb(index,
                               ...
                               0,  // fetched_size
                               (void*)request->buffer.ptr, request->buffer.size,
                               request->user_data);
  } else {

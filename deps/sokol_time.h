...
_SOKOL_PRIVATE int64_t _stm_int64_muldiv(int64_t value,
                                         int64_t numer,
                                         int64_t denom) {
...
SOKOL_API_IMPL uint64_t stm_now(void) {
  ...
  now = (uint64_t)_stm_int64_muldiv(qpc_t.QuadPart - _stm.start.QuadPart,
                                    1000000000, _stm.freq.QuadPart);
#elif defined(__APPLE__) && defined(__MACH__)
  const uint64_t mach_now = mach_absolute_time() - _stm.start;
  now = (uint64_t)_stm_int64_muldiv((int64_t)mach_now,

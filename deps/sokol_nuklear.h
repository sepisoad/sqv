...
typedef struct snk_logger_t {
  void (*func)(const char* tag,       // always "snk"
...
SOKOL_NUKLEAR_API_DECL nk_flags snk_edit_string(struct nk_context* ctx,
                                                nk_flags flags,
                                                char* memory,
                                                int* len,
                                                int max,
...
inline void snk_setup(const snk_desc_t& desc) {
  return snk_setup(&desc);
}
...
static void _snk_log(snk_log_item_t log_item,
                     uint32_t log_level,
                     const char* msg,
                     uint32_t line_nr) {
...
#endif
    _snuklear.desc.logger.func("snk", log_level, (uint32_t)log_item, msg,
                               line_nr, filename,
...
static void* _snk_malloc(size_t size) {
  ...
  if (_snuklear.desc.allocator.alloc_fn) {
    ptr = _snuklear.desc.allocator.alloc_fn(size,
                                            _snuklear.desc.allocator.user_data);
  } else {
    ptr = malloc(size);
  }
  if (0 == ptr) {
...
static void _snk_pool_free_index(_snk_pool_t* pool, int slot_index) {
  SOKOL_ASSERT((slot_index > _SNK_INVALID_SLOT_INDEX) &&
...
*/
static uint32_t _snk_slot_init(_snk_pool_t* pool,
                               _snk_slot_t* slot,
                               int slot_index) {
  ...
  */
  ...
  SOKOL_ASSERT((slot_index > _SNK_INVALID_SLOT_INDEX) &&
               (slot_index < pool->size));
  SOKOL_ASSERT((slot->state == _SNK_RESOURCESTATE_INITIAL) &&
...
static void _snk_init_item_pool(_snk_pool_t* pool,
                                int pool_size,
                                void** items_ptr,
...
static void _snk_setup_image_pool(int pool_size) {
  ...
  _snk_init_item_pool(&p->pool, pool_size, (void**)&p->items,
                      sizeof(_snk_image_t));
}
...
static _snk_resource_state _snk_init_image(_snk_image_t* img,
                                           const snk_image_desc_t* desc) {
...
SOKOL_API_IMPL void snk_setup(const snk_desc_t* desc) {
  ...
  _snuklear.vbuf =
      sg_make_buffer(&(sg_buffer_desc){.usage = SG_USAGE_STREAM,
  ...
  if (!_snuklear.desc.no_default_font) {
    ...
    const void* pixels = nk_font_atlas_bake(&_snuklear.atlas, &font_width,
    ...
    nk_font_atlas_end(&_snuklear.atlas, snk_nkhandle(_snuklear.default_font),
                      0);
...
// shader
  ...
  _snuklear.pip = sg_make_pipeline(&(sg_pipeline_desc){
      .layout = {.attrs = {[0] = {.offset = offsetof(_snk_vertex_t, pos),
                                  .format = SG_VERTEXFORMAT_FLOAT2},
                           [1] = {.offset = offsetof(_snk_vertex_t, uv),
                                  .format = SG_VERTEXFORMAT_FLOAT2},
                           [2] = {.offset = offsetof(_snk_vertex_t, col),
...
SOKOL_API_IMPL struct nk_context* snk_new_frame(void) {
  ...
  if (_snuklear.mouse_did_move) {
    nk_input_motion(&_snuklear.ctx, _snuklear.mouse_pos[0],
                    _snuklear.mouse_pos[1]);
    _snuklear.mouse_did_move = false;
  }
  if (_snuklear.mouse_did_scroll) {
    nk_input_scroll(&_snuklear.ctx, nk_vec2(_snuklear.mouse_scroll[0],
  ...
  for (int i = 0; i < NK_BUTTON_MAX; i++) {
    if (_snuklear.btn_down[i]) {
      ...
      nk_input_button(&_snuklear.ctx, (enum nk_buttons)i,
...
SOKOL_API_IMPL snk_image_t snk_make_image(const snk_image_desc_t* desc) {
  ...
  if (img) {
    ...
    SOKOL_ASSERT((img->slot.state == _SNK_RESOURCESTATE_VALID) ||
...
SOKOL_API_IMPL void snk_render(int width, int height) {
  ...
  if (_snuklear.desc.enable_set_mouse_cursor) {
    for (enum nk_style_cursor c = 0; c < NK_CURSOR_COUNT; ++c) {
      if (_snuklear.ctx.style.cursor_active == _snuklear.ctx.style.cursors[c]) {
        ...
        switch (c) {
          case NK_CURSOR_TEXT:
            sapp_cur = SAPP_MOUSECURSOR_IBEAM;
            break;
          case NK_CURSOR_MOVE:
            sapp_cur = SAPP_MOUSECURSOR_RESIZE_ALL;
            break;
          case NK_CURSOR_RESIZE_VERTICAL:
            sapp_cur = SAPP_MOUSECURSOR_RESIZE_NS;
            break;
          case NK_CURSOR_RESIZE_HORIZONTAL:
            sapp_cur = SAPP_MOUSECURSOR_RESIZE_EW;
            break;
          case NK_CURSOR_RESIZE_TOP_LEFT_DOWN_RIGHT:
            sapp_cur = SAPP_MOUSECURSOR_RESIZE_NESW;
            break;
          case NK_CURSOR_RESIZE_TOP_RIGHT_DOWN_LEFT:
            sapp_cur = SAPP_MOUSECURSOR_RESIZE_NWSE;
            break;
          default:
            break;
        }
  ...
  struct nk_convert_config cfg = {.shape_AA = NK_ANTI_ALIASING_ON,
  ...
  if (!vertex_buffer_overflow && !index_buffer_overflow) {
    // Setup rendering
    sg_update_buffer(_snuklear.vbuf, &(sg_range){nk_buffer_memory_const(&verts),
                                                 nk_buffer_total(&verts)});
    sg_update_buffer(_snuklear.ibuf, &(sg_range){nk_buffer_memory_const(&idx),
                                                 nk_buffer_total(&idx)});
    ...
    int idx_offset = 0;
    sg_bindings bindings = {.vertex_buffers[0] = _snuklear.vbuf,
...
SOKOL_API_IMPL bool snk_handle_event(const sapp_event* ev) {
  ...
  switch (ev->type) {
    ...
    case SAPP_EVENTTYPE_CLIPBOARD_PASTED:
      _snuklear.keys_down[NK_KEY_PASTE] = _snuklear.keys_up[NK_KEY_PASTE] =
          true;
      break;
    default:
      break;
...
SOKOL_API_IMPL nk_flags snk_edit_string(struct nk_context* ctx,
                                        nk_flags flags,
                                        char* memory,
                                        int* len,
                                        int max,

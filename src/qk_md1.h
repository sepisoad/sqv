#ifndef QK_MD1_HEADER_
#define QK_MD1_HEADER_
...
typedef f32 qk_md1_vectorf;

typedef qk_md1_vectorf qk_md1_vector3f[3];

typedef i32 qk_md1_triangle[3];

typedef u8 qk_md1_raw_vertex[3];
...
typedef struct {
  i32 magic_codes;
  i32 version;
  qk_md1_vector3f scale;
  qk_md1_vector3f translate;
  f32 bounding_radius;
  qk_md1_vector3f eye_position;
  i32 skins_length;
  i32 skin_width;
  i32 skin_height;
  i32 vertices_length;
...
typedef struct {
  i32 frontface;
  qk_md1_triangle vertices_idx;
} qk_md1_faced_triangle;

typedef struct {
  qk_md1_raw_vertex vertex;
  u8 normal_idx;
} qk_md1_normal_vertex;

typedef struct {
  qk_md1_normal_vertex bbox_min;
  qk_md1_normal_vertex bbox_max;
  char name[16];
} md1_frame_single;
...
typedef struct {
  qk_md1_header header;
  qk_md1_skin* skins;
  qk_md1_vertex* vertices;
  qk_md1_pose* poses;
  f32* vbuf;
  arena mem;
} qk_md1_model;
...
/* ****************** quake::mdl API ****************** */
qk_md1_error qk_load_mdl(const u8*, sz, qk_md1_model*);
void qk_get_frame_vertices(const qk_md1_model*, u32, u32, const f32**, u32*);
void qk_unload_mdl(qk_md1_model*);
/* ****************** quake::mdl API ****************** */
...
#ifdef QK_MD1_IMPLEMENTATION

#include "qk_md1_data.h"
...
static void md1_estimate_memory(arena* mem,
  ...
  sz skin_sz = sizeof(qk_md1_skin) * skn_len;
  arena_estimate_add(mem, skin_sz, alignof(qk_md1_skin));
  ...
  sz texcoord_sz = sizeof(qk_md1_st) * vrt_len;
  arena_estimate_add(mem, texcoord_sz, alignof(qk_md1_st));

  // Estimate memory for triangle indices
  sz trisix_sz = sizeof(qk_md1_faced_triangle) * tri_len;
  arena_estimate_add(mem, trisix_sz, alignof(qk_md1_faced_triangle));

  const u8* p = (const u8*)rhdr + sizeof(qk_md1_raw_header);
  const u8* pend = (const u8*)rhdr + bufsz;

  // Advance past skin data
  for (u32 i = 0; i < hdr->skins_length; i++) {
    qk_md1_skin_type* skin_type = (qk_md1_skin_type*)p;
    p += sizeof(qk_md1_skin_type);
  ...
  p += sizeof(qk_md1_st) * vrt_len;
  // Advance past triangle indices
  p += sizeof(qk_md1_faced_triangle) * tri_len;
  ...
  for (u32 i = 0; i < frm_len; i++) {
    if (p + sizeof(qk_md1_frame_type) > pend) {
      mustdie("failed to parse frames data");
    }

    qk_md1_frame_type ft = *(const qk_md1_frame_type*)p;
    p += sizeof(qk_md1_frame_type);

    if (ft == MD1_FT_SINGLE) {
      if (p + sizeof(md1_frame_single) +
              (vrt_len * sizeof(qk_md1_normal_vertex)) >
          pend) {
        mustdie("failed to parse frames data");
      }
      ...
      p += sizeof(md1_frame_single) + (vrt_len * sizeof(qk_md1_normal_vertex));
    } else {
  ...
  sz raw_verts_sz = sizeof(qk_md1_vertex) * vrt_len * frm_len;
  arena_estimate_add(mem, raw_verts_sz, alignof(qk_md1_vertex));

  // Add pose memory estimation
  sz poses_sz = sizeof(qk_md1_pose) * pos_len;
  arena_estimate_add(mem, poses_sz, alignof(qk_md1_pose));
...
static const u8* md1_load_skins(qk_md1_model* mdl, const u8* p) {
  const qk_md1_header* hdr = &mdl->header;
  arena* mem = &mdl->mem;
  u32 width = hdr->skin_width;
  u32 height = hdr->skin_height;
  sz skin_size = width * height;

  sz memsz = sizeof(qk_md1_skin) * hdr->skins_length;
  qk_md1_skin* skins =
      (qk_md1_skin*)arena_alloc(mem, memsz, alignof(qk_md1_skin));
  notnull(skins);
...
static const u8* md1_load_st(qk_md1_model* mdl,
                             const u8* p,
                             qk_md1_st** coords) {
  DBG("loading texture S/T coordinates");
  arena* mem = &mdl->mem;
  const qk_md1_header* hdr = &mdl->header;
  sz mem_sz = sizeof(qk_md1_st) * hdr->vertices_length;
  *coords = (qk_md1_st*)arena_alloc(mem, mem_sz, alignof(qk_md1_st));
  notnull(coords);

  const qk_md1_st* src = (const qk_md1_st*)p;
...
static const u8* md1_load_triangles(qk_md1_model* mdl,
                                    const u8* p,
                                    const qk_md1_header* hdr,
                                    qk_md1_faced_triangle** ftris) {
  DBG("loading triangles");
  arena* mem = &mdl->mem;
  sz mem_sz = sizeof(qk_md1_faced_triangle) * hdr->triangles_length;
  *ftris = (qk_md1_faced_triangle*)arena_alloc(mem, mem_sz,
                                               alignof(qk_md1_faced_triangle));
  notnull(ftris);

  const qk_md1_faced_triangle* src =
      (const qk_md1_faced_triangle*)p;  // Use separate pointer
...
static const u8* md1_load_single_frame(qk_md1_model* mdl,
                                       u32 frm_idx,
                                       u32* pos_len,
                                       u32* pos_idx,
                                       char* oldname,
  ...
  *pos_len = *pos_len + 1;
  ...
  qk_md1_normal_vertex* raw_verts = (qk_md1_normal_vertex*)(snl + 1);
  qk_md1_vertex* frmverts = mdl->vertices + (hdr->vertices_length * frm_idx);
...
static const u8* md1_load_frames(qk_md1_model* mdl, const u8* p) {
  DBG("loading frames");
  arena* mem = &mdl->mem;
  const qk_md1_header* hdr = &mdl->header;
  ...
  sz verts_sz = sizeof(qk_md1_vertex) * verts_length * frames_length;
  mdl->vertices =
      (qk_md1_vertex*)arena_alloc(mem, verts_sz, alignof(qk_md1_vertex));
  notnull(mdl->vertices);

  sz poses_sz = sizeof(qk_md1_pose) * mdl->header.poses_length;
  mdl->poses = (qk_md1_pose*)arena_alloc(mem, poses_sz, alignof(qk_md1_pose));
  notnull(mdl->poses);
  ...
  for (u32 i = 0; i < mdl->header.frames_length; i++) {
    qk_md1_frame_type ft = endian_i32(*(qk_md1_frame_type*)p);
    p += sizeof(qk_md1_frame_type);
...
static void md1_make_display_list(qk_md1_model* mdl,
                                  const qk_md1_st* coords,
                                  const qk_md1_faced_triangle* ftris) {
  DBG("generating vertex buffer data");
  arena* mem = &mdl->mem;
  qk_md1_header* hdr = &mdl->header;
  ...
  for (u32 frm_idx = 0; frm_idx < frm_len; frm_idx++) {
    for (u32 tri_idx = 0; tri_idx < tri_len; tri_idx++) {
      for (u8 vrt_idx = 0; vrt_idx < 3; vrt_idx++) {
        ...
        const qk_md1_vertex* vrts = mdl->vertices + (vrt_len * frm_idx);
...
void md1_scale_translate_bbox(qk_md1_model* mdl) {
...
void qk_get_frame_vertices(const qk_md1_model* mdl,
                           u32 pos_idx,
                           u32 frm_idx,
                           const f32** vbuf,
                           u32* vbuf_len) {
  ...
  qk_md1_pose* pos = &mdl->poses[pos_idx];
  *vbuf = &mdl->vbuf[(pos->start + frm_idx) * mdl->header.vbuf_length];
  *vbuf_len = mdl->header.vbuf_length;
}

qk_md1_error qk_load_mdl(const u8* buf, sz bufsz, qk_md1_model* mdl) {
  const u8* p = buf;
  const qk_md1_raw_header* rhdr = (qk_md1_raw_header*)buf;

  memset(mdl, 0, sizeof(qk_md1_model));

  DBG("loading MD1 haeder ...");
  mdl->header = (qk_md1_header){
  ...
  qk_md1_header* hdr = &mdl->header;
  arena* mem = &mdl->mem;
  qk_md1_st* stcoords = NULL;
  qk_md1_faced_triangle* ftris = NULL;
...
void qk_unload_mdl(qk_md1_model* mdl) {
...
#endif  // QK_MD1_IMPLEMENTATION
#endif  // QK_MD1_HEADER_

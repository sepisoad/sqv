...
void
update_offscreen_target(State* s, I32 width, I32 height) {
...
void
create_offscreen_target(State* s, CStr path) {
  if(s->ctx3d != NULL) {
...
render_md1(State* s) {
  ...
  &(sg_range) {
    .ptr = vb, .size = (size_t)vb_len * sizeof(F32)
  });

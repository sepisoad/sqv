...
local backend = require("nuklear.backend")
-- These are some examples to provide a small overview of what can be done
-- with this library:
local style = require("nuklear.style")
local overview = require("nuklear.overview")
local calculator = require("nuklear.calculator")
local node_editor = require("nuklear.node_editor")
...
local WINDOW_WIDTH, WINDOW_HEIGHT = 400, 470
...
local window_flags = nk.panelflags()

local function demogui(ctx)
   if nk.window_begin(ctx, "Demo", {0, 0, WINDOW_WIDTH, WINDOW_HEIGHT}, window_flags) then
      --  GUI 
...
ctx = backend.init(WINDOW_WIDTH, WINDOW_HEIGHT, "MoonNuklear Demo", true, nil)
set_style(ctx, THEME)
